...
class JohnLennonResourceIT {
    ...
    void createJohnLennon() throws Exception {
        ...
        var returnedJohnLennon = om.readValue(
            restJohnLennonMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(johnLennon)))
    ...
    @Test
    @Transactional
    void getAllJohnLennonsByJelloInitiumViIsEqualToSomething() throws Exception {
        InitiumVi jelloInitiumVi;
        if (TestUtil.findAll(em, InitiumVi.class).isEmpty()) {
            johnLennonRepository.saveAndFlush(johnLennon);
            jelloInitiumVi = InitiumViResourceIT.createEntity();
        } else {
            jelloInitiumVi = TestUtil.findAll(em, InitiumVi.class).get(0);
        }
        em.persist(jelloInitiumVi);
        em.flush();
        johnLennon.setJelloInitiumVi(jelloInitiumVi);
        johnLennonRepository.saveAndFlush(johnLennon);
        Long jelloInitiumViId = jelloInitiumVi.getId();
        // Get all the johnLennonList where jelloInitiumVi equals to jelloInitiumViId
        defaultJohnLennonShouldBeFound("jelloInitiumViId.equals=" + jelloInitiumViId);

        // Get all the johnLennonList where jelloInitiumVi equals to (jelloInitiumViId + 1)
        defaultJohnLennonShouldNotBeFound("jelloInitiumViId.equals=" + (jelloInitiumViId + 1));
    }

    @Test
    @Transactional
    void getAllJohnLennonsByInhouseInitiumViIsEqualToSomething() throws Exception {
        InitiumVi inhouseInitiumVi;
        if (TestUtil.findAll(em, InitiumVi.class).isEmpty()) {
            johnLennonRepository.saveAndFlush(johnLennon);
            inhouseInitiumVi = InitiumViResourceIT.createEntity();
        } else {
            inhouseInitiumVi = TestUtil.findAll(em, InitiumVi.class).get(0);
        }
        em.persist(inhouseInitiumVi);
        em.flush();
        johnLennon.setInhouseInitiumVi(inhouseInitiumVi);
        johnLennonRepository.saveAndFlush(johnLennon);
        Long inhouseInitiumViId = inhouseInitiumVi.getId();
        // Get all the johnLennonList where inhouseInitiumVi equals to inhouseInitiumViId
        defaultJohnLennonShouldBeFound("inhouseInitiumViId.equals=" + inhouseInitiumViId);

        // Get all the johnLennonList where inhouseInitiumVi equals to (inhouseInitiumViId + 1)
        defaultJohnLennonShouldNotBeFound("inhouseInitiumViId.equals=" + (inhouseInitiumViId + 1));
    }
    ...
    void putExistingJohnLennon() throws Exception {
        ...
        restJohnLennonMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedJohnLennon.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedJohnLennon))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingJohnLennon() throws Exception {
        ...
        restJohnLennonMockMvc
            .perform(
                put(ENTITY_API_URL_ID, johnLennon.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(johnLennon))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithIdMismatchJohnLennon() throws Exception {
        ...
        restJohnLennonMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(johnLennon))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamJohnLennon() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restJohnLennonMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(johnLennon)))
    ...
    void patchNonExistingJohnLennon() throws Exception {
        ...
        restJohnLennonMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, johnLennon.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(johnLennon))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamJohnLennon() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restJohnLennonMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(johnLennon)))
            .andExpect(status().isMethodNotAllowed());
