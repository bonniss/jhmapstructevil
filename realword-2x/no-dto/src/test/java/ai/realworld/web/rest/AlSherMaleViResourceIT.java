...
class AlSherMaleViResourceIT {
    ...
    void createAlSherMaleVi() throws Exception {
        ...
        var returnedAlSherMaleVi = om.readValue(
            restAlSherMaleViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alSherMaleVi)))
    ...
    void putExistingAlSherMaleVi() throws Exception {
        ...
        restAlSherMaleViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlSherMaleVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlSherMaleVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlSherMaleVi() throws Exception {
        ...
        restAlSherMaleViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alSherMaleVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alSherMaleVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlSherMaleVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlSherMaleViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alSherMaleVi)))
    ...
    void patchNonExistingAlSherMaleVi() throws Exception {
        ...
        restAlSherMaleViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alSherMaleVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alSherMaleVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlSherMaleVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlSherMaleViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alSherMaleVi)))
            .andExpect(status().isMethodNotAllowed());
