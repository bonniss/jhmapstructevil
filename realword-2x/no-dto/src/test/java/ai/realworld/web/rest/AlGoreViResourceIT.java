...
class AlGoreViResourceIT {
    ...
    void createAlGoreVi() throws Exception {
        ...
        // Create the AlGoreVi
        var returnedAlGoreVi = om.readValue(
            restAlGoreViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreVi)))
    ...
    void putExistingAlGoreVi() throws Exception {
        ...
        restAlGoreViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlGoreVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlGoreVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlGoreVi() throws Exception {
        ...
        restAlGoreViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alGoreVi.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreVi))
            )
            .andExpect(status().isBadRequest());
        ...
        restAlGoreViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alGoreVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlGoreVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreVi)))
    ...
    void patchNonExistingAlGoreVi() throws Exception {
        ...
        restAlGoreViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alGoreVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alGoreVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlGoreVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alGoreVi)))
            .andExpect(status().isMethodNotAllowed());
