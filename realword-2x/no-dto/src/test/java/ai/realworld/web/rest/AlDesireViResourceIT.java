...
class AlDesireViResourceIT {
    ...
    void createAlDesireVi() throws Exception {
        ...
        var returnedAlDesireVi = om.readValue(
            restAlDesireViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alDesireVi)))
    ...
    void putExistingAlDesireVi() throws Exception {
        ...
        restAlDesireViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlDesireVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlDesireVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlDesireVi() throws Exception {
        ...
        restAlDesireViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alDesireVi.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alDesireVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithIdMismatchAlDesireVi() throws Exception {
        ...
        restAlDesireViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alDesireVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlDesireVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlDesireViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alDesireVi)))
    ...
    void patchNonExistingAlDesireVi() throws Exception {
        ...
        restAlDesireViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alDesireVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alDesireVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlDesireVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlDesireViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alDesireVi)))
            .andExpect(status().isMethodNotAllowed());
