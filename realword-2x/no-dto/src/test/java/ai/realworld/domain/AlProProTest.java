...
class AlProProTest {
    ...
    void amenityTest() {
        AlProPro alProPro = getAlProProRandomSampleGenerator();
        AlMenity alMenityBack = getAlMenityRandomSampleGenerator();

        alProPro.addAmenity(alMenityBack);
        assertThat(alProPro.getAmenities()).containsOnly(alMenityBack);

        alProPro.removeAmenity(alMenityBack);
        assertThat(alProPro.getAmenities()).doesNotContain(alMenityBack);

        alProPro.amenities(new HashSet<>(Set.of(alMenityBack)));
        assertThat(alProPro.getAmenities()).containsOnly(alMenityBack);

        alProPro.setAmenities(new HashSet<>());
        assertThat(alProPro.getAmenities()).doesNotContain(alMenityBack);
    }
    ...
    void imageTest() {
        ...
        alProPro.addImage(metaverseBack);
        assertThat(alProPro.getImages()).containsOnly(metaverseBack);

        alProPro.removeImage(metaverseBack);
        assertThat(alProPro.getImages()).doesNotContain(metaverseBack);

        alProPro.images(new HashSet<>(Set.of(metaverseBack)));
        assertThat(alProPro.getImages()).containsOnly(metaverseBack);

        alProPro.setImages(new HashSet<>());
        assertThat(alProPro.getImages()).doesNotContain(metaverseBack);
    }
