...
class AlMemTierViResourceIT {
    ...
    void createAlMemTierVi() throws Exception {
        ...
        AlMemTierViDTO alMemTierViDTO = alMemTierViMapper.toDto(alMemTierVi);
        var returnedAlMemTierViDTO = om.readValue(
            restAlMemTierViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alMemTierViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlMemTierViDTO.class
        );

        // Validate the AlMemTierVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlMemTierVi = alMemTierViMapper.toEntity(returnedAlMemTierViDTO);
    ...
    void putExistingAlMemTierVi() throws Exception {
        ...
        AlMemTierViDTO alMemTierViDTO = alMemTierViMapper.toDto(updatedAlMemTierVi);

        restAlMemTierViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alMemTierViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alMemTierViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlMemTierVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlMemTierViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alMemTierViDTO)))
    ...
    void patchNonExistingAlMemTierVi() throws Exception {
        ...
        restAlMemTierViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alMemTierViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alMemTierViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlMemTierVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlMemTierViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alMemTierViDTO)))
            .andExpect(status().isMethodNotAllowed());
