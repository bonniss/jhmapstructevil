...
class AlGoreResourceIT {
    ...
    void createAlGore() throws Exception {
        ...
        // Create the AlGore
        AlGoreDTO alGoreDTO = alGoreMapper.toDto(alGore);
        var returnedAlGoreDTO = om.readValue(
            restAlGoreMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreDTO)))
        ...
        var returnedAlGore = alGoreMapper.toEntity(returnedAlGoreDTO);
    ...
    @Test
    @Transactional
    void getAllAlGoresByBizRelationViIsEqualToSomething() throws Exception {
        AlBetonamuRelationVi bizRelationVi;
        if (TestUtil.findAll(em, AlBetonamuRelationVi.class).isEmpty()) {
            alGoreRepository.saveAndFlush(alGore);
            bizRelationVi = AlBetonamuRelationViResourceIT.createEntity(em);
        } else {
            bizRelationVi = TestUtil.findAll(em, AlBetonamuRelationVi.class).get(0);
        }
        em.persist(bizRelationVi);
        em.flush();
        alGore.setBizRelationVi(bizRelationVi);
        alGoreRepository.saveAndFlush(alGore);
        Long bizRelationViId = bizRelationVi.getId();
        // Get all the alGoreList where bizRelationVi equals to bizRelationViId
        defaultAlGoreShouldBeFound("bizRelationViId.equals=" + bizRelationViId);

        // Get all the alGoreList where bizRelationVi equals to (bizRelationViId + 1)
        defaultAlGoreShouldNotBeFound("bizRelationViId.equals=" + (bizRelationViId + 1));
    }
    ...
    void putExistingAlGore() throws Exception {
        ...
        AlGoreDTO alGoreDTO = alGoreMapper.toDto(updatedAlGore);

        restAlGoreMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alGoreDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreDTO))
            )
            .andExpect(status().isOk());
        ...
        restAlGoreMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    ...
                    .content(om.writeValueAsBytes(alGoreDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlGore() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreDTO)))
    ...
    void patchNonExistingAlGore() throws Exception {
        ...
        restAlGoreMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alGoreDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alGoreDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlGore() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alGoreDTO)))
            .andExpect(status().isMethodNotAllowed());
