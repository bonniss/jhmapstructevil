...
class AppZnsTemplateResourceIT {
    ...
    void createAppZnsTemplate() throws Exception {
        ...
        AppZnsTemplateDTO appZnsTemplateDTO = appZnsTemplateMapper.toDto(appZnsTemplate);
        var returnedAppZnsTemplateDTO = om.readValue(
            restAppZnsTemplateMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(appZnsTemplateDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AppZnsTemplateDTO.class
        );

        // Validate the AppZnsTemplate in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAppZnsTemplate = appZnsTemplateMapper.toEntity(returnedAppZnsTemplateDTO);
    ...
    void putExistingAppZnsTemplate() throws Exception {
        ...
        updatedAppZnsTemplate
            ...
            .templateDataMapping(UPDATED_TEMPLATE_DATA_MAPPING)
            .targetUrls(UPDATED_TARGET_URLS);
        AppZnsTemplateDTO appZnsTemplateDTO = appZnsTemplateMapper.toDto(updatedAppZnsTemplate);

        restAppZnsTemplateMockMvc
            .perform(
                put(ENTITY_API_URL_ID, appZnsTemplateDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(appZnsTemplateDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAppZnsTemplate() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAppZnsTemplateMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(appZnsTemplateDTO)))
    ...
    void patchNonExistingAppZnsTemplate() throws Exception {
        ...
        restAppZnsTemplateMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, appZnsTemplateDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(appZnsTemplateDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAppZnsTemplate() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAppZnsTemplateMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(appZnsTemplateDTO)))
            .andExpect(status().isMethodNotAllowed());
