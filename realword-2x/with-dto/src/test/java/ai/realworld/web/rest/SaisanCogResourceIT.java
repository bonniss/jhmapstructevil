...
class SaisanCogResourceIT {
    ...
    void createSaisanCog() throws Exception {
        ...
        SaisanCogDTO saisanCogDTO = saisanCogMapper.toDto(saisanCog);
        var returnedSaisanCogDTO = om.readValue(
            restSaisanCogMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(saisanCogDTO)))
        ...
        var returnedSaisanCog = saisanCogMapper.toEntity(returnedSaisanCogDTO);
    ...
    void putExistingSaisanCog() throws Exception {
        ...
        SaisanCogDTO saisanCogDTO = saisanCogMapper.toDto(updatedSaisanCog);

        restSaisanCogMockMvc
            .perform(
                put(ENTITY_API_URL_ID, saisanCogDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(saisanCogDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamSaisanCog() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSaisanCogMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(saisanCogDTO)))
    ...
    void patchNonExistingSaisanCog() throws Exception {
        ...
        restSaisanCogMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, saisanCogDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(saisanCogDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSaisanCog() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSaisanCogMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(saisanCogDTO)))
            .andExpect(status().isMethodNotAllowed());
