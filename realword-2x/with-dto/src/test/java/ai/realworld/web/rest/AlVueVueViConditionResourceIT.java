...
class AlVueVueViConditionResourceIT {
    ...
    void createAlVueVueViCondition() throws Exception {
        ...
        AlVueVueViConditionDTO alVueVueViConditionDTO = alVueVueViConditionMapper.toDto(alVueVueViCondition);
        var returnedAlVueVueViConditionDTO = om.readValue(
            restAlVueVueViConditionMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alVueVueViConditionDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlVueVueViConditionDTO.class
        );

        // Validate the AlVueVueViCondition in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlVueVueViCondition = alVueVueViConditionMapper.toEntity(returnedAlVueVueViConditionDTO);
    ...
    void putExistingAlVueVueViCondition() throws Exception {
        ...
        AlVueVueViConditionDTO alVueVueViConditionDTO = alVueVueViConditionMapper.toDto(updatedAlVueVueViCondition);

        restAlVueVueViConditionMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alVueVueViConditionDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alVueVueViConditionDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlVueVueViCondition() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlVueVueViConditionMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alVueVueViConditionDTO)))
    ...
    void patchNonExistingAlVueVueViCondition() throws Exception {
        ...
        restAlVueVueViConditionMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alVueVueViConditionDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alVueVueViConditionDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlVueVueViCondition() throws Exception {
        ...
        restAlVueVueViConditionMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alVueVueViConditionDTO))
            )
            .andExpect(status().isMethodNotAllowed());
