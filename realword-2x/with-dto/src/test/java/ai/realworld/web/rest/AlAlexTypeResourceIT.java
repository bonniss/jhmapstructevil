...
class AlAlexTypeResourceIT {
    ...
    void createAlAlexType() throws Exception {
        ...
        AlAlexTypeDTO alAlexTypeDTO = alAlexTypeMapper.toDto(alAlexType);
        var returnedAlAlexTypeDTO = om.readValue(
            restAlAlexTypeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alAlexTypeDTO)))
        ...
        var returnedAlAlexType = alAlexTypeMapper.toEntity(returnedAlAlexTypeDTO);
    ...
    void putExistingAlAlexType() throws Exception {
        ...
        updatedAlAlexType
            ...
            .configJason(UPDATED_CONFIG_JASON)
            .treeDepth(UPDATED_TREE_DEPTH);
        AlAlexTypeDTO alAlexTypeDTO = alAlexTypeMapper.toDto(updatedAlAlexType);

        restAlAlexTypeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alAlexTypeDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alAlexTypeDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlAlexType() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlAlexTypeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alAlexTypeDTO)))
    ...
    void patchNonExistingAlAlexType() throws Exception {
        ...
        restAlAlexTypeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alAlexTypeDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alAlexTypeDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlAlexType() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlAlexTypeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alAlexTypeDTO)))
            .andExpect(status().isMethodNotAllowed());
