...
class AlLexFergViResourceIT {
    ...
    void createAlLexFergVi() throws Exception {
        ...
        AlLexFergViDTO alLexFergViDTO = alLexFergViMapper.toDto(alLexFergVi);
        var returnedAlLexFergViDTO = om.readValue(
            restAlLexFergViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLexFergViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlLexFergViDTO.class
        );

        // Validate the AlLexFergVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlLexFergVi = alLexFergViMapper.toEntity(returnedAlLexFergViDTO);
    ...
    void putExistingAlLexFergVi() throws Exception {
        ...
        updatedAlLexFergVi
            ...
            .publicationStatus(UPDATED_PUBLICATION_STATUS)
            .publishedDate(UPDATED_PUBLISHED_DATE);
        AlLexFergViDTO alLexFergViDTO = alLexFergViMapper.toDto(updatedAlLexFergVi);

        restAlLexFergViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alLexFergViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alLexFergViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlLexFergVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlLexFergViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLexFergViDTO)))
    ...
    void patchNonExistingAlLexFergVi() throws Exception {
        ...
        restAlLexFergViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alLexFergViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alLexFergViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlLexFergVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlLexFergViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alLexFergViDTO)))
            .andExpect(status().isMethodNotAllowed());
