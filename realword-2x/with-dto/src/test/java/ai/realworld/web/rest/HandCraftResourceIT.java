...
class HandCraftResourceIT {
    ...
    void createHandCraft() throws Exception {
        ...
        HandCraftDTO handCraftDTO = handCraftMapper.toDto(handCraft);
        var returnedHandCraftDTO = om.readValue(
            restHandCraftMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(handCraftDTO)))
        ...
        var returnedHandCraft = handCraftMapper.toEntity(returnedHandCraftDTO);
    ...
    void putExistingHandCraft() throws Exception {
        ...
        HandCraftDTO handCraftDTO = handCraftMapper.toDto(updatedHandCraft);

        restHandCraftMockMvc
            .perform(
                put(ENTITY_API_URL_ID, handCraftDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(handCraftDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamHandCraft() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHandCraftMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(handCraftDTO)))
    ...
    void patchNonExistingHandCraft() throws Exception {
        ...
        restHandCraftMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, handCraftDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(handCraftDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamHandCraft() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHandCraftMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(handCraftDTO)))
            .andExpect(status().isMethodNotAllowed());
