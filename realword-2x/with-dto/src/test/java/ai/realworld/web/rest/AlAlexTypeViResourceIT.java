...
class AlAlexTypeViResourceIT {
    ...
    void createAlAlexTypeVi() throws Exception {
        ...
        AlAlexTypeViDTO alAlexTypeViDTO = alAlexTypeViMapper.toDto(alAlexTypeVi);
        var returnedAlAlexTypeViDTO = om.readValue(
            restAlAlexTypeViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alAlexTypeViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlAlexTypeViDTO.class
        );

        // Validate the AlAlexTypeVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlAlexTypeVi = alAlexTypeViMapper.toEntity(returnedAlAlexTypeViDTO);
    ...
    void putExistingAlAlexTypeVi() throws Exception {
        ...
        updatedAlAlexTypeVi
            ...
            .configJason(UPDATED_CONFIG_JASON)
            .treeDepth(UPDATED_TREE_DEPTH);
        AlAlexTypeViDTO alAlexTypeViDTO = alAlexTypeViMapper.toDto(updatedAlAlexTypeVi);

        restAlAlexTypeViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alAlexTypeViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alAlexTypeViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlAlexTypeVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlAlexTypeViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alAlexTypeViDTO)))
    ...
    void patchNonExistingAlAlexTypeVi() throws Exception {
        ...
        restAlAlexTypeViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alAlexTypeViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alAlexTypeViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlAlexTypeVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlAlexTypeViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alAlexTypeViDTO)))
            .andExpect(status().isMethodNotAllowed());
