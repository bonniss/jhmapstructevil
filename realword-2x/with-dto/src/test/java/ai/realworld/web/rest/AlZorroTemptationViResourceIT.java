...
class AlZorroTemptationViResourceIT {
    ...
    void createAlZorroTemptationVi() throws Exception {
        ...
        AlZorroTemptationViDTO alZorroTemptationViDTO = alZorroTemptationViMapper.toDto(alZorroTemptationVi);
        var returnedAlZorroTemptationViDTO = om.readValue(
            restAlZorroTemptationViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alZorroTemptationViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlZorroTemptationViDTO.class
        );

        // Validate the AlZorroTemptationVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlZorroTemptationVi = alZorroTemptationViMapper.toEntity(returnedAlZorroTemptationViDTO);
    ...
    void putExistingAlZorroTemptationVi() throws Exception {
        ...
        updatedAlZorroTemptationVi
            ...
            .templateDataMapping(UPDATED_TEMPLATE_DATA_MAPPING)
            .targetUrls(UPDATED_TARGET_URLS);
        AlZorroTemptationViDTO alZorroTemptationViDTO = alZorroTemptationViMapper.toDto(updatedAlZorroTemptationVi);

        restAlZorroTemptationViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alZorroTemptationViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alZorroTemptationViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlZorroTemptationVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlZorroTemptationViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alZorroTemptationViDTO)))
    ...
    void patchNonExistingAlZorroTemptationVi() throws Exception {
        ...
        restAlZorroTemptationViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alZorroTemptationViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alZorroTemptationViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlZorroTemptationVi() throws Exception {
        ...
        restAlZorroTemptationViMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alZorroTemptationViDTO))
            )
            .andExpect(status().isMethodNotAllowed());
