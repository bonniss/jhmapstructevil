...
class AlActisoResourceIT {
    ...
    void createAlActiso() throws Exception {
        ...
        // Create the AlActiso
        AlActisoDTO alActisoDTO = alActisoMapper.toDto(alActiso);
        var returnedAlActisoDTO = om.readValue(
            restAlActisoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alActisoDTO)))
        ...
        var returnedAlActiso = alActisoMapper.toEntity(returnedAlActisoDTO);
    ...
    void putExistingAlActiso() throws Exception {
        ...
        AlActisoDTO alActisoDTO = alActisoMapper.toDto(updatedAlActiso);

        restAlActisoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alActisoDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alActisoDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlActiso() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlActisoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alActisoDTO)))
    ...
    void patchNonExistingAlActiso() throws Exception {
        ...
        restAlActisoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alActisoDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alActisoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlActiso() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlActisoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alActisoDTO)))
            .andExpect(status().isMethodNotAllowed());
