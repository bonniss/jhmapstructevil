...
class AlPowerShellViResourceIT {
    ...
    void createAlPowerShellVi() throws Exception {
        ...
        AlPowerShellViDTO alPowerShellViDTO = alPowerShellViMapper.toDto(alPowerShellVi);
        var returnedAlPowerShellViDTO = om.readValue(
            restAlPowerShellViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPowerShellViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlPowerShellViDTO.class
        );

        // Validate the AlPowerShellVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlPowerShellVi = alPowerShellViMapper.toEntity(returnedAlPowerShellViDTO);
    ...
    void putExistingAlPowerShellVi() throws Exception {
        ...
        AlPowerShellViDTO alPowerShellViDTO = alPowerShellViMapper.toDto(updatedAlPowerShellVi);

        restAlPowerShellViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPowerShellViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alPowerShellViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlPowerShellVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPowerShellViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPowerShellViDTO)))
    ...
    void patchNonExistingAlPowerShellVi() throws Exception {
        ...
        restAlPowerShellViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPowerShellViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPowerShellViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPowerShellVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPowerShellViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPowerShellViDTO)))
            .andExpect(status().isMethodNotAllowed());
