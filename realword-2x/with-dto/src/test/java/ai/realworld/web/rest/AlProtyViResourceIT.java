...
class AlProtyViResourceIT {
    ...
    void createAlProtyVi() throws Exception {
        ...
        AlProtyViDTO alProtyViDTO = alProtyViMapper.toDto(alProtyVi);
        var returnedAlProtyViDTO = om.readValue(
            restAlProtyViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProtyViDTO)))
        ...
        var returnedAlProtyVi = alProtyViMapper.toEntity(returnedAlProtyViDTO);
    ...
    void putExistingAlProtyVi() throws Exception {
        ...
        updatedAlProtyVi
            ...
            .status(UPDATED_STATUS)
            .isEnabled(UPDATED_IS_ENABLED);
        AlProtyViDTO alProtyViDTO = alProtyViMapper.toDto(updatedAlProtyVi);

        restAlProtyViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alProtyViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alProtyViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlProtyVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProtyViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProtyViDTO)))
    ...
    void patchNonExistingAlProtyVi() throws Exception {
        ...
        restAlProtyViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alProtyViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alProtyViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlProtyVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProtyViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alProtyViDTO)))
            .andExpect(status().isMethodNotAllowed());
