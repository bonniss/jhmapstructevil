...
class OlMasterResourceIT {
    ...
    void createOlMaster() throws Exception {
        ...
        // Create the OlMaster
        OlMasterDTO olMasterDTO = olMasterMapper.toDto(olMaster);
        var returnedOlMasterDTO = om.readValue(
            restOlMasterMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(olMasterDTO)))
        ...
        var returnedOlMaster = olMasterMapper.toEntity(returnedOlMasterDTO);
    ...
    @Test
    @Transactional
    void getAllOlMastersByAddressViIsEqualToSomething() throws Exception {
        AndreiRightHandVi addressVi;
        if (TestUtil.findAll(em, AndreiRightHandVi.class).isEmpty()) {
            olMasterRepository.saveAndFlush(olMaster);
            addressVi = AndreiRightHandViResourceIT.createEntity();
        } else {
            addressVi = TestUtil.findAll(em, AndreiRightHandVi.class).get(0);
        }
        em.persist(addressVi);
        em.flush();
        olMaster.setAddressVi(addressVi);
        olMasterRepository.saveAndFlush(olMaster);
        Long addressViId = addressVi.getId();
        // Get all the olMasterList where addressVi equals to addressViId
        defaultOlMasterShouldBeFound("addressViId.equals=" + addressViId);

        // Get all the olMasterList where addressVi equals to (addressViId + 1)
        defaultOlMasterShouldNotBeFound("addressViId.equals=" + (addressViId + 1));
    }
    ...
    void putExistingOlMaster() throws Exception {
        ...
        updatedOlMaster
            ...
            .extensionJason(UPDATED_EXTENSION_JASON)
            .isEnabled(UPDATED_IS_ENABLED);
        OlMasterDTO olMasterDTO = olMasterMapper.toDto(updatedOlMaster);

        restOlMasterMockMvc
            .perform(
                put(ENTITY_API_URL_ID, olMasterDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(olMasterDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithIdMismatchOlMaster() throws Exception {
        ...
        restOlMasterMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(olMasterDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamOlMaster() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOlMasterMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(olMasterDTO)))
    ...
    void patchNonExistingOlMaster() throws Exception {
        ...
        restOlMasterMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, olMasterDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(olMasterDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamOlMaster() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOlMasterMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(olMasterDTO)))
            .andExpect(status().isMethodNotAllowed());
