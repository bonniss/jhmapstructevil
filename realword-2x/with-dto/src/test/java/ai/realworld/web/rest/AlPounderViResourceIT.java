...
class AlPounderViResourceIT {
    ...
    void createAlPounderVi() throws Exception {
        ...
        AlPounderViDTO alPounderViDTO = alPounderViMapper.toDto(alPounderVi);
        var returnedAlPounderViDTO = om.readValue(
            restAlPounderViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPounderViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlPounderViDTO.class
        );

        // Validate the AlPounderVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlPounderVi = alPounderViMapper.toEntity(returnedAlPounderViDTO);
    ...
    void putExistingAlPounderVi() throws Exception {
        ...
        AlPounderViDTO alPounderViDTO = alPounderViMapper.toDto(updatedAlPounderVi);

        restAlPounderViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPounderViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alPounderViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlPounderVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPounderViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPounderViDTO)))
    ...
    void patchNonExistingAlPounderVi() throws Exception {
        ...
        restAlPounderViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPounderViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPounderViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPounderVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPounderViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPounderViDTO)))
            .andExpect(status().isMethodNotAllowed());
