...
class AlProtyResourceIT {
    ...
    @Mock
    private AlProtyRepository alProtyRepositoryMock;

    @Autowired
    private AlProtyMapper alProtyMapper;

    @Mock
    private AlProtyService alProtyServiceMock;
    ...
    void createAlProty() throws Exception {
        ...
        // Create the AlProty
        AlProtyDTO alProtyDTO = alProtyMapper.toDto(alProty);
        var returnedAlProtyDTO = om.readValue(
            restAlProtyMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProtyDTO)))
        ...
        var returnedAlProty = alProtyMapper.toEntity(returnedAlProtyDTO);
    ...
    void getAllAlProtiesWithEagerRelationshipsIsEnabled() throws Exception {
        when(alProtyServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAlProtyMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(alProtyServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllAlProtiesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(alProtyServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAlProtyMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(alProtyRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void getAllAlProtiesByImageIsEqualToSomething() throws Exception {
        Metaverse image;
        if (TestUtil.findAll(em, Metaverse.class).isEmpty()) {
            alProtyRepository.saveAndFlush(alProty);
        ...
        em.persist(image);
        em.flush();
        alProty.addImage(image);
        alProtyRepository.saveAndFlush(alProty);
        Long imageId = image.getId();
        // Get all the alProtyList where image equals to imageId
        defaultAlProtyShouldBeFound("imageId.equals=" + imageId);

        // Get all the alProtyList where image equals to (imageId + 1)
        defaultAlProtyShouldNotBeFound("imageId.equals=" + (imageId + 1));
    }

    @Test
    @Transactional
    void getAllAlProtiesByBookingIsEqualToSomething() throws Exception {
        AlPyuJoker booking;
        ...
        em.persist(booking);
        em.flush();
        alProty.addBooking(booking);
        alProtyRepository.saveAndFlush(alProty);
        UUID bookingId = booking.getId();
        // Get all the alProtyList where booking equals to bookingId
        defaultAlProtyShouldBeFound("bookingId.equals=" + bookingId);

        // Get all the alProtyList where booking equals to UUID.randomUUID()
        defaultAlProtyShouldNotBeFound("bookingId.equals=" + UUID.randomUUID());
    }
    ...
    void putExistingAlProty() throws Exception {
        ...
        updatedAlProty
            ...
            .status(UPDATED_STATUS)
            .isEnabled(UPDATED_IS_ENABLED);
        AlProtyDTO alProtyDTO = alProtyMapper.toDto(updatedAlProty);

        restAlProtyMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alProtyDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProtyDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithIdMismatchAlProty() throws Exception {
        ...
        restAlProtyMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProtyDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlProty() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProtyMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProtyDTO)))
    ...
    void patchNonExistingAlProty() throws Exception {
        ...
        restAlProtyMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alProtyDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alProtyDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlProty() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProtyMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alProtyDTO)))
            .andExpect(status().isMethodNotAllowed());
