...
class AlGoreConditionViResourceIT {
    ...
    void createAlGoreConditionVi() throws Exception {
        ...
        AlGoreConditionViDTO alGoreConditionViDTO = alGoreConditionViMapper.toDto(alGoreConditionVi);
        var returnedAlGoreConditionViDTO = om.readValue(
            restAlGoreConditionViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreConditionViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlGoreConditionViDTO.class
        );

        // Validate the AlGoreConditionVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlGoreConditionVi = alGoreConditionViMapper.toEntity(returnedAlGoreConditionViDTO);
    ...
    void putExistingAlGoreConditionVi() throws Exception {
        ...
        AlGoreConditionViDTO alGoreConditionViDTO = alGoreConditionViMapper.toDto(updatedAlGoreConditionVi);

        restAlGoreConditionViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alGoreConditionViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alGoreConditionViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlGoreConditionVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreConditionViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreConditionViDTO)))
    ...
    void patchNonExistingAlGoreConditionVi() throws Exception {
        ...
        restAlGoreConditionViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alGoreConditionViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alGoreConditionViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlGoreConditionVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreConditionViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alGoreConditionViDTO)))
            .andExpect(status().isMethodNotAllowed());
