...
class AlProtyViTest {
    ...
    void imageTest() {
        ...
        alProtyVi.addImage(metaverseBack);
        assertThat(alProtyVi.getImages()).containsOnly(metaverseBack);

        alProtyVi.removeImage(metaverseBack);
        assertThat(alProtyVi.getImages()).doesNotContain(metaverseBack);

        alProtyVi.images(new HashSet<>(Set.of(metaverseBack)));
        assertThat(alProtyVi.getImages()).containsOnly(metaverseBack);

        alProtyVi.setImages(new HashSet<>());
        assertThat(alProtyVi.getImages()).doesNotContain(metaverseBack);
    }
    ...
    void bookingTest() {
        ...
        alProtyVi.addBooking(alPyuJokerViBack);
        assertThat(alProtyVi.getBookings()).containsOnly(alPyuJokerViBack);
        assertThat(alPyuJokerViBack.getProperties()).containsOnly(alProtyVi);

        alProtyVi.removeBooking(alPyuJokerViBack);
        assertThat(alProtyVi.getBookings()).doesNotContain(alPyuJokerViBack);
        assertThat(alPyuJokerViBack.getProperties()).doesNotContain(alProtyVi);

        alProtyVi.bookings(new HashSet<>(Set.of(alPyuJokerViBack)));
        assertThat(alProtyVi.getBookings()).containsOnly(alPyuJokerViBack);
        assertThat(alPyuJokerViBack.getProperties()).containsOnly(alProtyVi);

        alProtyVi.setBookings(new HashSet<>());
        assertThat(alProtyVi.getBookings()).doesNotContain(alPyuJokerViBack);
        assertThat(alPyuJokerViBack.getProperties()).doesNotContain(alProtyVi);
    }
}
