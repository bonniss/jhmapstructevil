...
class AlPacinoResourceIT {
    ...
    void createAlPacino() throws Exception {
        ...
        // Create the AlPacino
        var returnedAlPacino = om.readValue(
            restAlPacinoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPacino)))
    ...
    @Test
    @Transactional
    void getAllAlPacinosByMembershipTierViIsEqualToSomething() throws Exception {
        AlMemTierVi membershipTierVi;
        if (TestUtil.findAll(em, AlMemTierVi.class).isEmpty()) {
            alPacinoRepository.saveAndFlush(alPacino);
            membershipTierVi = AlMemTierViResourceIT.createEntity();
        } else {
            membershipTierVi = TestUtil.findAll(em, AlMemTierVi.class).get(0);
        }
        em.persist(membershipTierVi);
        em.flush();
        alPacino.setMembershipTierVi(membershipTierVi);
        alPacinoRepository.saveAndFlush(alPacino);
        Long membershipTierViId = membershipTierVi.getId();
        // Get all the alPacinoList where membershipTierVi equals to membershipTierViId
        defaultAlPacinoShouldBeFound("membershipTierViId.equals=" + membershipTierViId);

        // Get all the alPacinoList where membershipTierVi equals to (membershipTierViId + 1)
        defaultAlPacinoShouldNotBeFound("membershipTierViId.equals=" + (membershipTierViId + 1));
    }

    @Test
    @Transactional
    void getAllAlPacinosByAlVueVueViUsageIsEqualToSomething() throws Exception {
        ...
        em.flush();
        alPacino.setAlVueVueViUsage(alVueVueViUsage);
        alPacinoRepository.saveAndFlush(alPacino);
        UUID alVueVueViUsageId = alVueVueViUsage.getId();
        // Get all the alPacinoList where alVueVueViUsage equals to alVueVueViUsageId
        defaultAlPacinoShouldBeFound("alVueVueViUsageId.equals=" + alVueVueViUsageId);

        // Get all the alPacinoList where alVueVueViUsage equals to UUID.randomUUID()
        defaultAlPacinoShouldNotBeFound("alVueVueViUsageId.equals=" + UUID.randomUUID());
    }
    ...
    void putExistingAlPacino() throws Exception {
        ...
        restAlPacinoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlPacino.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlPacino))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlPacino() throws Exception {
        ...
        restAlPacinoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPacino.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPacino))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithIdMismatchAlPacino() throws Exception {
        ...
        restAlPacinoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPacino))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlPacino() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPacinoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPacino)))
    ...
    void patchNonExistingAlPacino() throws Exception {
        ...
        restAlPacinoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPacino.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPacino))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPacino() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPacinoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPacino)))
            .andExpect(status().isMethodNotAllowed());
