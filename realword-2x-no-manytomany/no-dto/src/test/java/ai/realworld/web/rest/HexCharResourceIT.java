...
class HexCharResourceIT {
    ...
    void createHexChar() throws Exception {
        ...
        // Create the HexChar
        var returnedHexChar = om.readValue(
            restHexCharMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hexChar)))
    ...
    void putExistingHexChar() throws Exception {
        ...
        restHexCharMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedHexChar.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedHexChar))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingHexChar() throws Exception {
        ...
        restHexCharMockMvc
            .perform(put(ENTITY_API_URL_ID, hexChar.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hexChar)))
            .andExpect(status().isBadRequest());
        ...
        restHexCharMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(hexChar))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamHexChar() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHexCharMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hexChar)))
    ...
    void patchNonExistingHexChar() throws Exception {
        ...
        restHexCharMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, hexChar.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(hexChar))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamHexChar() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHexCharMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(hexChar)))
            .andExpect(status().isMethodNotAllowed());
