...
class AlZorroTemptationResourceIT {
    ...
    void createAlZorroTemptation() throws Exception {
        ...
        var returnedAlZorroTemptation = om.readValue(
            restAlZorroTemptationMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alZorroTemptation)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlZorroTemptation.class
        );
    ...
    void putExistingAlZorroTemptation() throws Exception {
        ...
        restAlZorroTemptationMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlZorroTemptation.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlZorroTemptation))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlZorroTemptation() throws Exception {
        ...
        restAlZorroTemptationMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alZorroTemptation.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alZorroTemptation))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlZorroTemptation() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlZorroTemptationMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alZorroTemptation)))
    ...
    void patchNonExistingAlZorroTemptation() throws Exception {
        ...
        restAlZorroTemptationMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alZorroTemptation.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alZorroTemptation))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlZorroTemptation() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlZorroTemptationMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alZorroTemptation)))
            .andExpect(status().isMethodNotAllowed());
