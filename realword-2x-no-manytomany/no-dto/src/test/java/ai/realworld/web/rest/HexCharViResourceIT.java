...
class HexCharViResourceIT {
    ...
    void createHexCharVi() throws Exception {
        ...
        var returnedHexCharVi = om.readValue(
            restHexCharViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hexCharVi)))
    ...
    void putExistingHexCharVi() throws Exception {
        ...
        restHexCharViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedHexCharVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedHexCharVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingHexCharVi() throws Exception {
        ...
        restHexCharViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, hexCharVi.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hexCharVi))
            )
            .andExpect(status().isBadRequest());
        ...
        restHexCharViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(hexCharVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamHexCharVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHexCharViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hexCharVi)))
    ...
    void patchNonExistingHexCharVi() throws Exception {
        ...
        restHexCharViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, hexCharVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(hexCharVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamHexCharVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHexCharViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(hexCharVi)))
            .andExpect(status().isMethodNotAllowed());
