...
class AlCatalinaViResourceIT {
    ...
    void createAlCatalinaVi() throws Exception {
        ...
        var returnedAlCatalinaVi = om.readValue(
            restAlCatalinaViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alCatalinaVi)))
    ...
    void putExistingAlCatalinaVi() throws Exception {
        ...
        restAlCatalinaViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlCatalinaVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlCatalinaVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlCatalinaVi() throws Exception {
        ...
        restAlCatalinaViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alCatalinaVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alCatalinaVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlCatalinaVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlCatalinaViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alCatalinaVi)))
    ...
    void patchNonExistingAlCatalinaVi() throws Exception {
        ...
        restAlCatalinaViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alCatalinaVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alCatalinaVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlCatalinaVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlCatalinaViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alCatalinaVi)))
            .andExpect(status().isMethodNotAllowed());
