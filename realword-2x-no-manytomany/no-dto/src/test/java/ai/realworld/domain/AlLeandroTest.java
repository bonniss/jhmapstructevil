...
class AlLeandroTest {
    ...
    void awardVisTest() {
        AlLeandro alLeandro = getAlLeandroRandomSampleGenerator();
        AlDesireVi alDesireViBack = getAlDesireViRandomSampleGenerator();

        alLeandro.addAwardVis(alDesireViBack);
        assertThat(alLeandro.getAwardVis()).containsOnly(alDesireViBack);
        assertThat(alDesireViBack.getMaggi()).isEqualTo(alLeandro);

        alLeandro.removeAwardVis(alDesireViBack);
        assertThat(alLeandro.getAwardVis()).doesNotContain(alDesireViBack);
        assertThat(alDesireViBack.getMaggi()).isNull();

        alLeandro.awardVis(new HashSet<>(Set.of(alDesireViBack)));
        assertThat(alLeandro.getAwardVis()).containsOnly(alDesireViBack);
        assertThat(alDesireViBack.getMaggi()).isEqualTo(alLeandro);

        alLeandro.setAwardVis(new HashSet<>());
        assertThat(alLeandro.getAwardVis()).doesNotContain(alDesireViBack);
        assertThat(alDesireViBack.getMaggi()).isNull();
    }
}
