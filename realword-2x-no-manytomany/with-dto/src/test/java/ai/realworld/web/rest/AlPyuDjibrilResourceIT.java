...
class AlPyuDjibrilResourceIT {
    ...
    void createAlPyuDjibril() throws Exception {
        ...
        AlPyuDjibrilDTO alPyuDjibrilDTO = alPyuDjibrilMapper.toDto(alPyuDjibril);
        var returnedAlPyuDjibrilDTO = om.readValue(
            restAlPyuDjibrilMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPyuDjibrilDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlPyuDjibrilDTO.class
        );

        // Validate the AlPyuDjibril in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlPyuDjibril = alPyuDjibrilMapper.toEntity(returnedAlPyuDjibrilDTO);
    ...
    void putExistingAlPyuDjibril() throws Exception {
        ...
        AlPyuDjibrilDTO alPyuDjibrilDTO = alPyuDjibrilMapper.toDto(updatedAlPyuDjibril);

        restAlPyuDjibrilMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPyuDjibrilDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alPyuDjibrilDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlPyuDjibril() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPyuDjibrilMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPyuDjibrilDTO)))
    ...
    void patchNonExistingAlPyuDjibril() throws Exception {
        ...
        restAlPyuDjibrilMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPyuDjibrilDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPyuDjibrilDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPyuDjibril() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPyuDjibrilMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPyuDjibrilDTO)))
            .andExpect(status().isMethodNotAllowed());
