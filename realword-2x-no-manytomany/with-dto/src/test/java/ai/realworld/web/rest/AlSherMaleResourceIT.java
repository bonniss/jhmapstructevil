...
class AlSherMaleResourceIT {
    ...
    void createAlSherMale() throws Exception {
        ...
        AlSherMaleDTO alSherMaleDTO = alSherMaleMapper.toDto(alSherMale);
        var returnedAlSherMaleDTO = om.readValue(
            restAlSherMaleMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alSherMaleDTO)))
        ...
        var returnedAlSherMale = alSherMaleMapper.toEntity(returnedAlSherMaleDTO);
    ...
    void putExistingAlSherMale() throws Exception {
        ...
        updatedAlSherMale
            ...
            .property(UPDATED_PROPERTY)
            .title(UPDATED_TITLE);
        AlSherMaleDTO alSherMaleDTO = alSherMaleMapper.toDto(updatedAlSherMale);

        restAlSherMaleMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alSherMaleDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alSherMaleDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlSherMale() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlSherMaleMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alSherMaleDTO)))
    ...
    void patchNonExistingAlSherMale() throws Exception {
        ...
        restAlSherMaleMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alSherMaleDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alSherMaleDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlSherMale() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlSherMaleMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alSherMaleDTO)))
            .andExpect(status().isMethodNotAllowed());
