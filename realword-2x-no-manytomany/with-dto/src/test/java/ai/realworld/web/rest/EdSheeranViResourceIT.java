...
class EdSheeranViResourceIT {
    ...
    void createEdSheeranVi() throws Exception {
        ...
        EdSheeranViDTO edSheeranViDTO = edSheeranViMapper.toDto(edSheeranVi);
        var returnedEdSheeranViDTO = om.readValue(
            restEdSheeranViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(edSheeranViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            EdSheeranViDTO.class
        );

        // Validate the EdSheeranVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedEdSheeranVi = edSheeranViMapper.toEntity(returnedEdSheeranViDTO);
    ...
    void putExistingEdSheeranVi() throws Exception {
        ...
        updatedEdSheeranVi
            ...
            .contactsJason(UPDATED_CONTACTS_JASON)
            .isEnabled(UPDATED_IS_ENABLED);
        EdSheeranViDTO edSheeranViDTO = edSheeranViMapper.toDto(updatedEdSheeranVi);

        restEdSheeranViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, edSheeranViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(edSheeranViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamEdSheeranVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEdSheeranViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(edSheeranViDTO)))
    ...
    void patchNonExistingEdSheeranVi() throws Exception {
        ...
        restEdSheeranViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, edSheeranViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(edSheeranViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEdSheeranVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEdSheeranViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(edSheeranViDTO)))
            .andExpect(status().isMethodNotAllowed());
