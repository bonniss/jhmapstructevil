...
class AlProProViResourceIT {
    ...
    void createAlProProVi() throws Exception {
        ...
        AlProProViDTO alProProViDTO = alProProViMapper.toDto(alProProVi);
        var returnedAlProProViDTO = om.readValue(
            restAlProProViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProProViDTO)))
        ...
        var returnedAlProProVi = alProProViMapper.toEntity(returnedAlProProViDTO);
    ...
    void putExistingAlProProVi() throws Exception {
        ...
        updatedAlProProVi
            ...
            .bedSize(UPDATED_BED_SIZE)
            .isEnabled(UPDATED_IS_ENABLED);
        AlProProViDTO alProProViDTO = alProProViMapper.toDto(updatedAlProProVi);

        restAlProProViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alProProViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alProProViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlProProVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProProViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProProViDTO)))
    ...
    void patchNonExistingAlProProVi() throws Exception {
        ...
        restAlProProViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alProProViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alProProViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlProProVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProProViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alProProViDTO)))
            .andExpect(status().isMethodNotAllowed());
