...
class AlMemTierResourceIT {
    ...
    void createAlMemTier() throws Exception {
        ...
        AlMemTierDTO alMemTierDTO = alMemTierMapper.toDto(alMemTier);
        var returnedAlMemTierDTO = om.readValue(
            restAlMemTierMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alMemTierDTO)))
        ...
        var returnedAlMemTier = alMemTierMapper.toEntity(returnedAlMemTierDTO);
    ...
    void putExistingAlMemTier() throws Exception {
        ...
        AlMemTierDTO alMemTierDTO = alMemTierMapper.toDto(updatedAlMemTier);

        restAlMemTierMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alMemTierDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alMemTierDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlMemTier() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlMemTierMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alMemTierDTO)))
    ...
    void patchNonExistingAlMemTier() throws Exception {
        ...
        restAlMemTierMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alMemTierDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alMemTierDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlMemTier() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlMemTierMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alMemTierDTO)))
            .andExpect(status().isMethodNotAllowed());
