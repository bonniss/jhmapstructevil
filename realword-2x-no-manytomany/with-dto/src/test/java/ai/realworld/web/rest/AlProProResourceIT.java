...
class AlProProResourceIT {
    ...
    void createAlProPro() throws Exception {
        ...
        // Create the AlProPro
        AlProProDTO alProProDTO = alProProMapper.toDto(alProPro);
        var returnedAlProProDTO = om.readValue(
            restAlProProMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProProDTO)))
        ...
        var returnedAlProPro = alProProMapper.toEntity(returnedAlProProDTO);
    ...
    void putExistingAlProPro() throws Exception {
        ...
        updatedAlProPro
            ...
            .bedSize(UPDATED_BED_SIZE)
            .isEnabled(UPDATED_IS_ENABLED);
        AlProProDTO alProProDTO = alProProMapper.toDto(updatedAlProPro);

        restAlProProMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alProProDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alProProDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithIdMismatchAlProPro() throws Exception {
        ...
        restAlProProMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProProDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlProPro() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProProMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alProProDTO)))
    ...
    void patchNonExistingAlProPro() throws Exception {
        ...
        restAlProProMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alProProDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alProProDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlProPro() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlProProMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alProProDTO)))
            .andExpect(status().isMethodNotAllowed());
