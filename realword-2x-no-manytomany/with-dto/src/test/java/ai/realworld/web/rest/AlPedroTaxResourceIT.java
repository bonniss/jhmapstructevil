...
class AlPedroTaxResourceIT {
    ...
    void createAlPedroTax() throws Exception {
        ...
        AlPedroTaxDTO alPedroTaxDTO = alPedroTaxMapper.toDto(alPedroTax);
        var returnedAlPedroTaxDTO = om.readValue(
            restAlPedroTaxMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPedroTaxDTO)))
        ...
        var returnedAlPedroTax = alPedroTaxMapper.toEntity(returnedAlPedroTaxDTO);
    ...
    void putExistingAlPedroTax() throws Exception {
        ...
        AlPedroTaxDTO alPedroTaxDTO = alPedroTaxMapper.toDto(updatedAlPedroTax);

        restAlPedroTaxMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPedroTaxDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alPedroTaxDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlPedroTax() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPedroTaxMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPedroTaxDTO)))
    ...
    void patchNonExistingAlPedroTax() throws Exception {
        ...
        restAlPedroTaxMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPedroTaxDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPedroTaxDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPedroTax() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPedroTaxMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPedroTaxDTO)))
            .andExpect(status().isMethodNotAllowed());
