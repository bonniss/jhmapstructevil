...
class AlAppleViResourceIT {
    ...
    void createAlAppleVi() throws Exception {
        ...
        AlAppleViDTO alAppleViDTO = alAppleViMapper.toDto(alAppleVi);
        var returnedAlAppleViDTO = om.readValue(
            restAlAppleViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alAppleViDTO)))
        ...
        var returnedAlAppleVi = alAppleViMapper.toEntity(returnedAlAppleViDTO);
    ...
    void putExistingAlAppleVi() throws Exception {
        ...
        updatedAlAppleVi
            ...
            .extensionJason(UPDATED_EXTENSION_JASON)
            .isEnabled(UPDATED_IS_ENABLED);
        AlAppleViDTO alAppleViDTO = alAppleViMapper.toDto(updatedAlAppleVi);

        restAlAppleViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alAppleViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alAppleViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlAppleVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlAppleViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alAppleViDTO)))
    ...
    void patchNonExistingAlAppleVi() throws Exception {
        ...
        restAlAppleViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alAppleViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alAppleViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlAppleVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlAppleViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alAppleViDTO)))
            .andExpect(status().isMethodNotAllowed());
