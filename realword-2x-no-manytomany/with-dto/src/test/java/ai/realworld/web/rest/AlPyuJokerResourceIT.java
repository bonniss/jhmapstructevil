...
class AlPyuJokerResourceIT {
    ...
    void createAlPyuJoker() throws Exception {
        ...
        AlPyuJokerDTO alPyuJokerDTO = alPyuJokerMapper.toDto(alPyuJoker);
        var returnedAlPyuJokerDTO = om.readValue(
            restAlPyuJokerMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPyuJokerDTO)))
        ...
        var returnedAlPyuJoker = alPyuJokerMapper.toEntity(returnedAlPyuJokerDTO);
    ...
    void putExistingAlPyuJoker() throws Exception {
        ...
        updatedAlPyuJoker
            ...
            .bookingStatus(UPDATED_BOOKING_STATUS)
            .historyRefJason(UPDATED_HISTORY_REF_JASON);
        AlPyuJokerDTO alPyuJokerDTO = alPyuJokerMapper.toDto(updatedAlPyuJoker);

        restAlPyuJokerMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPyuJokerDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alPyuJokerDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlPyuJoker() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPyuJokerMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPyuJokerDTO)))
    ...
    void patchNonExistingAlPyuJoker() throws Exception {
        ...
        restAlPyuJokerMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPyuJokerDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPyuJokerDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPyuJoker() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPyuJokerMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPyuJokerDTO)))
            .andExpect(status().isMethodNotAllowed());
