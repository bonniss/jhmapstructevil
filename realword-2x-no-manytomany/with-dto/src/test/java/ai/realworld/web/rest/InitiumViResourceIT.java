...
class InitiumViResourceIT {
    ...
    void createInitiumVi() throws Exception {
        ...
        InitiumViDTO initiumViDTO = initiumViMapper.toDto(initiumVi);
        var returnedInitiumViDTO = om.readValue(
            restInitiumViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(initiumViDTO)))
        ...
        var returnedInitiumVi = initiumViMapper.toEntity(returnedInitiumViDTO);
    ...
    void putExistingInitiumVi() throws Exception {
        ...
        updatedInitiumVi
            ...
            .description(UPDATED_DESCRIPTION)
            .isJelloSupported(UPDATED_IS_JELLO_SUPPORTED);
        InitiumViDTO initiumViDTO = initiumViMapper.toDto(updatedInitiumVi);

        restInitiumViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, initiumViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(initiumViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamInitiumVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInitiumViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(initiumViDTO)))
    ...
    void patchNonExistingInitiumVi() throws Exception {
        ...
        restInitiumViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, initiumViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(initiumViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamInitiumVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInitiumViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(initiumViDTO)))
            .andExpect(status().isMethodNotAllowed());
