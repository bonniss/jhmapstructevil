...
class AlBetonamuRelationResourceIT {
    ...
    void createAlBetonamuRelation() throws Exception {
        ...
        AlBetonamuRelationDTO alBetonamuRelationDTO = alBetonamuRelationMapper.toDto(alBetonamuRelation);
        var returnedAlBetonamuRelationDTO = om.readValue(
            restAlBetonamuRelationMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alBetonamuRelationDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlBetonamuRelationDTO.class
        );

        // Validate the AlBetonamuRelation in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlBetonamuRelation = alBetonamuRelationMapper.toEntity(returnedAlBetonamuRelationDTO);
    ...
    void putExistingAlBetonamuRelation() throws Exception {
        ...
        AlBetonamuRelationDTO alBetonamuRelationDTO = alBetonamuRelationMapper.toDto(updatedAlBetonamuRelation);

        restAlBetonamuRelationMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alBetonamuRelationDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alBetonamuRelationDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlBetonamuRelation() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlBetonamuRelationMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alBetonamuRelationDTO)))
    ...
    void patchNonExistingAlBetonamuRelation() throws Exception {
        ...
        restAlBetonamuRelationMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alBetonamuRelationDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alBetonamuRelationDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlBetonamuRelation() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlBetonamuRelationMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alBetonamuRelationDTO)))
            .andExpect(status().isMethodNotAllowed());
