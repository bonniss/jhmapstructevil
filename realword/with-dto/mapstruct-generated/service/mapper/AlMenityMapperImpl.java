...
@Generated(
    ...
    date = "2024-10-22T00:19:35+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlMenityMapperImpl implements AlMenityMapper {

    @Override
    public List<AlMenity> toEntity(List<AlMenityDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlMenity> list = new ArrayList<AlMenity>( dtoList.size() );
        for ( AlMenityDTO alMenityDTO : dtoList ) {
            list.add( toEntity( alMenityDTO ) );
        }
    ...
    @Override
    public List<AlMenityDTO> toDto(List<AlMenity> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlMenityDTO> list = new ArrayList<AlMenityDTO>( entityList.size() );
        for ( AlMenity alMenity : entityList ) {
            list.add( toDto( alMenity ) );
        }
    ...
    @Override
    public void partialUpdate(AlMenity entity, AlMenityDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getIconSvg() != null ) {
            entity.setIconSvg( dto.getIconSvg() );
        }
        ...
        if ( entity.getPropertyProfiles() != null ) {
            Set<AlProPro> set = alProProDTOSetToAlProProSet( dto.getPropertyProfiles() );
            if ( set != null ) {
                entity.getPropertyProfiles().clear();
                entity.getPropertyProfiles().addAll( set );
            }
        }
        else {
            Set<AlProPro> set = alProProDTOSetToAlProProSet( dto.getPropertyProfiles() );
            if ( set != null ) {
                entity.propertyProfiles( set );
            }
        }
    }

    @Override
    public AlMenityDTO toDto(AlMenity s) {
        if ( s == null ) {
            return null;
        }
        ...
        alMenityDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alMenityDTO.setPropertyProfiles( toDtoAlProProIdSet( s.getPropertyProfiles() ) );
        alMenityDTO.setId( s.getId() );
        alMenityDTO.setName( s.getName() );
        alMenityDTO.setIconSvg( s.getIconSvg() );
        alMenityDTO.setPropertyType( s.getPropertyType() );

        return alMenityDTO;
    }

    @Override
    public AlMenity toEntity(AlMenityDTO alMenityDTO) {
        if ( alMenityDTO == null ) {
            return null;
        }
