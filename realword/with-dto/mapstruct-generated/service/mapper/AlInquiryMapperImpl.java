...
@Generated(
    ...
    date = "2024-10-22T00:21:32+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlInquiryMapperImpl implements AlInquiryMapper {

    @Override
    public AlInquiry toEntity(AlInquiryDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AlInquiry alInquiry = new AlInquiry();

        alInquiry.setId( dto.getId() );
        alInquiry.setTitle( dto.getTitle() );
        alInquiry.setBody( dto.getBody() );
        alInquiry.setSender( dto.getSender() );
        alInquiry.setEmail( dto.getEmail() );
        alInquiry.setPhone( dto.getPhone() );
        alInquiry.setContentJason( dto.getContentJason() );
        alInquiry.customer( alPacinoDTOToAlPacino( dto.getCustomer() ) );
        alInquiry.agency( alAppleDTOToAlApple( dto.getAgency() ) );
        alInquiry.personInCharge( edSheeranDTOToEdSheeran( dto.getPersonInCharge() ) );
        alInquiry.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alInquiry;
    }

    @Override
    public List<AlInquiry> toEntity(List<AlInquiryDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlInquiry> list = new ArrayList<AlInquiry>( dtoList.size() );
        for ( AlInquiryDTO alInquiryDTO : dtoList ) {
            list.add( toEntity( alInquiryDTO ) );
        }
    ...
    @Override
    public List<AlInquiryDTO> toDto(List<AlInquiry> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlInquiryDTO> list = new ArrayList<AlInquiryDTO>( entityList.size() );
        for ( AlInquiry alInquiry : entityList ) {
            list.add( toDto( alInquiry ) );
        }
    ...
    @Override
    public void partialUpdate(AlInquiry entity, AlInquiryDTO dto) {
        if ( dto == null ) {
            return;
        }
    ...
    @Override
    public AlInquiryDTO toDto(AlInquiry s) {
        if ( s == null ) {
            return null;
        }

        AlInquiryDTO alInquiryDTO = new AlInquiryDTO();

        alInquiryDTO.setCustomer( toDtoAlPacinoId( s.getCustomer() ) );
        alInquiryDTO.setAgency( toDtoAlAppleId( s.getAgency() ) );
        alInquiryDTO.setPersonInCharge( toDtoEdSheeranId( s.getPersonInCharge() ) );
        alInquiryDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alInquiryDTO.setId( s.getId() );
        alInquiryDTO.setTitle( s.getTitle() );
        alInquiryDTO.setBody( s.getBody() );
        alInquiryDTO.setSender( s.getSender() );
        alInquiryDTO.setEmail( s.getEmail() );
        alInquiryDTO.setPhone( s.getPhone() );
        alInquiryDTO.setContentJason( s.getContentJason() );
