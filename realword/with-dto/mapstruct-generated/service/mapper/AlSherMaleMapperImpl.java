...
@Generated(
    ...
    date = "2024-10-22T00:22:59+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlSherMaleMapperImpl implements AlSherMaleMapper {

    @Override
    public AlSherMale toEntity(AlSherMaleDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AlSherMale alSherMale = new AlSherMale();

        alSherMale.setId( dto.getId() );
        alSherMale.setDataSourceMappingType( dto.getDataSourceMappingType() );
        alSherMale.setKeyword( dto.getKeyword() );
        alSherMale.setProperty( dto.getProperty() );
        alSherMale.setTitle( dto.getTitle() );
        alSherMale.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alSherMale;
    }

    @Override
    public List<AlSherMale> toEntity(List<AlSherMaleDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlSherMale> list = new ArrayList<AlSherMale>( dtoList.size() );
        for ( AlSherMaleDTO alSherMaleDTO : dtoList ) {
            list.add( toEntity( alSherMaleDTO ) );
        }
    ...
    @Override
    public List<AlSherMaleDTO> toDto(List<AlSherMale> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlSherMaleDTO> list = new ArrayList<AlSherMaleDTO>( entityList.size() );
        for ( AlSherMale alSherMale : entityList ) {
            list.add( toDto( alSherMale ) );
        }
    ...
    @Override
    public void partialUpdate(AlSherMale entity, AlSherMaleDTO dto) {
        if ( dto == null ) {
            return;
        }
    ...
    @Override
    public AlSherMaleDTO toDto(AlSherMale s) {
        if ( s == null ) {
            return null;
        }

        AlSherMaleDTO alSherMaleDTO = new AlSherMaleDTO();

        alSherMaleDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alSherMaleDTO.setId( s.getId() );
        alSherMaleDTO.setDataSourceMappingType( s.getDataSourceMappingType() );
        alSherMaleDTO.setKeyword( s.getKeyword() );
        alSherMaleDTO.setProperty( s.getProperty() );
        alSherMaleDTO.setTitle( s.getTitle() );
