...
@Generated(
    ...
    date = "2024-10-22T00:20:18+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlPacinoPointHistoryMapperImpl implements AlPacinoPointHistoryMapper {

    @Override
    public AlPacinoPointHistory toEntity(AlPacinoPointHistoryDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AlPacinoPointHistory alPacinoPointHistory = new AlPacinoPointHistory();

        alPacinoPointHistory.setId( dto.getId() );
        alPacinoPointHistory.setSource( dto.getSource() );
        alPacinoPointHistory.setAssociatedId( dto.getAssociatedId() );
        alPacinoPointHistory.setPointAmount( dto.getPointAmount() );
        alPacinoPointHistory.customer( alPacinoDTOToAlPacino( dto.getCustomer() ) );
        alPacinoPointHistory.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alPacinoPointHistory;
    }

    @Override
    public List<AlPacinoPointHistory> toEntity(List<AlPacinoPointHistoryDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlPacinoPointHistory> list = new ArrayList<AlPacinoPointHistory>( dtoList.size() );
        for ( AlPacinoPointHistoryDTO alPacinoPointHistoryDTO : dtoList ) {
            list.add( toEntity( alPacinoPointHistoryDTO ) );
        }
    ...
    @Override
    public List<AlPacinoPointHistoryDTO> toDto(List<AlPacinoPointHistory> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlPacinoPointHistoryDTO> list = new ArrayList<AlPacinoPointHistoryDTO>( entityList.size() );
        for ( AlPacinoPointHistory alPacinoPointHistory : entityList ) {
            list.add( toDto( alPacinoPointHistory ) );
        }
    ...
    @Override
    public void partialUpdate(AlPacinoPointHistory entity, AlPacinoPointHistoryDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getAssociatedId() != null ) {
            entity.setAssociatedId( dto.getAssociatedId() );
        }
        if ( dto.getPointAmount() != null ) {
    ...
    @Override
    public AlPacinoPointHistoryDTO toDto(AlPacinoPointHistory s) {
        if ( s == null ) {
            return null;
        }

        AlPacinoPointHistoryDTO alPacinoPointHistoryDTO = new AlPacinoPointHistoryDTO();

        alPacinoPointHistoryDTO.setCustomer( toDtoAlPacinoId( s.getCustomer() ) );
        alPacinoPointHistoryDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alPacinoPointHistoryDTO.setId( s.getId() );
        alPacinoPointHistoryDTO.setSource( s.getSource() );
        alPacinoPointHistoryDTO.setAssociatedId( s.getAssociatedId() );
        alPacinoPointHistoryDTO.setPointAmount( s.getPointAmount() );
