...
@Generated(
    ...
    date = "2024-10-22T00:18:43+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class HandCraftMapperImpl implements HandCraftMapper {

    @Override
    public HandCraft toEntity(HandCraftDTO dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        handCraft.setId( dto.getId() );
        handCraft.agent( edSheeranDTOToEdSheeran( dto.getAgent() ) );
        handCraft.role( rihannaDTOToRihanna( dto.getRole() ) );
        handCraft.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return handCraft;
    }

    @Override
    public List<HandCraft> toEntity(List<HandCraftDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<HandCraft> list = new ArrayList<HandCraft>( dtoList.size() );
        for ( HandCraftDTO handCraftDTO : dtoList ) {
            list.add( toEntity( handCraftDTO ) );
        }
    ...
    @Override
    public List<HandCraftDTO> toDto(List<HandCraft> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<HandCraftDTO> list = new ArrayList<HandCraftDTO>( entityList.size() );
        for ( HandCraft handCraft : entityList ) {
            list.add( toDto( handCraft ) );
        }
    ...
    @Override
    public void partialUpdate(HandCraft entity, HandCraftDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getAgent() != null ) {
            if ( entity.getAgent() == null ) {
                entity.agent( new EdSheeran() );
            }
            edSheeranDTOToEdSheeran1( dto.getAgent(), entity.getAgent() );
        }
        if ( dto.getRole() != null ) {
            if ( entity.getRole() == null ) {
                entity.role( new Rihanna() );
            }
            rihannaDTOToRihanna1( dto.getRole(), entity.getRole() );
        }
    ...
    @Override
    public HandCraftDTO toDto(HandCraft s) {
        if ( s == null ) {
            return null;
        }

        HandCraftDTO handCraftDTO = new HandCraftDTO();

        handCraftDTO.setAgent( toDtoEdSheeranId( s.getAgent() ) );
        handCraftDTO.setRole( toDtoRihannaId( s.getRole() ) );
        handCraftDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        handCraftDTO.setId( s.getId() );
    ...
    @Override
    public RihannaDTO toDtoRihannaId(Rihanna rihanna) {
        if ( rihanna == null ) {
            return null;
        }

        RihannaDTO rihannaDTO = new RihannaDTO();

        rihannaDTO.setId( rihanna.getId() );
    ...
    protected Rihanna rihannaDTOToRihanna(RihannaDTO rihannaDTO) {
        if ( rihannaDTO == null ) {
            return null;
        }

        Rihanna rihanna = new Rihanna();

        rihanna.setId( rihannaDTO.getId() );
        rihanna.setName( rihannaDTO.getName() );
        rihanna.setDescription( rihannaDTO.getDescription() );
        rihanna.setPermissionGridJason( rihannaDTO.getPermissionGridJason() );
        rihanna.application( johnLennonDTOToJohnLennon( rihannaDTO.getApplication() ) );
    ...
    protected void rihannaDTOToRihanna1(RihannaDTO rihannaDTO, Rihanna mappingTarget) {
        if ( rihannaDTO == null ) {
            return;
        }

        if ( rihannaDTO.getId() != null ) {
            mappingTarget.setId( rihannaDTO.getId() );
        }
        if ( rihannaDTO.getName() != null ) {
            mappingTarget.setName( rihannaDTO.getName() );
        }
        if ( rihannaDTO.getDescription() != null ) {
            mappingTarget.setDescription( rihannaDTO.getDescription() );
        }
        if ( rihannaDTO.getPermissionGridJason() != null ) {
            mappingTarget.setPermissionGridJason( rihannaDTO.getPermissionGridJason() );
        }
        if ( rihannaDTO.getApplication() != null ) {
            if ( mappingTarget.getApplication() == null ) {
                mappingTarget.application( new JohnLennon() );
            }
            johnLennonDTOToJohnLennon1( rihannaDTO.getApplication(), mappingTarget.getApplication() );
        }
    }
}
