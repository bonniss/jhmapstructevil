...
@Generated(
    ...
    date = "2024-10-22T00:20:27+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlLexFergMapperImpl implements AlLexFergMapper {

    @Override
    public List<AlLexFerg> toEntity(List<AlLexFergDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlLexFerg> list = new ArrayList<AlLexFerg>( dtoList.size() );
        for ( AlLexFergDTO alLexFergDTO : dtoList ) {
            list.add( toEntity( alLexFergDTO ) );
        }
    ...
    @Override
    public List<AlLexFergDTO> toDto(List<AlLexFerg> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlLexFergDTO> list = new ArrayList<AlLexFergDTO>( entityList.size() );
        for ( AlLexFerg alLexFerg : entityList ) {
            list.add( toDto( alLexFerg ) );
        }
    ...
    @Override
    public void partialUpdate(AlLexFerg entity, AlLexFergDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getCategory() != null ) {
            if ( entity.getCategory() == null ) {
                entity.category( new AlCatalina() );
            }
            alCatalinaDTOToAlCatalina( dto.getCategory(), entity.getCategory() );
        }
        ...
        if ( entity.getTags() != null ) {
            Set<AlBestTooth> set = alBestToothDTOSetToAlBestToothSet( dto.getTags() );
            if ( set != null ) {
    ...
    @Override
    public AlLexFergDTO toDto(AlLexFerg s) {
        if ( s == null ) {
            return null;
        }

        AlLexFergDTO alLexFergDTO = new AlLexFergDTO();

        alLexFergDTO.setAvatar( toDtoMetaverseId( s.getAvatar() ) );
        alLexFergDTO.setCategory( toDtoAlCatalinaId( s.getCategory() ) );
        alLexFergDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alLexFergDTO.setTags( toDtoAlBestToothIdSet( s.getTags() ) );
        alLexFergDTO.setId( s.getId() );
        alLexFergDTO.setTitle( s.getTitle() );
        alLexFergDTO.setSlug( s.getSlug() );
        alLexFergDTO.setSummary( s.getSummary() );
        alLexFergDTO.setContentHeitiga( s.getContentHeitiga() );
        alLexFergDTO.setPublicationStatus( s.getPublicationStatus() );
        alLexFergDTO.setPublishedDate( s.getPublishedDate() );

        return alLexFergDTO;
    }

    @Override
    public AlLexFerg toEntity(AlLexFergDTO alLexFergDTO) {
        if ( alLexFergDTO == null ) {
            return null;
        }

        AlLexFerg alLexFerg = new AlLexFerg();

        alLexFerg.setId( alLexFergDTO.getId() );
        alLexFerg.setTitle( alLexFergDTO.getTitle() );
        alLexFerg.setSlug( alLexFergDTO.getSlug() );
        alLexFerg.setSummary( alLexFergDTO.getSummary() );
        alLexFerg.setContentHeitiga( alLexFergDTO.getContentHeitiga() );
        alLexFerg.setPublicationStatus( alLexFergDTO.getPublicationStatus() );
        alLexFerg.setPublishedDate( alLexFergDTO.getPublishedDate() );
        alLexFerg.avatar( metaverseDTOToMetaverse( alLexFergDTO.getAvatar() ) );
        alLexFerg.category( alCatalinaDTOToAlCatalina1( alLexFergDTO.getCategory() ) );
        alLexFerg.application( johnLennonDTOToJohnLennon( alLexFergDTO.getApplication() ) );
        alLexFerg.tags( alBestToothDTOSetToAlBestToothSet( alLexFergDTO.getTags() ) );
    ...
    @Override
    public AlCatalinaDTO toDtoAlCatalinaId(AlCatalina alCatalina) {
        if ( alCatalina == null ) {
            return null;
        }

        AlCatalinaDTO alCatalinaDTO = new AlCatalinaDTO();

        alCatalinaDTO.setId( alCatalina.getId() );
    ...
    @Override
    public AlBestToothDTO toDtoAlBestToothId(AlBestTooth alBestTooth) {
        if ( alBestTooth == null ) {
            return null;
        }

        AlBestToothDTO alBestToothDTO = new AlBestToothDTO();

        alBestToothDTO.setId( alBestTooth.getId() );
    ...
    protected void alCatalinaDTOToAlCatalina(AlCatalinaDTO alCatalinaDTO, AlCatalina mappingTarget) {
        if ( alCatalinaDTO == null ) {
            return;
        }

        if ( alCatalinaDTO.getId() != null ) {
            mappingTarget.setId( alCatalinaDTO.getId() );
        }
        if ( alCatalinaDTO.getName() != null ) {
            mappingTarget.setName( alCatalinaDTO.getName() );
        }
        if ( alCatalinaDTO.getDescription() != null ) {
            mappingTarget.setDescription( alCatalinaDTO.getDescription() );
        }
        if ( alCatalinaDTO.getTreeDepth() != null ) {
            mappingTarget.setTreeDepth( alCatalinaDTO.getTreeDepth() );
        }
        if ( alCatalinaDTO.getParent() != null ) {
            if ( mappingTarget.getParent() == null ) {
                mappingTarget.parent( new AlCatalina() );
            }
            alCatalinaDTOToAlCatalina( alCatalinaDTO.getParent(), mappingTarget.getParent() );
        }
        if ( alCatalinaDTO.getAvatar() != null ) {
            if ( mappingTarget.getAvatar() == null ) {
                mappingTarget.avatar( new Metaverse() );
            }
            metaverseDTOToMetaverse( alCatalinaDTO.getAvatar(), mappingTarget.getAvatar() );
        }
        if ( alCatalinaDTO.getApplication() != null ) {
            if ( mappingTarget.getApplication() == null ) {
                mappingTarget.application( new JohnLennon() );
            }
            johnLennonDTOToJohnLennon1( alCatalinaDTO.getApplication(), mappingTarget.getApplication() );
        }
    }

    protected Set<AlLexFerg> alLexFergDTOSetToAlLexFergSet(Set<AlLexFergDTO> set) {
        if ( set == null ) {
            return null;
        }

        Set<AlLexFerg> set1 = new LinkedHashSet<AlLexFerg>( Math.max( (int) ( set.size() / .75f ) + 1, 16 ) );
        for ( AlLexFergDTO alLexFergDTO : set ) {
            set1.add( toEntity( alLexFergDTO ) );
        }

        return set1;
    }

    protected AlBestTooth alBestToothDTOToAlBestTooth(AlBestToothDTO alBestToothDTO) {
        if ( alBestToothDTO == null ) {
            return null;
        }

        AlBestTooth alBestTooth = new AlBestTooth();

        alBestTooth.setId( alBestToothDTO.getId() );
        alBestTooth.setName( alBestToothDTO.getName() );
        alBestTooth.application( johnLennonDTOToJohnLennon( alBestToothDTO.getApplication() ) );
        alBestTooth.articles( alLexFergDTOSetToAlLexFergSet( alBestToothDTO.getArticles() ) );

        return alBestTooth;
    }

    protected Set<AlBestTooth> alBestToothDTOSetToAlBestToothSet(Set<AlBestToothDTO> set) {
        if ( set == null ) {
            return null;
        }

        Set<AlBestTooth> set1 = new LinkedHashSet<AlBestTooth>( Math.max( (int) ( set.size() / .75f ) + 1, 16 ) );
        for ( AlBestToothDTO alBestToothDTO : set ) {
            set1.add( alBestToothDTOToAlBestTooth( alBestToothDTO ) );
        }

        return set1;
    }

    protected AlCatalina alCatalinaDTOToAlCatalina1(AlCatalinaDTO alCatalinaDTO) {
        if ( alCatalinaDTO == null ) {
            return null;
        }

        AlCatalina alCatalina = new AlCatalina();

        alCatalina.setId( alCatalinaDTO.getId() );
        alCatalina.setName( alCatalinaDTO.getName() );
        alCatalina.setDescription( alCatalinaDTO.getDescription() );
        alCatalina.setTreeDepth( alCatalinaDTO.getTreeDepth() );
        alCatalina.parent( alCatalinaDTOToAlCatalina1( alCatalinaDTO.getParent() ) );
        alCatalina.avatar( metaverseDTOToMetaverse( alCatalinaDTO.getAvatar() ) );
        alCatalina.application( johnLennonDTOToJohnLennon( alCatalinaDTO.getApplication() ) );
