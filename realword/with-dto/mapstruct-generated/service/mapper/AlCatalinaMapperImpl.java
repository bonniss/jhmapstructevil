...
@Generated(
    ...
    date = "2024-10-22T00:18:54+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlCatalinaMapperImpl implements AlCatalinaMapper {

    @Override
    public AlCatalina toEntity(AlCatalinaDTO dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        alCatalina.setId( dto.getId() );
        alCatalina.setName( dto.getName() );
        alCatalina.setDescription( dto.getDescription() );
        alCatalina.setTreeDepth( dto.getTreeDepth() );
        alCatalina.parent( toEntity( dto.getParent() ) );
        alCatalina.avatar( metaverseDTOToMetaverse( dto.getAvatar() ) );
        alCatalina.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alCatalina;
    }

    @Override
    public List<AlCatalina> toEntity(List<AlCatalinaDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlCatalina> list = new ArrayList<AlCatalina>( dtoList.size() );
        for ( AlCatalinaDTO alCatalinaDTO : dtoList ) {
            list.add( toEntity( alCatalinaDTO ) );
        }
    ...
    @Override
    public List<AlCatalinaDTO> toDto(List<AlCatalina> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlCatalinaDTO> list = new ArrayList<AlCatalinaDTO>( entityList.size() );
        for ( AlCatalina alCatalina : entityList ) {
            list.add( toDto( alCatalina ) );
        }
    ...
    @Override
    public void partialUpdate(AlCatalina entity, AlCatalinaDTO dto) {
        if ( dto == null ) {
            return;
        }
    ...
    @Override
    public AlCatalinaDTO toDto(AlCatalina s) {
        if ( s == null ) {
            return null;
        }
        ...
        alCatalinaDTO.setParent( toDtoAlCatalinaId( s.getParent() ) );
        alCatalinaDTO.setAvatar( toDtoMetaverseId( s.getAvatar() ) );
        alCatalinaDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alCatalinaDTO.setId( s.getId() );
        alCatalinaDTO.setName( s.getName() );
        alCatalinaDTO.setDescription( s.getDescription() );
        alCatalinaDTO.setTreeDepth( s.getTreeDepth() );
