...
@Generated(
    ...
    date = "2024-10-22T00:21:34+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlPedroTaxMapperImpl implements AlPedroTaxMapper {

    @Override
    public AlPedroTax toEntity(AlPedroTaxDTO dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        alPedroTax.setId( dto.getId() );
        alPedroTax.setName( dto.getName() );
        alPedroTax.setDescription( dto.getDescription() );
        alPedroTax.setWeight( dto.getWeight() );
        alPedroTax.setPropertyType( dto.getPropertyType() );
        alPedroTax.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alPedroTax;
    }

    @Override
    public List<AlPedroTax> toEntity(List<AlPedroTaxDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlPedroTax> list = new ArrayList<AlPedroTax>( dtoList.size() );
        for ( AlPedroTaxDTO alPedroTaxDTO : dtoList ) {
            list.add( toEntity( alPedroTaxDTO ) );
        }
    ...
    @Override
    public List<AlPedroTaxDTO> toDto(List<AlPedroTax> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlPedroTaxDTO> list = new ArrayList<AlPedroTaxDTO>( entityList.size() );
        for ( AlPedroTax alPedroTax : entityList ) {
            list.add( toDto( alPedroTax ) );
        }
    ...
    @Override
    public void partialUpdate(AlPedroTax entity, AlPedroTaxDTO dto) {
        if ( dto == null ) {
            return;
        }
    ...
    @Override
    public AlPedroTaxDTO toDto(AlPedroTax s) {
        if ( s == null ) {
            return null;
        }
        ...
        alPedroTaxDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alPedroTaxDTO.setId( s.getId() );
        alPedroTaxDTO.setName( s.getName() );
        alPedroTaxDTO.setDescription( s.getDescription() );
        alPedroTaxDTO.setWeight( s.getWeight() );
        alPedroTaxDTO.setPropertyType( s.getPropertyType() );
