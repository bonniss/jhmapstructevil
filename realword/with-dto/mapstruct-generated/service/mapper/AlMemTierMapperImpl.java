...
@Generated(
    ...
    date = "2024-10-22T00:20:01+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlMemTierMapperImpl implements AlMemTierMapper {

    @Override
    public AlMemTier toEntity(AlMemTierDTO dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        alMemTier.setId( dto.getId() );
        alMemTier.setName( dto.getName() );
        alMemTier.setDescription( dto.getDescription() );
        alMemTier.setMinPoint( dto.getMinPoint() );
        alMemTier.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alMemTier;
    }

    @Override
    public List<AlMemTier> toEntity(List<AlMemTierDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlMemTier> list = new ArrayList<AlMemTier>( dtoList.size() );
        for ( AlMemTierDTO alMemTierDTO : dtoList ) {
            list.add( toEntity( alMemTierDTO ) );
        }
    ...
    @Override
    public List<AlMemTierDTO> toDto(List<AlMemTier> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlMemTierDTO> list = new ArrayList<AlMemTierDTO>( entityList.size() );
        for ( AlMemTier alMemTier : entityList ) {
            list.add( toDto( alMemTier ) );
        }
    ...
    @Override
    public void partialUpdate(AlMemTier entity, AlMemTierDTO dto) {
        if ( dto == null ) {
            return;
        }
    ...
    @Override
    public AlMemTierDTO toDto(AlMemTier s) {
        if ( s == null ) {
            return null;
        }
        ...
        alMemTierDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alMemTierDTO.setId( s.getId() );
        alMemTierDTO.setName( s.getName() );
        alMemTierDTO.setDescription( s.getDescription() );
        alMemTierDTO.setMinPoint( s.getMinPoint() );
    ...
    protected AlPacino alPacinoDTOToAlPacino(AlPacinoDTO alPacinoDTO) {
        ...
        alPacino.membershipTier( toEntity( alPacinoDTO.getMembershipTier() ) );
