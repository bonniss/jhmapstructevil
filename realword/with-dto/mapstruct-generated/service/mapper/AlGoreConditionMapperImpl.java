...
@Component
public class AlGoreConditionMapperImpl implements AlGoreConditionMapper {

    @Override
    public AlGoreCondition toEntity(AlGoreConditionDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AlGoreCondition alGoreCondition = new AlGoreCondition();

        alGoreCondition.setId( dto.getId() );
        alGoreCondition.setSubjectType( dto.getSubjectType() );
        alGoreCondition.setSubject( dto.getSubject() );
        alGoreCondition.setAction( dto.getAction() );
        alGoreCondition.setNote( dto.getNote() );
        alGoreCondition.parent( alGoreDTOToAlGore( dto.getParent() ) );
        alGoreCondition.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alGoreCondition;
    }

    @Override
    public List<AlGoreCondition> toEntity(List<AlGoreConditionDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlGoreCondition> list = new ArrayList<AlGoreCondition>( dtoList.size() );
        for ( AlGoreConditionDTO alGoreConditionDTO : dtoList ) {
            list.add( toEntity( alGoreConditionDTO ) );
        }
    ...
    @Override
    public List<AlGoreConditionDTO> toDto(List<AlGoreCondition> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlGoreConditionDTO> list = new ArrayList<AlGoreConditionDTO>( entityList.size() );
        for ( AlGoreCondition alGoreCondition : entityList ) {
            list.add( toDto( alGoreCondition ) );
        }
    ...
    @Override
    public void partialUpdate(AlGoreCondition entity, AlGoreConditionDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getParent() != null ) {
            if ( entity.getParent() == null ) {
                entity.parent( new AlGore() );
            }
            alGoreDTOToAlGore1( dto.getParent(), entity.getParent() );
        }
    ...
    @Override
    public AlGoreConditionDTO toDto(AlGoreCondition s) {
        if ( s == null ) {
            return null;
        }

        AlGoreConditionDTO alGoreConditionDTO = new AlGoreConditionDTO();

        alGoreConditionDTO.setParent( toDtoAlGoreId( s.getParent() ) );
        alGoreConditionDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alGoreConditionDTO.setId( s.getId() );
        alGoreConditionDTO.setSubjectType( s.getSubjectType() );
        alGoreConditionDTO.setSubject( s.getSubject() );
        alGoreConditionDTO.setAction( s.getAction() );
        alGoreConditionDTO.setNote( s.getNote() );

        return alGoreConditionDTO;
    }

    @Override
    public AlGoreDTO toDtoAlGoreId(AlGore alGore) {
        if ( alGore == null ) {
            return null;
        }
    ...
    protected AlGore alGoreDTOToAlGore(AlGoreDTO alGoreDTO) {
        if ( alGoreDTO == null ) {
            return null;
        }
        ...
        alGore.setId( alGoreDTO.getId() );
        alGore.setName( alGoreDTO.getName() );
        alGore.setDiscountType( alGoreDTO.getDiscountType() );
        alGore.setDiscountRate( alGoreDTO.getDiscountRate() );
        alGore.setScope( alGoreDTO.getScope() );
        alGore.bizRelation( alBetonamuRelationDTOToAlBetonamuRelation( alGoreDTO.getBizRelation() ) );
        alGore.application( johnLennonDTOToJohnLennon( alGoreDTO.getApplication() ) );
    ...
    protected void alGoreDTOToAlGore1(AlGoreDTO alGoreDTO, AlGore mappingTarget) {
        if ( alGoreDTO == null ) {
            return;
        }

        if ( alGoreDTO.getId() != null ) {
            mappingTarget.setId( alGoreDTO.getId() );
        }
        if ( alGoreDTO.getName() != null ) {
            mappingTarget.setName( alGoreDTO.getName() );
        }
        if ( alGoreDTO.getDiscountType() != null ) {
            mappingTarget.setDiscountType( alGoreDTO.getDiscountType() );
        }
        if ( alGoreDTO.getDiscountRate() != null ) {
            mappingTarget.setDiscountRate( alGoreDTO.getDiscountRate() );
        }
        if ( alGoreDTO.getScope() != null ) {
            mappingTarget.setScope( alGoreDTO.getScope() );
        }
        if ( alGoreDTO.getBizRelation() != null ) {
            if ( mappingTarget.getBizRelation() == null ) {
                mappingTarget.bizRelation( new AlBetonamuRelation() );
            }
            alBetonamuRelationDTOToAlBetonamuRelation1( alGoreDTO.getBizRelation(), mappingTarget.getBizRelation() );
        }
        if ( alGoreDTO.getApplication() != null ) {
            if ( mappingTarget.getApplication() == null ) {
                mappingTarget.application( new JohnLennon() );
            }
            johnLennonDTOToJohnLennon1( alGoreDTO.getApplication(), mappingTarget.getApplication() );
        }
    }
}
