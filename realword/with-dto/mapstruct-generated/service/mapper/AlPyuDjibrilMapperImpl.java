...
public class AlPyuDjibrilMapperImpl implements AlPyuDjibrilMapper {
    ...
    protected void alProtyDTOToAlProty1(AlProtyDTO alProtyDTO, AlProty mappingTarget) {
        ...
        if ( mappingTarget.getImages() != null ) {
            Set<Metaverse> set = metaverseDTOSetToMetaverseSet( alProtyDTO.getImages() );
            if ( set != null ) {
                mappingTarget.getImages().clear();
                mappingTarget.getImages().addAll( set );
            }
        }
        else {
            Set<Metaverse> set = metaverseDTOSetToMetaverseSet( alProtyDTO.getImages() );
            if ( set != null ) {
                mappingTarget.images( set );
            }
        }
        if ( mappingTarget.getBookings() != null ) {
            Set<AlPyuJoker> set1 = alPyuJokerDTOSetToAlPyuJokerSet( alProtyDTO.getBookings() );
            if ( set1 != null ) {
                mappingTarget.getBookings().clear();
                mappingTarget.getBookings().addAll( set1 );
            }
        }
        else {
            Set<AlPyuJoker> set1 = alPyuJokerDTOSetToAlPyuJokerSet( alProtyDTO.getBookings() );
            if ( set1 != null ) {
                mappingTarget.bookings( set1 );
            }
        }
    }
}
