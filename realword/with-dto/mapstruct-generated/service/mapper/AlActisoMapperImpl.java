...
@Generated(
    ...
    date = "2024-10-22T00:21:50+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlActisoMapperImpl implements AlActisoMapper {

    @Override
    public AlActiso toEntity(AlActisoDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AlActiso alActiso = new AlActiso();

        alActiso.setId( dto.getId() );
        alActiso.setKey( dto.getKey() );
        alActiso.setValueJason( dto.getValueJason() );
        alActiso.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alActiso;
    }

    @Override
    public List<AlActiso> toEntity(List<AlActisoDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlActiso> list = new ArrayList<AlActiso>( dtoList.size() );
        for ( AlActisoDTO alActisoDTO : dtoList ) {
            list.add( toEntity( alActisoDTO ) );
        }
    ...
    @Override
    public List<AlActisoDTO> toDto(List<AlActiso> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlActisoDTO> list = new ArrayList<AlActisoDTO>( entityList.size() );
        for ( AlActiso alActiso : entityList ) {
            list.add( toDto( alActiso ) );
        }
    ...
    @Override
    public void partialUpdate(AlActiso entity, AlActisoDTO dto) {
        if ( dto == null ) {
            return;
        }
    ...
    @Override
    public AlActisoDTO toDto(AlActiso s) {
        if ( s == null ) {
            return null;
        }

        AlActisoDTO alActisoDTO = new AlActisoDTO();

        alActisoDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alActisoDTO.setId( s.getId() );
        alActisoDTO.setKey( s.getKey() );
        alActisoDTO.setValueJason( s.getValueJason() );
