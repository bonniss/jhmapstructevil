...
@Generated(
    ...
    date = "2024-10-22T00:18:51+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlVueVueConditionMapperImpl implements AlVueVueConditionMapper {

    @Override
    public AlVueVueCondition toEntity(AlVueVueConditionDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AlVueVueCondition alVueVueCondition = new AlVueVueCondition();

        alVueVueCondition.setId( dto.getId() );
        alVueVueCondition.setSubjectType( dto.getSubjectType() );
        alVueVueCondition.setSubject( dto.getSubject() );
        alVueVueCondition.setAction( dto.getAction() );
        alVueVueCondition.setNote( dto.getNote() );
        alVueVueCondition.parent( alVueVueDTOToAlVueVue( dto.getParent() ) );
        alVueVueCondition.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alVueVueCondition;
    }

    @Override
    public List<AlVueVueCondition> toEntity(List<AlVueVueConditionDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlVueVueCondition> list = new ArrayList<AlVueVueCondition>( dtoList.size() );
        for ( AlVueVueConditionDTO alVueVueConditionDTO : dtoList ) {
            list.add( toEntity( alVueVueConditionDTO ) );
        }
    ...
    @Override
    public List<AlVueVueConditionDTO> toDto(List<AlVueVueCondition> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlVueVueConditionDTO> list = new ArrayList<AlVueVueConditionDTO>( entityList.size() );
        for ( AlVueVueCondition alVueVueCondition : entityList ) {
            list.add( toDto( alVueVueCondition ) );
        }
    ...
    @Override
    public void partialUpdate(AlVueVueCondition entity, AlVueVueConditionDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getParent() != null ) {
            if ( entity.getParent() == null ) {
                entity.parent( new AlVueVue() );
            }
            alVueVueDTOToAlVueVue1( dto.getParent(), entity.getParent() );
        }
    ...
    @Override
    public AlVueVueConditionDTO toDto(AlVueVueCondition s) {
        if ( s == null ) {
            return null;
        }

        AlVueVueConditionDTO alVueVueConditionDTO = new AlVueVueConditionDTO();

        alVueVueConditionDTO.setParent( toDtoAlVueVueId( s.getParent() ) );
        alVueVueConditionDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alVueVueConditionDTO.setId( s.getId() );
        alVueVueConditionDTO.setSubjectType( s.getSubjectType() );
        alVueVueConditionDTO.setSubject( s.getSubject() );
        alVueVueConditionDTO.setAction( s.getAction() );
        alVueVueConditionDTO.setNote( s.getNote() );
