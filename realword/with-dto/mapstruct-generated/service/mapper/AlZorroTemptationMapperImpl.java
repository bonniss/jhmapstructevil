...
@Generated(
    ...
    date = "2024-10-22T00:23:18+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlZorroTemptationMapperImpl implements AlZorroTemptationMapper {

    @Override
    public AlZorroTemptation toEntity(AlZorroTemptationDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AlZorroTemptation alZorroTemptation = new AlZorroTemptation();

        alZorroTemptation.setId( dto.getId() );
        alZorroTemptation.setZipAction( dto.getZipAction() );
        alZorroTemptation.setName( dto.getName() );
        alZorroTemptation.setTemplateId( dto.getTemplateId() );
        alZorroTemptation.setDataSourceMappingType( dto.getDataSourceMappingType() );
        alZorroTemptation.setTemplateDataMapping( dto.getTemplateDataMapping() );
        alZorroTemptation.setTargetUrls( dto.getTargetUrls() );
        alZorroTemptation.thumbnail( metaverseDTOToMetaverse( dto.getThumbnail() ) );
        alZorroTemptation.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alZorroTemptation;
    }

    @Override
    public List<AlZorroTemptation> toEntity(List<AlZorroTemptationDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlZorroTemptation> list = new ArrayList<AlZorroTemptation>( dtoList.size() );
        for ( AlZorroTemptationDTO alZorroTemptationDTO : dtoList ) {
            list.add( toEntity( alZorroTemptationDTO ) );
        }
    ...
    @Override
    public List<AlZorroTemptationDTO> toDto(List<AlZorroTemptation> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlZorroTemptationDTO> list = new ArrayList<AlZorroTemptationDTO>( entityList.size() );
        for ( AlZorroTemptation alZorroTemptation : entityList ) {
            list.add( toDto( alZorroTemptation ) );
        }
    ...
    @Override
    public void partialUpdate(AlZorroTemptation entity, AlZorroTemptationDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getZipAction() != null ) {
            entity.setZipAction( dto.getZipAction() );
        }
        if ( dto.getName() != null ) {
        ...
        if ( dto.getTemplateDataMapping() != null ) {
            entity.setTemplateDataMapping( dto.getTemplateDataMapping() );
        }
    ...
    @Override
    public AlZorroTemptationDTO toDto(AlZorroTemptation s) {
        if ( s == null ) {
            return null;
        }

        AlZorroTemptationDTO alZorroTemptationDTO = new AlZorroTemptationDTO();

        alZorroTemptationDTO.setThumbnail( toDtoMetaverseId( s.getThumbnail() ) );
        alZorroTemptationDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alZorroTemptationDTO.setId( s.getId() );
        alZorroTemptationDTO.setZipAction( s.getZipAction() );
        alZorroTemptationDTO.setName( s.getName() );
        alZorroTemptationDTO.setTemplateId( s.getTemplateId() );
        alZorroTemptationDTO.setDataSourceMappingType( s.getDataSourceMappingType() );
        alZorroTemptationDTO.setTemplateDataMapping( s.getTemplateDataMapping() );
        alZorroTemptationDTO.setTargetUrls( s.getTargetUrls() );

        return alZorroTemptationDTO;
    }
