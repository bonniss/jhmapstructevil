...
@Generated(
    ...
    date = "2024-10-22T00:23:43+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlLadyGagaMapperImpl implements AlLadyGagaMapper {

    @Override
    public AlLadyGaga toEntity(AlLadyGagaDTO dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        alLadyGaga.setId( dto.getId() );
        alLadyGaga.setName( dto.getName() );
        alLadyGaga.setDescriptionHeitiga( dto.getDescriptionHeitiga() );
        alLadyGaga.address( andreiRightHandDTOToAndreiRightHand( dto.getAddress() ) );
        alLadyGaga.avatar( metaverseDTOToMetaverse( dto.getAvatar() ) );
        alLadyGaga.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alLadyGaga;
    }

    @Override
    public List<AlLadyGaga> toEntity(List<AlLadyGagaDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlLadyGaga> list = new ArrayList<AlLadyGaga>( dtoList.size() );
        for ( AlLadyGagaDTO alLadyGagaDTO : dtoList ) {
            list.add( toEntity( alLadyGagaDTO ) );
        }
    ...
    @Override
    public List<AlLadyGagaDTO> toDto(List<AlLadyGaga> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlLadyGagaDTO> list = new ArrayList<AlLadyGagaDTO>( entityList.size() );
        for ( AlLadyGaga alLadyGaga : entityList ) {
            list.add( toDto( alLadyGaga ) );
        }
    ...
    @Override
    public void partialUpdate(AlLadyGaga entity, AlLadyGagaDTO dto) {
        if ( dto == null ) {
            return;
        }
    ...
    @Override
    public AlLadyGagaDTO toDto(AlLadyGaga s) {
        if ( s == null ) {
            return null;
        }
        ...
        alLadyGagaDTO.setAddress( toDtoAndreiRightHandId( s.getAddress() ) );
        alLadyGagaDTO.setAvatar( toDtoMetaverseId( s.getAvatar() ) );
        alLadyGagaDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alLadyGagaDTO.setId( s.getId() );
        alLadyGagaDTO.setName( s.getName() );
        alLadyGagaDTO.setDescriptionHeitiga( s.getDescriptionHeitiga() );
    ...
    protected AlProPro alProProDTOToAlProPro(AlProProDTO alProProDTO) {
        ...
        alProPro.project( toEntity( alProProDTO.getProject() ) );
