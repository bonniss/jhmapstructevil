...
class AlGoreConditionResourceIT {
    ...
    void createAlGoreCondition() throws Exception {
        ...
        AlGoreConditionDTO alGoreConditionDTO = alGoreConditionMapper.toDto(alGoreCondition);
        var returnedAlGoreConditionDTO = om.readValue(
            restAlGoreConditionMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreConditionDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlGoreConditionDTO.class
        );

        // Validate the AlGoreCondition in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlGoreCondition = alGoreConditionMapper.toEntity(returnedAlGoreConditionDTO);
    ...
    void putExistingAlGoreCondition() throws Exception {
        ...
        AlGoreConditionDTO alGoreConditionDTO = alGoreConditionMapper.toDto(updatedAlGoreCondition);

        restAlGoreConditionMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alGoreConditionDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alGoreConditionDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlGoreCondition() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreConditionMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alGoreConditionDTO)))
    ...
    void patchNonExistingAlGoreCondition() throws Exception {
        ...
        restAlGoreConditionMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alGoreConditionDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alGoreConditionDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlGoreCondition() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlGoreConditionMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alGoreConditionDTO)))
            .andExpect(status().isMethodNotAllowed());
