...
class SicilyUmetoResourceIT {
    ...
    void createSicilyUmeto() throws Exception {
        ...
        SicilyUmetoDTO sicilyUmetoDTO = sicilyUmetoMapper.toDto(sicilyUmeto);
        var returnedSicilyUmetoDTO = om.readValue(
            restSicilyUmetoMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(sicilyUmetoDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            SicilyUmetoDTO.class
        );

        // Validate the SicilyUmeto in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedSicilyUmeto = sicilyUmetoMapper.toEntity(returnedSicilyUmetoDTO);
    ...
    void putExistingSicilyUmeto() throws Exception {
        ...
        SicilyUmetoDTO sicilyUmetoDTO = sicilyUmetoMapper.toDto(updatedSicilyUmeto);

        restSicilyUmetoMockMvc
            .perform(
                put(ENTITY_API_URL_ID, sicilyUmetoDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(sicilyUmetoDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamSicilyUmeto() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSicilyUmetoMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(sicilyUmetoDTO)))
    ...
    void patchNonExistingSicilyUmeto() throws Exception {
        ...
        restSicilyUmetoMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, sicilyUmetoDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(sicilyUmetoDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSicilyUmeto() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSicilyUmetoMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(sicilyUmetoDTO)))
            .andExpect(status().isMethodNotAllowed());
