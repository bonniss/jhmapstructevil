...
class HashRossResourceIT {
    ...
    void createHashRoss() throws Exception {
        ...
        // Create the HashRoss
        HashRossDTO hashRossDTO = hashRossMapper.toDto(hashRoss);
        var returnedHashRossDTO = om.readValue(
            restHashRossMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hashRossDTO)))
        ...
        var returnedHashRoss = hashRossMapper.toEntity(returnedHashRossDTO);
    ...
    void putExistingHashRoss() throws Exception {
        ...
        updatedHashRoss
            ...
            .description(UPDATED_DESCRIPTION)
            .permissionGridJason(UPDATED_PERMISSION_GRID_JASON);
        HashRossDTO hashRossDTO = hashRossMapper.toDto(updatedHashRoss);

        restHashRossMockMvc
            .perform(
                put(ENTITY_API_URL_ID, hashRossDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(hashRossDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamHashRoss() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHashRossMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(hashRossDTO)))
    ...
    void patchNonExistingHashRoss() throws Exception {
        ...
        restHashRossMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, hashRossDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(hashRossDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamHashRoss() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restHashRossMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(hashRossDTO)))
            .andExpect(status().isMethodNotAllowed());
