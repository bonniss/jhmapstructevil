...
class AlPowerShellResourceIT {
    ...
    void createAlPowerShell() throws Exception {
        ...
        AlPowerShellDTO alPowerShellDTO = alPowerShellMapper.toDto(alPowerShell);
        var returnedAlPowerShellDTO = om.readValue(
            restAlPowerShellMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPowerShellDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlPowerShellDTO.class
        );

        // Validate the AlPowerShell in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlPowerShell = alPowerShellMapper.toEntity(returnedAlPowerShellDTO);
    ...
    void putExistingAlPowerShell() throws Exception {
        ...
        AlPowerShellDTO alPowerShellDTO = alPowerShellMapper.toDto(updatedAlPowerShell);

        restAlPowerShellMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPowerShellDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alPowerShellDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlPowerShell() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPowerShellMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPowerShellDTO)))
    ...
    void patchNonExistingAlPowerShell() throws Exception {
        ...
        restAlPowerShellMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPowerShellDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPowerShellDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPowerShell() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPowerShellMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPowerShellDTO)))
            .andExpect(status().isMethodNotAllowed());
