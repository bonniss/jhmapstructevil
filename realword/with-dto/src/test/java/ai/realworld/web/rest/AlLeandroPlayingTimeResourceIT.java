...
class AlLeandroPlayingTimeResourceIT {
    ...
    void createAlLeandroPlayingTime() throws Exception {
        ...
        AlLeandroPlayingTimeDTO alLeandroPlayingTimeDTO = alLeandroPlayingTimeMapper.toDto(alLeandroPlayingTime);
        var returnedAlLeandroPlayingTimeDTO = om.readValue(
            restAlLeandroPlayingTimeMockMvc
                .perform(
                    post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLeandroPlayingTimeDTO))
                )
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AlLeandroPlayingTimeDTO.class
        );

        // Validate the AlLeandroPlayingTime in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAlLeandroPlayingTime = alLeandroPlayingTimeMapper.toEntity(returnedAlLeandroPlayingTimeDTO);
    ...
    void createAlLeandroPlayingTimeWithExistingId() throws Exception {
        ...
        restAlLeandroPlayingTimeMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLeandroPlayingTimeDTO)))
    ...
    void putExistingAlLeandroPlayingTime() throws Exception {
        ...
        updatedAlLeandroPlayingTime
            ...
            .playerReceivedTheAwardAt(UPDATED_PLAYER_RECEIVED_THE_AWARD_AT)
            .playSourceTime(UPDATED_PLAY_SOURCE_TIME);
        AlLeandroPlayingTimeDTO alLeandroPlayingTimeDTO = alLeandroPlayingTimeMapper.toDto(updatedAlLeandroPlayingTime);

        restAlLeandroPlayingTimeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alLeandroPlayingTimeDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alLeandroPlayingTimeDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlLeandroPlayingTime() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlLeandroPlayingTimeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLeandroPlayingTimeDTO)))
    ...
    void patchNonExistingAlLeandroPlayingTime() throws Exception {
        ...
        restAlLeandroPlayingTimeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alLeandroPlayingTimeDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alLeandroPlayingTimeDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlLeandroPlayingTime() throws Exception {
        ...
        restAlLeandroPlayingTimeMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alLeandroPlayingTimeDTO))
            )
            .andExpect(status().isMethodNotAllowed());
