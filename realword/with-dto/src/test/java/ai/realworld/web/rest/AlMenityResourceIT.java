...
class AlMenityResourceIT {
    ...
    void createAlMenity() throws Exception {
        ...
        // Create the AlMenity
        AlMenityDTO alMenityDTO = alMenityMapper.toDto(alMenity);
        var returnedAlMenityDTO = om.readValue(
            restAlMenityMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alMenityDTO)))
        ...
        var returnedAlMenity = alMenityMapper.toEntity(returnedAlMenityDTO);
    ...
    @Test
    @Transactional
    void getAllAlMenitiesByPropertyProfileIsEqualToSomething() throws Exception {
        ...
        em.flush();
        alMenity.addPropertyProfile(propertyProfile);
        alMenityRepository.saveAndFlush(alMenity);
        UUID propertyProfileId = propertyProfile.getId();
        // Get all the alMenityList where propertyProfile equals to propertyProfileId
        defaultAlMenityShouldBeFound("propertyProfileId.equals=" + propertyProfileId);

        // Get all the alMenityList where propertyProfile equals to UUID.randomUUID()
        defaultAlMenityShouldNotBeFound("propertyProfileId.equals=" + UUID.randomUUID());
    }
    ...
    void putExistingAlMenity() throws Exception {
        ...
        AlMenityDTO alMenityDTO = alMenityMapper.toDto(updatedAlMenity);

        restAlMenityMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alMenityDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alMenityDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlMenity() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlMenityMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alMenityDTO)))
    ...
    void patchNonExistingAlMenity() throws Exception {
        ...
        restAlMenityMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alMenityDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alMenityDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlMenity() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlMenityMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alMenityDTO)))
            .andExpect(status().isMethodNotAllowed());
