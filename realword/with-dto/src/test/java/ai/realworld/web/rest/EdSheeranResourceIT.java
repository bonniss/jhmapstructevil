...
class EdSheeranResourceIT {
    ...
    void createEdSheeran() throws Exception {
        ...
        EdSheeranDTO edSheeranDTO = edSheeranMapper.toDto(edSheeran);
        var returnedEdSheeranDTO = om.readValue(
            restEdSheeranMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(edSheeranDTO)))
        ...
        var returnedEdSheeran = edSheeranMapper.toEntity(returnedEdSheeranDTO);
    ...
    void putExistingEdSheeran() throws Exception {
        ...
        updatedEdSheeran
            ...
            .contactsJason(UPDATED_CONTACTS_JASON)
            .isEnabled(UPDATED_IS_ENABLED);
        EdSheeranDTO edSheeranDTO = edSheeranMapper.toDto(updatedEdSheeran);

        restEdSheeranMockMvc
            .perform(
                put(ENTITY_API_URL_ID, edSheeranDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(edSheeranDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamEdSheeran() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEdSheeranMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(edSheeranDTO)))
    ...
    void patchNonExistingEdSheeran() throws Exception {
        ...
        restEdSheeranMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, edSheeranDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(edSheeranDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEdSheeran() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEdSheeranMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(edSheeranDTO)))
            .andExpect(status().isMethodNotAllowed());
