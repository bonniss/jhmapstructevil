...
class AlDesireResourceIT {
    ...
    void createAlDesire() throws Exception {
        ...
        // Create the AlDesire
        AlDesireDTO alDesireDTO = alDesireMapper.toDto(alDesire);
        var returnedAlDesireDTO = om.readValue(
            restAlDesireMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alDesireDTO)))
        ...
        var returnedAlDesire = alDesireMapper.toEntity(returnedAlDesireDTO);
    ...
    void putExistingAlDesire() throws Exception {
        ...
        updatedAlDesire
            ...
            .awardReference(UPDATED_AWARD_REFERENCE)
            .isDefault(UPDATED_IS_DEFAULT);
        AlDesireDTO alDesireDTO = alDesireMapper.toDto(updatedAlDesire);

        restAlDesireMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alDesireDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alDesireDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithIdMismatchAlDesire() throws Exception {
        ...
        restAlDesireMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alDesireDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlDesire() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlDesireMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alDesireDTO)))
    ...
    void patchNonExistingAlDesire() throws Exception {
        ...
        restAlDesireMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alDesireDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alDesireDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlDesire() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlDesireMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alDesireDTO)))
            .andExpect(status().isMethodNotAllowed());
