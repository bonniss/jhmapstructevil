...
class AlLexFergResourceIT {
    ...
    @Mock
    private AlLexFergRepository alLexFergRepositoryMock;

    @Mock
    private AlLexFergService alLexFergServiceMock;
    ...
    void createAlLexFerg() throws Exception {
        ...
        var returnedAlLexFerg = om.readValue(
            restAlLexFergMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLexFerg)))
    ...
    void getAllAlLexFergsWithEagerRelationshipsIsEnabled() throws Exception {
        when(alLexFergServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAlLexFergMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(alLexFergServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllAlLexFergsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(alLexFergServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAlLexFergMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(alLexFergRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void getAllAlLexFergsByTagIsEqualToSomething() throws Exception {
        AlBestTooth tag;
        if (TestUtil.findAll(em, AlBestTooth.class).isEmpty()) {
            alLexFergRepository.saveAndFlush(alLexFerg);
            tag = AlBestToothResourceIT.createEntity();
        } else {
            tag = TestUtil.findAll(em, AlBestTooth.class).get(0);
        }
        em.persist(tag);
        em.flush();
        alLexFerg.addTag(tag);
        alLexFergRepository.saveAndFlush(alLexFerg);
        Long tagId = tag.getId();
        // Get all the alLexFergList where tag equals to tagId
        defaultAlLexFergShouldBeFound("tagId.equals=" + tagId);

        // Get all the alLexFergList where tag equals to (tagId + 1)
        defaultAlLexFergShouldNotBeFound("tagId.equals=" + (tagId + 1));
    }
    ...
    void putExistingAlLexFerg() throws Exception {
        ...
        restAlLexFergMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlLexFerg.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlLexFerg))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlLexFerg() throws Exception {
        ...
        restAlLexFergMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alLexFerg.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLexFerg))
            )
            .andExpect(status().isBadRequest());
        ...
        restAlLexFergMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alLexFerg))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlLexFerg() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlLexFergMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alLexFerg)))
    ...
    void patchNonExistingAlLexFerg() throws Exception {
        ...
        restAlLexFergMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alLexFerg.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alLexFerg))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlLexFerg() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlLexFergMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alLexFerg)))
            .andExpect(status().isMethodNotAllowed());
