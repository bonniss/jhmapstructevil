...
class MetaverseResourceIT {
    ...
    void createMetaverse() throws Exception {
        ...
        var returnedMetaverse = om.readValue(
            restMetaverseMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(metaverse)))
    ...
    void putExistingMetaverse() throws Exception {
        ...
        restMetaverseMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedMetaverse.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedMetaverse))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingMetaverse() throws Exception {
        ...
        restMetaverseMockMvc
            .perform(
                put(ENTITY_API_URL_ID, metaverse.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(metaverse))
            )
            .andExpect(status().isBadRequest());
        ...
        restMetaverseMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(metaverse))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamMetaverse() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMetaverseMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(metaverse)))
    ...
    void patchNonExistingMetaverse() throws Exception {
        ...
        restMetaverseMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, metaverse.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(metaverse))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamMetaverse() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMetaverseMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(metaverse)))
            .andExpect(status().isMethodNotAllowed());
