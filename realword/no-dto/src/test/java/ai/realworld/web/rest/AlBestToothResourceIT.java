...
class AlBestToothResourceIT {
    ...
    void createAlBestTooth() throws Exception {
        ...
        var returnedAlBestTooth = om.readValue(
            restAlBestToothMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alBestTooth)))
    ...
    @Test
    @Transactional
    void getAllAlBestToothsByArticleIsEqualToSomething() throws Exception {
        AlLexFerg article;
        if (TestUtil.findAll(em, AlLexFerg.class).isEmpty()) {
            alBestToothRepository.saveAndFlush(alBestTooth);
            article = AlLexFergResourceIT.createEntity();
        } else {
            article = TestUtil.findAll(em, AlLexFerg.class).get(0);
        }
        em.persist(article);
        em.flush();
        alBestTooth.addArticle(article);
        alBestToothRepository.saveAndFlush(alBestTooth);
        Long articleId = article.getId();
        // Get all the alBestToothList where article equals to articleId
        defaultAlBestToothShouldBeFound("articleId.equals=" + articleId);

        // Get all the alBestToothList where article equals to (articleId + 1)
        defaultAlBestToothShouldNotBeFound("articleId.equals=" + (articleId + 1));
    }
    ...
    void putExistingAlBestTooth() throws Exception {
        ...
        restAlBestToothMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlBestTooth.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlBestTooth))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlBestTooth() throws Exception {
        ...
        restAlBestToothMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alBestTooth.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alBestTooth))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlBestTooth() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlBestToothMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alBestTooth)))
    ...
    void patchNonExistingAlBestTooth() throws Exception {
        ...
        restAlBestToothMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alBestTooth.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alBestTooth))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlBestTooth() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlBestToothMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alBestTooth)))
            .andExpect(status().isMethodNotAllowed());
