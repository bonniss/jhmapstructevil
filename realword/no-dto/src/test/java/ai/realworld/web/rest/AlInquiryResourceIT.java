...
class AlInquiryResourceIT {
    ...
    void createAlInquiry() throws Exception {
        ...
        var returnedAlInquiry = om.readValue(
            restAlInquiryMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alInquiry)))
    ...
    void putExistingAlInquiry() throws Exception {
        ...
        restAlInquiryMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlInquiry.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlInquiry))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlInquiry() throws Exception {
        ...
        restAlInquiryMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alInquiry.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alInquiry))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithIdMismatchAlInquiry() throws Exception {
        ...
        restAlInquiryMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alInquiry))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlInquiry() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlInquiryMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alInquiry)))
    ...
    void patchNonExistingAlInquiry() throws Exception {
        ...
        restAlInquiryMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alInquiry.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alInquiry))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlInquiry() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlInquiryMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alInquiry)))
            .andExpect(status().isMethodNotAllowed());
