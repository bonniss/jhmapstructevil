...
@Generated(
    ...
    date = "2024-10-22T00:29:21+0700",
    comments = "version: 1.6.2, compiler: javac, environment: Java 17.0.11 (Eclipse Adoptium)"
)
@Component
public class AlDesireMapperImpl implements AlDesireMapper {

    @Override
    public AlDesire toEntity(AlDesireDTO dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        alDesire.setId( dto.getId() );
        alDesire.setName( dto.getName() );
        alDesire.setWeight( dto.getWeight() );
        alDesire.setProbabilityOfWinning( dto.getProbabilityOfWinning() );
        alDesire.setMaximumWinningTime( dto.getMaximumWinningTime() );
        alDesire.setIsWinningTimeLimited( dto.getIsWinningTimeLimited() );
        alDesire.setAwardResultType( dto.getAwardResultType() );
        alDesire.setAwardReference( dto.getAwardReference() );
        alDesire.setIsDefault( dto.getIsDefault() );
        alDesire.image( metaverseDTOToMetaverse( dto.getImage() ) );
        alDesire.maggi( alLeandroDTOToAlLeandro( dto.getMaggi() ) );
        alDesire.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alDesire;
    }

    @Override
    public List<AlDesire> toEntity(List<AlDesireDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlDesire> list = new ArrayList<AlDesire>( dtoList.size() );
        for ( AlDesireDTO alDesireDTO : dtoList ) {
            list.add( toEntity( alDesireDTO ) );
        }
    ...
    @Override
    public List<AlDesireDTO> toDto(List<AlDesire> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlDesireDTO> list = new ArrayList<AlDesireDTO>( entityList.size() );
        for ( AlDesire alDesire : entityList ) {
            list.add( toDto( alDesire ) );
        }
    ...
    @Override
    public void partialUpdate(AlDesire entity, AlDesireDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getProbabilityOfWinning() != null ) {
            entity.setProbabilityOfWinning( dto.getProbabilityOfWinning() );
        }
        if ( dto.getMaximumWinningTime() != null ) {
            entity.setMaximumWinningTime( dto.getMaximumWinningTime() );
        }
        if ( dto.getIsWinningTimeLimited() != null ) {
            entity.setIsWinningTimeLimited( dto.getIsWinningTimeLimited() );
        }
        if ( dto.getAwardResultType() != null ) {
            entity.setAwardResultType( dto.getAwardResultType() );
        }
        if ( dto.getAwardReference() != null ) {
            entity.setAwardReference( dto.getAwardReference() );
        }
    ...
    @Override
    public AlDesireDTO toDto(AlDesire s) {
        if ( s == null ) {
            return null;
        }
        ...
        alDesireDTO.setImage( toDtoMetaverseId( s.getImage() ) );
        alDesireDTO.setMaggi( toDtoAlLeandroId( s.getMaggi() ) );
        alDesireDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alDesireDTO.setId( s.getId() );
        alDesireDTO.setName( s.getName() );
        alDesireDTO.setWeight( s.getWeight() );
        alDesireDTO.setProbabilityOfWinning( s.getProbabilityOfWinning() );
        alDesireDTO.setMaximumWinningTime( s.getMaximumWinningTime() );
        alDesireDTO.setIsWinningTimeLimited( s.getIsWinningTimeLimited() );
        alDesireDTO.setAwardResultType( s.getAwardResultType() );
        alDesireDTO.setAwardReference( s.getAwardReference() );
        alDesireDTO.setIsDefault( s.getIsDefault() );
