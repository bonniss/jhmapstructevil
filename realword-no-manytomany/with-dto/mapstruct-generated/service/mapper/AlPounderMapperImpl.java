...
@Component
public class AlPounderMapperImpl implements AlPounderMapper {

    @Override
    public AlPounder toEntity(AlPounderDTO dto) {
        if ( dto == null ) {
            return null;
        }
        ...
        alPounder.setId( dto.getId() );
        alPounder.setName( dto.getName() );
        alPounder.setWeight( dto.getWeight() );
        alPounder.attributeTaxonomy( alPedroTaxDTOToAlPedroTax( dto.getAttributeTaxonomy() ) );
        alPounder.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return alPounder;
    }

    @Override
    public List<AlPounder> toEntity(List<AlPounderDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AlPounder> list = new ArrayList<AlPounder>( dtoList.size() );
        for ( AlPounderDTO alPounderDTO : dtoList ) {
            list.add( toEntity( alPounderDTO ) );
        }
    ...
    @Override
    public List<AlPounderDTO> toDto(List<AlPounder> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AlPounderDTO> list = new ArrayList<AlPounderDTO>( entityList.size() );
        for ( AlPounder alPounder : entityList ) {
            list.add( toDto( alPounder ) );
        }
    ...
    @Override
    public void partialUpdate(AlPounder entity, AlPounderDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getAttributeTaxonomy() != null ) {
            if ( entity.getAttributeTaxonomy() == null ) {
                entity.attributeTaxonomy( new AlPedroTax() );
            }
            alPedroTaxDTOToAlPedroTax1( dto.getAttributeTaxonomy(), entity.getAttributeTaxonomy() );
        }
    ...
    @Override
    public AlPounderDTO toDto(AlPounder s) {
        if ( s == null ) {
            return null;
        }
        ...
        alPounderDTO.setAttributeTaxonomy( toDtoAlPedroTaxId( s.getAttributeTaxonomy() ) );
        alPounderDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        alPounderDTO.setId( s.getId() );
        alPounderDTO.setName( s.getName() );
        alPounderDTO.setWeight( s.getWeight() );

        return alPounderDTO;
    }

    @Override
    public AlPedroTaxDTO toDtoAlPedroTaxId(AlPedroTax alPedroTax) {
        if ( alPedroTax == null ) {
            return null;
        }

        AlPedroTaxDTO alPedroTaxDTO = new AlPedroTaxDTO();

        alPedroTaxDTO.setId( alPedroTax.getId() );

        return alPedroTaxDTO;
    }
