...
@Component
public class AppZnsTemplateMapperImpl implements AppZnsTemplateMapper {

    @Override
    public AppZnsTemplate toEntity(AppZnsTemplateDTO dto) {
        if ( dto == null ) {
            return null;
        }

        AppZnsTemplate appZnsTemplate = new AppZnsTemplate();

        appZnsTemplate.setId( dto.getId() );
        appZnsTemplate.setZnsAction( dto.getZnsAction() );
        appZnsTemplate.setName( dto.getName() );
        appZnsTemplate.setTemplateId( dto.getTemplateId() );
        appZnsTemplate.setDataSourceMappingType( dto.getDataSourceMappingType() );
        appZnsTemplate.setTemplateDataMapping( dto.getTemplateDataMapping() );
        appZnsTemplate.setTargetUrls( dto.getTargetUrls() );
        appZnsTemplate.thumbnail( metaverseDTOToMetaverse( dto.getThumbnail() ) );
        appZnsTemplate.application( johnLennonDTOToJohnLennon( dto.getApplication() ) );

        return appZnsTemplate;
    }

    @Override
    public List<AppZnsTemplate> toEntity(List<AppZnsTemplateDTO> dtoList) {
        if ( dtoList == null ) {
            return null;
        }

        List<AppZnsTemplate> list = new ArrayList<AppZnsTemplate>( dtoList.size() );
        for ( AppZnsTemplateDTO appZnsTemplateDTO : dtoList ) {
            list.add( toEntity( appZnsTemplateDTO ) );
        }
    ...
    @Override
    public List<AppZnsTemplateDTO> toDto(List<AppZnsTemplate> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<AppZnsTemplateDTO> list = new ArrayList<AppZnsTemplateDTO>( entityList.size() );
        for ( AppZnsTemplate appZnsTemplate : entityList ) {
            list.add( toDto( appZnsTemplate ) );
        }
    ...
    @Override
    public void partialUpdate(AppZnsTemplate entity, AppZnsTemplateDTO dto) {
        if ( dto == null ) {
            return;
        }
        ...
        if ( dto.getZnsAction() != null ) {
            entity.setZnsAction( dto.getZnsAction() );
        }
        if ( dto.getName() != null ) {
    ...
    @Override
    public AppZnsTemplateDTO toDto(AppZnsTemplate s) {
        if ( s == null ) {
            return null;
        }

        AppZnsTemplateDTO appZnsTemplateDTO = new AppZnsTemplateDTO();

        appZnsTemplateDTO.setThumbnail( toDtoMetaverseId( s.getThumbnail() ) );
        appZnsTemplateDTO.setApplication( toDtoJohnLennonId( s.getApplication() ) );
        appZnsTemplateDTO.setId( s.getId() );
        appZnsTemplateDTO.setZnsAction( s.getZnsAction() );
        appZnsTemplateDTO.setName( s.getName() );
        appZnsTemplateDTO.setTemplateId( s.getTemplateId() );
        appZnsTemplateDTO.setDataSourceMappingType( s.getDataSourceMappingType() );
        appZnsTemplateDTO.setTemplateDataMapping( s.getTemplateDataMapping() );
        appZnsTemplateDTO.setTargetUrls( s.getTargetUrls() );
