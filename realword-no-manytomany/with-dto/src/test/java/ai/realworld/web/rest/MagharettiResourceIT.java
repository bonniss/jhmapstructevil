...
class MagharettiResourceIT {
    ...
    void createMagharetti() throws Exception {
        ...
        MagharettiDTO magharettiDTO = magharettiMapper.toDto(magharetti);
        var returnedMagharettiDTO = om.readValue(
            restMagharettiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(magharettiDTO)))
        ...
        var returnedMagharetti = magharettiMapper.toEntity(returnedMagharettiDTO);
    ...
    void putExistingMagharetti() throws Exception {
        ...
        MagharettiDTO magharettiDTO = magharettiMapper.toDto(updatedMagharetti);

        restMagharettiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, magharettiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(magharettiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamMagharetti() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMagharettiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(magharettiDTO)))
    ...
    void patchNonExistingMagharetti() throws Exception {
        ...
        restMagharettiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, magharettiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(magharettiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamMagharetti() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMagharettiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(magharettiDTO)))
            .andExpect(status().isMethodNotAllowed());
