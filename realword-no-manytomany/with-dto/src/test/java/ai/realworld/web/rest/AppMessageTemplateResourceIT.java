...
class AppMessageTemplateResourceIT {
    ...
    void createAppMessageTemplate() throws Exception {
        ...
        AppMessageTemplateDTO appMessageTemplateDTO = appMessageTemplateMapper.toDto(appMessageTemplate);
        var returnedAppMessageTemplateDTO = om.readValue(
            restAppMessageTemplateMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(appMessageTemplateDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            AppMessageTemplateDTO.class
        );

        // Validate the AppMessageTemplate in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedAppMessageTemplate = appMessageTemplateMapper.toEntity(returnedAppMessageTemplateDTO);
    ...
    void putExistingAppMessageTemplate() throws Exception {
        ...
        updatedAppMessageTemplate
            ...
            .dataSourceMappingType(UPDATED_DATA_SOURCE_MAPPING_TYPE)
            .targetUrls(UPDATED_TARGET_URLS);
        AppMessageTemplateDTO appMessageTemplateDTO = appMessageTemplateMapper.toDto(updatedAppMessageTemplate);

        restAppMessageTemplateMockMvc
            .perform(
                put(ENTITY_API_URL_ID, appMessageTemplateDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(appMessageTemplateDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAppMessageTemplate() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAppMessageTemplateMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(appMessageTemplateDTO)))
    ...
    void patchNonExistingAppMessageTemplate() throws Exception {
        ...
        restAppMessageTemplateMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, appMessageTemplateDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(appMessageTemplateDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAppMessageTemplate() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAppMessageTemplateMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(appMessageTemplateDTO)))
            .andExpect(status().isMethodNotAllowed());
