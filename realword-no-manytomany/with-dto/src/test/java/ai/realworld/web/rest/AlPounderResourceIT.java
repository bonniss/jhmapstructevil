...
class AlPounderResourceIT {
    ...
    void createAlPounder() throws Exception {
        ...
        AlPounderDTO alPounderDTO = alPounderMapper.toDto(alPounder);
        var returnedAlPounderDTO = om.readValue(
            restAlPounderMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPounderDTO)))
        ...
        var returnedAlPounder = alPounderMapper.toEntity(returnedAlPounderDTO);
    ...
    void putExistingAlPounder() throws Exception {
        ...
        AlPounderDTO alPounderDTO = alPounderMapper.toDto(updatedAlPounder);

        restAlPounderMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alPounderDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alPounderDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamAlPounder() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPounderMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alPounderDTO)))
    ...
    void patchNonExistingAlPounder() throws Exception {
        ...
        restAlPounderMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alPounderDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alPounderDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlPounder() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlPounderMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alPounderDTO)))
            .andExpect(status().isMethodNotAllowed());
