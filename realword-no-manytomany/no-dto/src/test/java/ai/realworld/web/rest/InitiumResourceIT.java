...
class InitiumResourceIT {
    ...
    void createInitium() throws Exception {
        ...
        // Create the Initium
        var returnedInitium = om.readValue(
            restInitiumMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(initium)))
    ...
    void putExistingInitium() throws Exception {
        ...
        restInitiumMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedInitium.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedInitium))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingInitium() throws Exception {
        ...
        restInitiumMockMvc
            .perform(put(ENTITY_API_URL_ID, initium.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(initium)))
            .andExpect(status().isBadRequest());
        ...
        restInitiumMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(initium))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamInitium() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInitiumMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(initium)))
    ...
    void patchNonExistingInitium() throws Exception {
        ...
        restInitiumMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, initium.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(initium))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamInitium() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInitiumMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(initium)))
            .andExpect(status().isMethodNotAllowed());
