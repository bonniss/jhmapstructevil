...
class AlVueVueUsageResourceIT {
    ...
    void createAlVueVueUsage() throws Exception {
        ...
        var returnedAlVueVueUsage = om.readValue(
            restAlVueVueUsageMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alVueVueUsage)))
    ...
    void putExistingAlVueVueUsage() throws Exception {
        ...
        restAlVueVueUsageMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedAlVueVueUsage.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedAlVueVueUsage))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingAlVueVueUsage() throws Exception {
        ...
        restAlVueVueUsageMockMvc
            .perform(
                put(ENTITY_API_URL_ID, alVueVueUsage.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(alVueVueUsage))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAlVueVueUsage() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlVueVueUsageMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(alVueVueUsage)))
    ...
    void patchNonExistingAlVueVueUsage() throws Exception {
        ...
        restAlVueVueUsageMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, alVueVueUsage.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(alVueVueUsage))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAlVueVueUsage() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAlVueVueUsageMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(alVueVueUsage)))
            .andExpect(status().isMethodNotAllowed());
