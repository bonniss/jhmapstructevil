...
class RihannaResourceIT {
    ...
    void createRihanna() throws Exception {
        ...
        // Create the Rihanna
        var returnedRihanna = om.readValue(
            restRihannaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(rihanna)))
    ...
    void putExistingRihanna() throws Exception {
        ...
        restRihannaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedRihanna.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedRihanna))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingRihanna() throws Exception {
        ...
        restRihannaMockMvc
            .perform(put(ENTITY_API_URL_ID, rihanna.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(rihanna)))
            .andExpect(status().isBadRequest());
        ...
        restRihannaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(rihanna))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamRihanna() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restRihannaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(rihanna)))
    ...
    void patchNonExistingRihanna() throws Exception {
        ...
        restRihannaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, rihanna.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(rihanna))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamRihanna() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restRihannaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(rihanna)))
            .andExpect(status().isMethodNotAllowed());
