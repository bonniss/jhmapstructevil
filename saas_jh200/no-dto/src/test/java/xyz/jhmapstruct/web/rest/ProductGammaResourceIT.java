...
class ProductGammaResourceIT {
    ...
    void createProductGamma() throws Exception {
        ...
        var returnedProductGamma = om.readValue(
            restProductGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productGamma)))
    ...
    void putExistingProductGamma() throws Exception {
        ...
        restProductGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedProductGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedProductGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingProductGamma() throws Exception {
        ...
        restProductGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, productGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(productGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamProductGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productGamma)))
    ...
    void patchNonExistingProductGamma() throws Exception {
        ...
        restProductGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, productGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(productGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamProductGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(productGamma)))
            .andExpect(status().isMethodNotAllowed());
