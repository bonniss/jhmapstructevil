...
class NextSupplierResourceIT {
    ...
    void createNextSupplier() throws Exception {
        ...
        var returnedNextSupplier = om.readValue(
            restNextSupplierMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplier)))
    ...
    void putExistingNextSupplier() throws Exception {
        ...
        restNextSupplierMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextSupplier.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextSupplier))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextSupplier() throws Exception {
        ...
        restNextSupplierMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextSupplier.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextSupplier))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextSupplier() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplier)))
    ...
    void patchNonExistingNextSupplier() throws Exception {
        ...
        restNextSupplierMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextSupplier.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextSupplier))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextSupplier() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextSupplier)))
            .andExpect(status().isMethodNotAllowed());
