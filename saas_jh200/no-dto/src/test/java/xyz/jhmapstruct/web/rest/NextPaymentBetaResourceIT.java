...
class NextPaymentBetaResourceIT {
    ...
    void createNextPaymentBeta() throws Exception {
        ...
        var returnedNextPaymentBeta = om.readValue(
            restNextPaymentBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentBeta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentBeta.class
        );
    ...
    void putExistingNextPaymentBeta() throws Exception {
        ...
        restNextPaymentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextPaymentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextPaymentBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextPaymentBeta() throws Exception {
        ...
        restNextPaymentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextPaymentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentBeta)))
    ...
    void patchNonExistingNextPaymentBeta() throws Exception {
        ...
        restNextPaymentBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentBeta)))
            .andExpect(status().isMethodNotAllowed());
