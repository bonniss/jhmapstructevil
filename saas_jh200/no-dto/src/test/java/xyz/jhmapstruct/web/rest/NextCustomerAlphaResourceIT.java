...
class NextCustomerAlphaResourceIT {
    ...
    void createNextCustomerAlpha() throws Exception {
        ...
        var returnedNextCustomerAlpha = om.readValue(
            restNextCustomerAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerAlpha)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerAlpha.class
        );
    ...
    void putExistingNextCustomerAlpha() throws Exception {
        ...
        restNextCustomerAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextCustomerAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextCustomerAlpha))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextCustomerAlpha() throws Exception {
        ...
        restNextCustomerAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextCustomerAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerAlpha)))
    ...
    void patchNonExistingNextCustomerAlpha() throws Exception {
        ...
        restNextCustomerAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerAlpha.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerAlpha)))
            .andExpect(status().isMethodNotAllowed());
