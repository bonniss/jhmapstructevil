...
class ReviewBetaResourceIT {
    ...
    void createReviewBeta() throws Exception {
        ...
        var returnedReviewBeta = om.readValue(
            restReviewBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewBeta)))
    ...
    void putExistingReviewBeta() throws Exception {
        ...
        restReviewBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedReviewBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedReviewBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingReviewBeta() throws Exception {
        ...
        restReviewBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, reviewBeta.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewBeta))
            )
            .andExpect(status().isBadRequest());
        ...
        restReviewBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(reviewBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamReviewBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewBeta)))
    ...
    void patchNonExistingReviewBeta() throws Exception {
        ...
        restReviewBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, reviewBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(reviewBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamReviewBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(reviewBeta)))
            .andExpect(status().isMethodNotAllowed());
