...
class EmployeeThetaResourceIT {
    ...
    void createEmployeeTheta() throws Exception {
        ...
        var returnedEmployeeTheta = om.readValue(
            restEmployeeThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeTheta)))
    ...
    void putExistingEmployeeTheta() throws Exception {
        ...
        restEmployeeThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedEmployeeTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedEmployeeTheta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingEmployeeTheta() throws Exception {
        ...
        restEmployeeThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, employeeTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(employeeTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamEmployeeTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeTheta)))
    ...
    void patchNonExistingEmployeeTheta() throws Exception {
        ...
        restEmployeeThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, employeeTheta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(employeeTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEmployeeTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(employeeTheta)))
            .andExpect(status().isMethodNotAllowed());
