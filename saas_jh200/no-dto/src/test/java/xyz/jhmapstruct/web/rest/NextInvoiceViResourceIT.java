...
class NextInvoiceViResourceIT {
    ...
    void createNextInvoiceVi() throws Exception {
        ...
        var returnedNextInvoiceVi = om.readValue(
            restNextInvoiceViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceVi)))
    ...
    void putExistingNextInvoiceVi() throws Exception {
        ...
        restNextInvoiceViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoiceVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoiceVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoiceVi() throws Exception {
        ...
        restNextInvoiceViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoiceVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceVi)))
    ...
    void patchNonExistingNextInvoiceVi() throws Exception {
        ...
        restNextInvoiceViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceVi)))
            .andExpect(status().isMethodNotAllowed());
