...
class InvoiceSigmaResourceIT {
    ...
    void createInvoiceSigma() throws Exception {
        ...
        var returnedInvoiceSigma = om.readValue(
            restInvoiceSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceSigma)))
    ...
    void putExistingInvoiceSigma() throws Exception {
        ...
        restInvoiceSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedInvoiceSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedInvoiceSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingInvoiceSigma() throws Exception {
        ...
        restInvoiceSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, invoiceSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(invoiceSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamInvoiceSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceSigma)))
    ...
    void patchNonExistingInvoiceSigma() throws Exception {
        ...
        restInvoiceSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, invoiceSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(invoiceSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamInvoiceSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(invoiceSigma)))
            .andExpect(status().isMethodNotAllowed());
