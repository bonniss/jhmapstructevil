...
class NextProductSigmaResourceIT {
    ...
    void createNextProductSigma() throws Exception {
        ...
        var returnedNextProductSigma = om.readValue(
            restNextProductSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductSigma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextProductSigma.class
        );
    ...
    void putExistingNextProductSigma() throws Exception {
        ...
        restNextProductSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextProductSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextProductSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextProductSigma() throws Exception {
        ...
        restNextProductSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextProductSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextProductSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextProductSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductSigma)))
    ...
    void patchNonExistingNextProductSigma() throws Exception {
        ...
        restNextProductSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextProductSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextProductSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextProductSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextProductSigma)))
            .andExpect(status().isMethodNotAllowed());
