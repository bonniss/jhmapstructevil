...
class NextOrderGammaResourceIT {
    ...
    void createNextOrderGamma() throws Exception {
        ...
        var returnedNextOrderGamma = om.readValue(
            restNextOrderGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderGamma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextOrderGamma.class
        );
    ...
    void putExistingNextOrderGamma() throws Exception {
        ...
        restNextOrderGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextOrderGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextOrderGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextOrderGamma() throws Exception {
        ...
        restNextOrderGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextOrderGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderGamma)))
    ...
    void patchNonExistingNextOrderGamma() throws Exception {
        ...
        restNextOrderGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderGamma)))
            .andExpect(status().isMethodNotAllowed());
