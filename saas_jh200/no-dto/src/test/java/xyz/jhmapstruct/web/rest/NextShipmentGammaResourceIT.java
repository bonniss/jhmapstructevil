...
class NextShipmentGammaResourceIT {
    ...
    void createNextShipmentGamma() throws Exception {
        ...
        var returnedNextShipmentGamma = om.readValue(
            restNextShipmentGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentGamma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentGamma.class
        );
    ...
    void putExistingNextShipmentGamma() throws Exception {
        ...
        restNextShipmentGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextShipmentGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextShipmentGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextShipmentGamma() throws Exception {
        ...
        restNextShipmentGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextShipmentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentGamma)))
    ...
    void patchNonExistingNextShipmentGamma() throws Exception {
        ...
        restNextShipmentGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipmentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentGamma)))
            .andExpect(status().isMethodNotAllowed());
