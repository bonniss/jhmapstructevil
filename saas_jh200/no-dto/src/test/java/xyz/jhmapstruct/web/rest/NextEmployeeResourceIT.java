...
class NextEmployeeResourceIT {
    ...
    void createNextEmployee() throws Exception {
        ...
        var returnedNextEmployee = om.readValue(
            restNextEmployeeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployee)))
    ...
    void putExistingNextEmployee() throws Exception {
        ...
        restNextEmployeeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextEmployee.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextEmployee))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextEmployee() throws Exception {
        ...
        restNextEmployeeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextEmployee.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextEmployee))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextEmployee() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployee)))
    ...
    void patchNonExistingNextEmployee() throws Exception {
        ...
        restNextEmployeeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextEmployee.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextEmployee))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextEmployee() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextEmployee)))
            .andExpect(status().isMethodNotAllowed());
