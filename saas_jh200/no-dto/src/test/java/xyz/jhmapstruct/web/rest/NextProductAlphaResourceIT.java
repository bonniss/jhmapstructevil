...
class NextProductAlphaResourceIT {
    ...
    void createNextProductAlpha() throws Exception {
        ...
        var returnedNextProductAlpha = om.readValue(
            restNextProductAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductAlpha)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextProductAlpha.class
        );
    ...
    void putExistingNextProductAlpha() throws Exception {
        ...
        restNextProductAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextProductAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextProductAlpha))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextProductAlpha() throws Exception {
        ...
        restNextProductAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextProductAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextProductAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextProductAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductAlpha)))
    ...
    void patchNonExistingNextProductAlpha() throws Exception {
        ...
        restNextProductAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextProductAlpha.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextProductAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextProductAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextProductAlpha)))
            .andExpect(status().isMethodNotAllowed());
