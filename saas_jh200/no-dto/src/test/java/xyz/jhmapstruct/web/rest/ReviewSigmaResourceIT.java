...
class ReviewSigmaResourceIT {
    ...
    void createReviewSigma() throws Exception {
        ...
        var returnedReviewSigma = om.readValue(
            restReviewSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewSigma)))
    ...
    void putExistingReviewSigma() throws Exception {
        ...
        restReviewSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedReviewSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedReviewSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingReviewSigma() throws Exception {
        ...
        restReviewSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, reviewSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(reviewSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamReviewSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewSigma)))
    ...
    void patchNonExistingReviewSigma() throws Exception {
        ...
        restReviewSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, reviewSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(reviewSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamReviewSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(reviewSigma)))
            .andExpect(status().isMethodNotAllowed());
