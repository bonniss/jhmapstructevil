...
class NextInvoiceSigmaResourceIT {
    ...
    void createNextInvoiceSigma() throws Exception {
        ...
        var returnedNextInvoiceSigma = om.readValue(
            restNextInvoiceSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceSigma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextInvoiceSigma.class
        );
    ...
    void putExistingNextInvoiceSigma() throws Exception {
        ...
        restNextInvoiceSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoiceSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoiceSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoiceSigma() throws Exception {
        ...
        restNextInvoiceSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoiceSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceSigma)))
    ...
    void patchNonExistingNextInvoiceSigma() throws Exception {
        ...
        restNextInvoiceSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceSigma)))
            .andExpect(status().isMethodNotAllowed());
