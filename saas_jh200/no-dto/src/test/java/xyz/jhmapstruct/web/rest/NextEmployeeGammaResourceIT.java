...
class NextEmployeeGammaResourceIT {
    ...
    void createNextEmployeeGamma() throws Exception {
        ...
        var returnedNextEmployeeGamma = om.readValue(
            restNextEmployeeGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeGamma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextEmployeeGamma.class
        );
    ...
    void putExistingNextEmployeeGamma() throws Exception {
        ...
        restNextEmployeeGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextEmployeeGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextEmployeeGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextEmployeeGamma() throws Exception {
        ...
        restNextEmployeeGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextEmployeeGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextEmployeeGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextEmployeeGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeGamma)))
    ...
    void patchNonExistingNextEmployeeGamma() throws Exception {
        ...
        restNextEmployeeGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextEmployeeGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextEmployeeGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextEmployeeGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextEmployeeGamma)))
            .andExpect(status().isMethodNotAllowed());
