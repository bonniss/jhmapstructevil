...
class NextCustomerMiResourceIT {
    ...
    void createNextCustomerMi() throws Exception {
        ...
        var returnedNextCustomerMi = om.readValue(
            restNextCustomerMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerMi)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerMi.class
        );
    ...
    void putExistingNextCustomerMi() throws Exception {
        ...
        restNextCustomerMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextCustomerMi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextCustomerMi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextCustomerMi() throws Exception {
        ...
        restNextCustomerMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerMi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerMi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextCustomerMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerMi)))
    ...
    void patchNonExistingNextCustomerMi() throws Exception {
        ...
        restNextCustomerMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerMi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerMi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerMi)))
            .andExpect(status().isMethodNotAllowed());
