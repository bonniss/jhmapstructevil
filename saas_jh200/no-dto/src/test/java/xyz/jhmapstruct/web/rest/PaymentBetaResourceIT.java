...
class PaymentBetaResourceIT {
    ...
    void createPaymentBeta() throws Exception {
        ...
        var returnedPaymentBeta = om.readValue(
            restPaymentBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentBeta)))
    ...
    void putExistingPaymentBeta() throws Exception {
        ...
        restPaymentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedPaymentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedPaymentBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingPaymentBeta() throws Exception {
        ...
        restPaymentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paymentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(paymentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamPaymentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentBeta)))
    ...
    void patchNonExistingPaymentBeta() throws Exception {
        ...
        restPaymentBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paymentBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(paymentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaymentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(paymentBeta)))
            .andExpect(status().isMethodNotAllowed());
