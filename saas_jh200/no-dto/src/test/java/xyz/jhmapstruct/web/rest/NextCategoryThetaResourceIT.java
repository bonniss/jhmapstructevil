...
class NextCategoryThetaResourceIT {
    ...
    void createNextCategoryTheta() throws Exception {
        ...
        var returnedNextCategoryTheta = om.readValue(
            restNextCategoryThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryTheta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCategoryTheta.class
        );
    ...
    void putExistingNextCategoryTheta() throws Exception {
        ...
        restNextCategoryThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextCategoryTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextCategoryTheta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextCategoryTheta() throws Exception {
        ...
        restNextCategoryThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCategoryTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCategoryTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextCategoryTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryTheta)))
    ...
    void patchNonExistingNextCategoryTheta() throws Exception {
        ...
        restNextCategoryThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCategoryTheta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCategoryTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCategoryTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCategoryTheta)))
            .andExpect(status().isMethodNotAllowed());
