...
class NextPaymentSigmaResourceIT {
    ...
    void createNextPaymentSigma() throws Exception {
        ...
        var returnedNextPaymentSigma = om.readValue(
            restNextPaymentSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentSigma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentSigma.class
        );
    ...
    void putExistingNextPaymentSigma() throws Exception {
        ...
        restNextPaymentSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextPaymentSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextPaymentSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextPaymentSigma() throws Exception {
        ...
        restNextPaymentSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextPaymentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentSigma)))
    ...
    void patchNonExistingNextPaymentSigma() throws Exception {
        ...
        restNextPaymentSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentSigma)))
            .andExpect(status().isMethodNotAllowed());
