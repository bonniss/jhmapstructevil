...
class NextShipmentMiResourceIT {
    ...
    void createNextShipmentMi() throws Exception {
        ...
        var returnedNextShipmentMi = om.readValue(
            restNextShipmentMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentMi)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentMi.class
        );
    ...
    void putExistingNextShipmentMi() throws Exception {
        ...
        restNextShipmentMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextShipmentMi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextShipmentMi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextShipmentMi() throws Exception {
        ...
        restNextShipmentMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentMi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentMi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextShipmentMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentMi)))
    ...
    void patchNonExistingNextShipmentMi() throws Exception {
        ...
        restNextShipmentMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentMi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentMi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipmentMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentMi)))
            .andExpect(status().isMethodNotAllowed());
