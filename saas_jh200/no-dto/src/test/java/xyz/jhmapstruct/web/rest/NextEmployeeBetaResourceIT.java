...
class NextEmployeeBetaResourceIT {
    ...
    void createNextEmployeeBeta() throws Exception {
        ...
        var returnedNextEmployeeBeta = om.readValue(
            restNextEmployeeBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeBeta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextEmployeeBeta.class
        );
    ...
    void putExistingNextEmployeeBeta() throws Exception {
        ...
        restNextEmployeeBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextEmployeeBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextEmployeeBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextEmployeeBeta() throws Exception {
        ...
        restNextEmployeeBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextEmployeeBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextEmployeeBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextEmployeeBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeBeta)))
    ...
    void patchNonExistingNextEmployeeBeta() throws Exception {
        ...
        restNextEmployeeBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextEmployeeBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextEmployeeBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextEmployeeBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextEmployeeBeta)))
            .andExpect(status().isMethodNotAllowed());
