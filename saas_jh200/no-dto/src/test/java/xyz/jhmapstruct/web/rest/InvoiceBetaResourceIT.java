...
class InvoiceBetaResourceIT {
    ...
    void createInvoiceBeta() throws Exception {
        ...
        var returnedInvoiceBeta = om.readValue(
            restInvoiceBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceBeta)))
    ...
    void putExistingInvoiceBeta() throws Exception {
        ...
        restInvoiceBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedInvoiceBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedInvoiceBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingInvoiceBeta() throws Exception {
        ...
        restInvoiceBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, invoiceBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(invoiceBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamInvoiceBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceBeta)))
    ...
    void patchNonExistingInvoiceBeta() throws Exception {
        ...
        restInvoiceBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, invoiceBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(invoiceBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamInvoiceBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(invoiceBeta)))
            .andExpect(status().isMethodNotAllowed());
