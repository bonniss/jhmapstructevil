...
class ProductThetaResourceIT {
    ...
    void createProductTheta() throws Exception {
        ...
        var returnedProductTheta = om.readValue(
            restProductThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productTheta)))
    ...
    void putExistingProductTheta() throws Exception {
        ...
        restProductThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedProductTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedProductTheta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingProductTheta() throws Exception {
        ...
        restProductThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, productTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(productTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamProductTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productTheta)))
    ...
    void patchNonExistingProductTheta() throws Exception {
        ...
        restProductThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, productTheta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(productTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamProductTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(productTheta)))
            .andExpect(status().isMethodNotAllowed());
