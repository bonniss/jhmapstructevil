...
class CustomerBetaResourceIT {
    ...
    void createCustomerBeta() throws Exception {
        ...
        var returnedCustomerBeta = om.readValue(
            restCustomerBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerBeta)))
    ...
    void putExistingCustomerBeta() throws Exception {
        ...
        restCustomerBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedCustomerBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedCustomerBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingCustomerBeta() throws Exception {
        ...
        restCustomerBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, customerBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(customerBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamCustomerBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCustomerBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerBeta)))
    ...
    void patchNonExistingCustomerBeta() throws Exception {
        ...
        restCustomerBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, customerBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(customerBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCustomerBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCustomerBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(customerBeta)))
            .andExpect(status().isMethodNotAllowed());
