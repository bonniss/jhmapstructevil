...
class OrderThetaResourceIT {
    ...
    void createOrderTheta() throws Exception {
        ...
        var returnedOrderTheta = om.readValue(
            restOrderThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderTheta)))
    ...
    void putExistingOrderTheta() throws Exception {
        ...
        restOrderThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedOrderTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedOrderTheta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingOrderTheta() throws Exception {
        ...
        restOrderThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, orderTheta.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderTheta))
            )
            .andExpect(status().isBadRequest());
        ...
        restOrderThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(orderTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamOrderTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderTheta)))
    ...
    void patchNonExistingOrderTheta() throws Exception {
        ...
        restOrderThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, orderTheta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(orderTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamOrderTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(orderTheta)))
            .andExpect(status().isMethodNotAllowed());
