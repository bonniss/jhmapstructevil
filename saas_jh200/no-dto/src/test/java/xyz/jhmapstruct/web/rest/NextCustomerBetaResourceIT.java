...
class NextCustomerBetaResourceIT {
    ...
    void createNextCustomerBeta() throws Exception {
        ...
        var returnedNextCustomerBeta = om.readValue(
            restNextCustomerBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerBeta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerBeta.class
        );
    ...
    void putExistingNextCustomerBeta() throws Exception {
        ...
        restNextCustomerBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextCustomerBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextCustomerBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextCustomerBeta() throws Exception {
        ...
        restNextCustomerBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextCustomerBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerBeta)))
    ...
    void patchNonExistingNextCustomerBeta() throws Exception {
        ...
        restNextCustomerBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerBeta)))
            .andExpect(status().isMethodNotAllowed());
