...
class ShipmentGammaResourceIT {
    ...
    void createShipmentGamma() throws Exception {
        ...
        var returnedShipmentGamma = om.readValue(
            restShipmentGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentGamma)))
    ...
    void putExistingShipmentGamma() throws Exception {
        ...
        restShipmentGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedShipmentGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedShipmentGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingShipmentGamma() throws Exception {
        ...
        restShipmentGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, shipmentGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(shipmentGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamShipmentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentGamma)))
    ...
    void patchNonExistingShipmentGamma() throws Exception {
        ...
        restShipmentGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, shipmentGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(shipmentGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamShipmentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(shipmentGamma)))
            .andExpect(status().isMethodNotAllowed());
