...
class NextInvoiceAlphaResourceIT {
    ...
    void createNextInvoiceAlpha() throws Exception {
        ...
        var returnedNextInvoiceAlpha = om.readValue(
            restNextInvoiceAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceAlpha)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextInvoiceAlpha.class
        );
    ...
    void putExistingNextInvoiceAlpha() throws Exception {
        ...
        restNextInvoiceAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoiceAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoiceAlpha))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoiceAlpha() throws Exception {
        ...
        restNextInvoiceAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoiceAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceAlpha)))
    ...
    void patchNonExistingNextInvoiceAlpha() throws Exception {
        ...
        restNextInvoiceAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceAlpha.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceAlpha)))
            .andExpect(status().isMethodNotAllowed());
