...
class NextReviewViResourceIT {
    ...
    void createNextReviewVi() throws Exception {
        ...
        var returnedNextReviewVi = om.readValue(
            restNextReviewViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewVi)))
    ...
    void putExistingNextReviewVi() throws Exception {
        ...
        restNextReviewViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextReviewVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextReviewVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextReviewVi() throws Exception {
        ...
        restNextReviewViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextReviewVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextReviewVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextReviewVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewVi)))
    ...
    void patchNonExistingNextReviewVi() throws Exception {
        ...
        restNextReviewViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextReviewVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextReviewVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextReviewVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextReviewVi)))
            .andExpect(status().isMethodNotAllowed());
