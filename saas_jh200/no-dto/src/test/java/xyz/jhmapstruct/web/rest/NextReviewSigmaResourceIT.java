...
class NextReviewSigmaResourceIT {
    ...
    void createNextReviewSigma() throws Exception {
        ...
        var returnedNextReviewSigma = om.readValue(
            restNextReviewSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewSigma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextReviewSigma.class
        );
    ...
    void putExistingNextReviewSigma() throws Exception {
        ...
        restNextReviewSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextReviewSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextReviewSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextReviewSigma() throws Exception {
        ...
        restNextReviewSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextReviewSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextReviewSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextReviewSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewSigma)))
    ...
    void patchNonExistingNextReviewSigma() throws Exception {
        ...
        restNextReviewSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextReviewSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextReviewSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextReviewSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextReviewSigma)))
            .andExpect(status().isMethodNotAllowed());
