...
class ShipmentAlphaResourceIT {
    ...
    void createShipmentAlpha() throws Exception {
        ...
        var returnedShipmentAlpha = om.readValue(
            restShipmentAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentAlpha)))
    ...
    void putExistingShipmentAlpha() throws Exception {
        ...
        restShipmentAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedShipmentAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedShipmentAlpha))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingShipmentAlpha() throws Exception {
        ...
        restShipmentAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, shipmentAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(shipmentAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamShipmentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentAlpha)))
    ...
    void patchNonExistingShipmentAlpha() throws Exception {
        ...
        restShipmentAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, shipmentAlpha.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(shipmentAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamShipmentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(shipmentAlpha)))
            .andExpect(status().isMethodNotAllowed());
