...
class NextCategorySigmaResourceIT {
    ...
    void createNextCategorySigma() throws Exception {
        ...
        var returnedNextCategorySigma = om.readValue(
            restNextCategorySigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategorySigma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCategorySigma.class
        );
    ...
    void putExistingNextCategorySigma() throws Exception {
        ...
        restNextCategorySigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextCategorySigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextCategorySigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextCategorySigma() throws Exception {
        ...
        restNextCategorySigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCategorySigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCategorySigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextCategorySigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategorySigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategorySigma)))
    ...
    void patchNonExistingNextCategorySigma() throws Exception {
        ...
        restNextCategorySigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCategorySigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCategorySigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCategorySigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategorySigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCategorySigma)))
            .andExpect(status().isMethodNotAllowed());
