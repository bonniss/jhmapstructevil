...
class OrderSigmaResourceIT {
    ...
    void createOrderSigma() throws Exception {
        ...
        var returnedOrderSigma = om.readValue(
            restOrderSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderSigma)))
    ...
    void putExistingOrderSigma() throws Exception {
        ...
        restOrderSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedOrderSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedOrderSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingOrderSigma() throws Exception {
        ...
        restOrderSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, orderSigma.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderSigma))
            )
            .andExpect(status().isBadRequest());
        ...
        restOrderSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(orderSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamOrderSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderSigma)))
    ...
    void patchNonExistingOrderSigma() throws Exception {
        ...
        restOrderSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, orderSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(orderSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamOrderSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(orderSigma)))
            .andExpect(status().isMethodNotAllowed());
