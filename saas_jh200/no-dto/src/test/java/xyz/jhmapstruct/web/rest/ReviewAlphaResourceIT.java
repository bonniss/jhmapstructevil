...
class ReviewAlphaResourceIT {
    ...
    void createReviewAlpha() throws Exception {
        ...
        var returnedReviewAlpha = om.readValue(
            restReviewAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewAlpha)))
    ...
    void putExistingReviewAlpha() throws Exception {
        ...
        restReviewAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedReviewAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedReviewAlpha))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingReviewAlpha() throws Exception {
        ...
        restReviewAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, reviewAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(reviewAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamReviewAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewAlpha)))
    ...
    void patchNonExistingReviewAlpha() throws Exception {
        ...
        restReviewAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, reviewAlpha.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(reviewAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamReviewAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(reviewAlpha)))
            .andExpect(status().isMethodNotAllowed());
