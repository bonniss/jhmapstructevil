...
class OrderBetaResourceIT {
    ...
    void createOrderBeta() throws Exception {
        ...
        var returnedOrderBeta = om.readValue(
            restOrderBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderBeta)))
    ...
    void putExistingOrderBeta() throws Exception {
        ...
        restOrderBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedOrderBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedOrderBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingOrderBeta() throws Exception {
        ...
        restOrderBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, orderBeta.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderBeta))
            )
            .andExpect(status().isBadRequest());
        ...
        restOrderBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(orderBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamOrderBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderBeta)))
    ...
    void patchNonExistingOrderBeta() throws Exception {
        ...
        restOrderBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, orderBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(orderBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamOrderBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(orderBeta)))
            .andExpect(status().isMethodNotAllowed());
