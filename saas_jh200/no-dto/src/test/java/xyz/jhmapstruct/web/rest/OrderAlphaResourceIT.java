...
class OrderAlphaResourceIT {
    ...
    void createOrderAlpha() throws Exception {
        ...
        var returnedOrderAlpha = om.readValue(
            restOrderAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderAlpha)))
    ...
    void putExistingOrderAlpha() throws Exception {
        ...
        restOrderAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedOrderAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedOrderAlpha))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingOrderAlpha() throws Exception {
        ...
        restOrderAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, orderAlpha.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderAlpha))
            )
            .andExpect(status().isBadRequest());
        ...
        restOrderAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(orderAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamOrderAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderAlpha)))
    ...
    void patchNonExistingOrderAlpha() throws Exception {
        ...
        restOrderAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, orderAlpha.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(orderAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamOrderAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(orderAlpha)))
            .andExpect(status().isMethodNotAllowed());
