...
class NextProductThetaResourceIT {
    ...
    void createNextProductTheta() throws Exception {
        ...
        var returnedNextProductTheta = om.readValue(
            restNextProductThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductTheta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextProductTheta.class
        );
    ...
    void putExistingNextProductTheta() throws Exception {
        ...
        restNextProductThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextProductTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextProductTheta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextProductTheta() throws Exception {
        ...
        restNextProductThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextProductTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextProductTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextProductTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductTheta)))
    ...
    void patchNonExistingNextProductTheta() throws Exception {
        ...
        restNextProductThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextProductTheta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextProductTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextProductTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextProductTheta)))
            .andExpect(status().isMethodNotAllowed());
