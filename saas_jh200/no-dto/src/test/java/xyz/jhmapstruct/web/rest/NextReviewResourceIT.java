...
class NextReviewResourceIT {
    ...
    void createNextReview() throws Exception {
        ...
        var returnedNextReview = om.readValue(
            restNextReviewMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReview)))
    ...
    void putExistingNextReview() throws Exception {
        ...
        restNextReviewMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextReview.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextReview))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextReview() throws Exception {
        ...
        restNextReviewMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextReview.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReview))
            )
            .andExpect(status().isBadRequest());
        ...
        restNextReviewMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextReview))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextReview() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReview)))
    ...
    void patchNonExistingNextReview() throws Exception {
        ...
        restNextReviewMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextReview.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextReview))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextReview() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextReview)))
            .andExpect(status().isMethodNotAllowed());
