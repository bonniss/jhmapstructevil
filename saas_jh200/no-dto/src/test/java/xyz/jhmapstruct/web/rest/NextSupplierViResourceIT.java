...
class NextSupplierViResourceIT {
    ...
    void createNextSupplierVi() throws Exception {
        ...
        var returnedNextSupplierVi = om.readValue(
            restNextSupplierViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierVi)))
    ...
    void putExistingNextSupplierVi() throws Exception {
        ...
        restNextSupplierViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextSupplierVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextSupplierVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextSupplierVi() throws Exception {
        ...
        restNextSupplierViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextSupplierVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextSupplierVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextSupplierVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierVi)))
    ...
    void patchNonExistingNextSupplierVi() throws Exception {
        ...
        restNextSupplierViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextSupplierVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextSupplierVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextSupplierVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextSupplierVi)))
            .andExpect(status().isMethodNotAllowed());
