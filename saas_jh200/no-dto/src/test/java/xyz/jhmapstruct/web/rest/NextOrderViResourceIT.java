...
class NextOrderViResourceIT {
    ...
    void createNextOrderVi() throws Exception {
        ...
        var returnedNextOrderVi = om.readValue(
            restNextOrderViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderVi)))
    ...
    void putExistingNextOrderVi() throws Exception {
        ...
        restNextOrderViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextOrderVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextOrderVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextOrderVi() throws Exception {
        ...
        restNextOrderViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextOrderVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderVi)))
    ...
    void patchNonExistingNextOrderVi() throws Exception {
        ...
        restNextOrderViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderVi)))
            .andExpect(status().isMethodNotAllowed());
