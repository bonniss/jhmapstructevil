...
class NextReviewBetaResourceIT {
    ...
    void createNextReviewBeta() throws Exception {
        ...
        var returnedNextReviewBeta = om.readValue(
            restNextReviewBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewBeta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextReviewBeta.class
        );
    ...
    void putExistingNextReviewBeta() throws Exception {
        ...
        restNextReviewBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextReviewBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextReviewBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextReviewBeta() throws Exception {
        ...
        restNextReviewBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextReviewBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextReviewBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextReviewBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewBeta)))
    ...
    void patchNonExistingNextReviewBeta() throws Exception {
        ...
        restNextReviewBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextReviewBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextReviewBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextReviewBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextReviewBeta)))
            .andExpect(status().isMethodNotAllowed());
