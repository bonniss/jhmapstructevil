...
class NextShipmentAlphaResourceIT {
    ...
    void createNextShipmentAlpha() throws Exception {
        ...
        var returnedNextShipmentAlpha = om.readValue(
            restNextShipmentAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentAlpha)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentAlpha.class
        );
    ...
    void putExistingNextShipmentAlpha() throws Exception {
        ...
        restNextShipmentAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextShipmentAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextShipmentAlpha))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextShipmentAlpha() throws Exception {
        ...
        restNextShipmentAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentAlpha.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextShipmentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentAlpha)))
    ...
    void patchNonExistingNextShipmentAlpha() throws Exception {
        ...
        restNextShipmentAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentAlpha.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentAlpha))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipmentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentAlpha)))
            .andExpect(status().isMethodNotAllowed());
