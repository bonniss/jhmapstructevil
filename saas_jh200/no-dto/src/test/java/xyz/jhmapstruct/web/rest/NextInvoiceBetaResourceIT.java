...
class NextInvoiceBetaResourceIT {
    ...
    void createNextInvoiceBeta() throws Exception {
        ...
        var returnedNextInvoiceBeta = om.readValue(
            restNextInvoiceBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceBeta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextInvoiceBeta.class
        );
    ...
    void putExistingNextInvoiceBeta() throws Exception {
        ...
        restNextInvoiceBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoiceBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoiceBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoiceBeta() throws Exception {
        ...
        restNextInvoiceBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoiceBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceBeta)))
    ...
    void patchNonExistingNextInvoiceBeta() throws Exception {
        ...
        restNextInvoiceBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceBeta)))
            .andExpect(status().isMethodNotAllowed());
