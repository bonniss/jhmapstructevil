...
class NextCustomerResourceIT {
    ...
    void createNextCustomer() throws Exception {
        ...
        var returnedNextCustomer = om.readValue(
            restNextCustomerMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomer)))
    ...
    void putExistingNextCustomer() throws Exception {
        ...
        restNextCustomerMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextCustomer.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextCustomer))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextCustomer() throws Exception {
        ...
        restNextCustomerMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomer.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomer))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextCustomer() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomer)))
    ...
    void patchNonExistingNextCustomer() throws Exception {
        ...
        restNextCustomerMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomer.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomer))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomer() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomer)))
            .andExpect(status().isMethodNotAllowed());
