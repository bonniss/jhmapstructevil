...
class NextShipmentSigmaResourceIT {
    ...
    void createNextShipmentSigma() throws Exception {
        ...
        var returnedNextShipmentSigma = om.readValue(
            restNextShipmentSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentSigma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentSigma.class
        );
    ...
    void putExistingNextShipmentSigma() throws Exception {
        ...
        restNextShipmentSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextShipmentSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextShipmentSigma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextShipmentSigma() throws Exception {
        ...
        restNextShipmentSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentSigma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextShipmentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentSigma)))
    ...
    void patchNonExistingNextShipmentSigma() throws Exception {
        ...
        restNextShipmentSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentSigma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentSigma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipmentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentSigma)))
            .andExpect(status().isMethodNotAllowed());
