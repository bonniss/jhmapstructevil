...
class NextInvoiceViViResourceIT {
    ...
    void createNextInvoiceViVi() throws Exception {
        ...
        var returnedNextInvoiceViVi = om.readValue(
            restNextInvoiceViViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceViVi)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextInvoiceViVi.class
        );
    ...
    void putExistingNextInvoiceViVi() throws Exception {
        ...
        restNextInvoiceViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoiceViVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoiceViVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoiceViVi() throws Exception {
        ...
        restNextInvoiceViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceViVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceViVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoiceViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceViViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceViVi)))
    ...
    void patchNonExistingNextInvoiceViVi() throws Exception {
        ...
        restNextInvoiceViViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceViVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceViVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceViViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceViVi)))
            .andExpect(status().isMethodNotAllowed());
