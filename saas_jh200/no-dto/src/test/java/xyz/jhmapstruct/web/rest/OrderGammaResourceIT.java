...
class OrderGammaResourceIT {
    ...
    void createOrderGamma() throws Exception {
        ...
        var returnedOrderGamma = om.readValue(
            restOrderGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderGamma)))
    ...
    void putExistingOrderGamma() throws Exception {
        ...
        restOrderGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedOrderGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedOrderGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingOrderGamma() throws Exception {
        ...
        restOrderGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, orderGamma.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderGamma))
            )
            .andExpect(status().isBadRequest());
        ...
        restOrderGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(orderGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamOrderGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderGamma)))
    ...
    void patchNonExistingOrderGamma() throws Exception {
        ...
        restOrderGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, orderGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(orderGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamOrderGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(orderGamma)))
            .andExpect(status().isMethodNotAllowed());
