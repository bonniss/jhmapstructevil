...
class ShipmentBetaResourceIT {
    ...
    void createShipmentBeta() throws Exception {
        ...
        var returnedShipmentBeta = om.readValue(
            restShipmentBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentBeta)))
    ...
    void putExistingShipmentBeta() throws Exception {
        ...
        restShipmentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedShipmentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedShipmentBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingShipmentBeta() throws Exception {
        ...
        restShipmentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, shipmentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(shipmentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamShipmentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentBeta)))
    ...
    void patchNonExistingShipmentBeta() throws Exception {
        ...
        restShipmentBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, shipmentBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(shipmentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamShipmentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(shipmentBeta)))
            .andExpect(status().isMethodNotAllowed());
