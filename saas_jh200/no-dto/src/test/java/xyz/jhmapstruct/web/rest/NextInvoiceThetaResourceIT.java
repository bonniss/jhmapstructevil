...
class NextInvoiceThetaResourceIT {
    ...
    void createNextInvoiceTheta() throws Exception {
        ...
        var returnedNextInvoiceTheta = om.readValue(
            restNextInvoiceThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceTheta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextInvoiceTheta.class
        );
    ...
    void putExistingNextInvoiceTheta() throws Exception {
        ...
        restNextInvoiceThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoiceTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoiceTheta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoiceTheta() throws Exception {
        ...
        restNextInvoiceThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoiceTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceTheta)))
    ...
    void patchNonExistingNextInvoiceTheta() throws Exception {
        ...
        restNextInvoiceThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceTheta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceTheta)))
            .andExpect(status().isMethodNotAllowed());
