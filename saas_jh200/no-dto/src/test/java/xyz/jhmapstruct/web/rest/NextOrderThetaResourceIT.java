...
class NextOrderThetaResourceIT {
    ...
    void createNextOrderTheta() throws Exception {
        ...
        var returnedNextOrderTheta = om.readValue(
            restNextOrderThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderTheta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextOrderTheta.class
        );
    ...
    void putExistingNextOrderTheta() throws Exception {
        ...
        restNextOrderThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextOrderTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextOrderTheta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextOrderTheta() throws Exception {
        ...
        restNextOrderThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderTheta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextOrderTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderTheta)))
    ...
    void patchNonExistingNextOrderTheta() throws Exception {
        ...
        restNextOrderThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderTheta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderTheta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderTheta)))
            .andExpect(status().isMethodNotAllowed());
