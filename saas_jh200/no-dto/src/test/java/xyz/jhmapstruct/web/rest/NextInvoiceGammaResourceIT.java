...
class NextInvoiceGammaResourceIT {
    ...
    void createNextInvoiceGamma() throws Exception {
        ...
        var returnedNextInvoiceGamma = om.readValue(
            restNextInvoiceGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceGamma)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextInvoiceGamma.class
        );
    ...
    void putExistingNextInvoiceGamma() throws Exception {
        ...
        restNextInvoiceGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoiceGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoiceGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoiceGamma() throws Exception {
        ...
        restNextInvoiceGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoiceGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceGamma)))
    ...
    void patchNonExistingNextInvoiceGamma() throws Exception {
        ...
        restNextInvoiceGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceGamma)))
            .andExpect(status().isMethodNotAllowed());
