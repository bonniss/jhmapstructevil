...
class NextInvoiceResourceIT {
    ...
    void createNextInvoice() throws Exception {
        ...
        var returnedNextInvoice = om.readValue(
            restNextInvoiceMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoice)))
    ...
    void putExistingNextInvoice() throws Exception {
        ...
        restNextInvoiceMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextInvoice.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextInvoice))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextInvoice() throws Exception {
        ...
        restNextInvoiceMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoice.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoice))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextInvoice() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoice)))
    ...
    void patchNonExistingNextInvoice() throws Exception {
        ...
        restNextInvoiceMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoice.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoice))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoice() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoice)))
            .andExpect(status().isMethodNotAllowed());
