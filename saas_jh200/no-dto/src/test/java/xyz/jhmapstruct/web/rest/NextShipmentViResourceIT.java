...
class NextShipmentViResourceIT {
    ...
    void createNextShipmentVi() throws Exception {
        ...
        var returnedNextShipmentVi = om.readValue(
            restNextShipmentViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentVi)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentVi.class
        );
    ...
    void putExistingNextShipmentVi() throws Exception {
        ...
        restNextShipmentViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextShipmentVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextShipmentVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextShipmentVi() throws Exception {
        ...
        restNextShipmentViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextShipmentVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentVi)))
    ...
    void patchNonExistingNextShipmentVi() throws Exception {
        ...
        restNextShipmentViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipmentVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentVi)))
            .andExpect(status().isMethodNotAllowed());
