...
class NextShipmentBetaResourceIT {
    ...
    void createNextShipmentBeta() throws Exception {
        ...
        var returnedNextShipmentBeta = om.readValue(
            restNextShipmentBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentBeta)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentBeta.class
        );
    ...
    void putExistingNextShipmentBeta() throws Exception {
        ...
        restNextShipmentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextShipmentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextShipmentBeta))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextShipmentBeta() throws Exception {
        ...
        restNextShipmentBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentBeta.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextShipmentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentBeta)))
    ...
    void patchNonExistingNextShipmentBeta() throws Exception {
        ...
        restNextShipmentBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentBeta.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentBeta))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipmentBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentBeta)))
            .andExpect(status().isMethodNotAllowed());
