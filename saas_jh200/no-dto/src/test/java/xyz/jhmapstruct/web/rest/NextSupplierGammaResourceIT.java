...
class NextSupplierGammaResourceIT {
    ...
    void createNextSupplierGamma() throws Exception {
        ...
        var returnedNextSupplierGamma = om.readValue(
            restNextSupplierGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierGamma)))
    ...
    void putExistingNextSupplierGamma() throws Exception {
        ...
        restNextSupplierGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedNextSupplierGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedNextSupplierGamma))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingNextSupplierGamma() throws Exception {
        ...
        restNextSupplierGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextSupplierGamma.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextSupplierGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamNextSupplierGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierGamma)))
    ...
    void patchNonExistingNextSupplierGamma() throws Exception {
        ...
        restNextSupplierGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextSupplierGamma.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextSupplierGamma))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextSupplierGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextSupplierGamma)))
            .andExpect(status().isMethodNotAllowed());
