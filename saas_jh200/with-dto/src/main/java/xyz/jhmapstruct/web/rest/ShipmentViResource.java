...
import xyz.jhmapstruct.service.ShipmentViQueryService;
...
public class ShipmentViResource {
    ...
    private final ShipmentViQueryService shipmentViQueryService;

    public ShipmentViResource(
        ShipmentViService shipmentViService,
        ShipmentViRepository shipmentViRepository,
        ShipmentViQueryService shipmentViQueryService
    ) {
        this.shipmentViService = shipmentViService;
        this.shipmentViRepository = shipmentViRepository;
        this.shipmentViQueryService = shipmentViQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<ShipmentViDTO>> getAllShipmentVis(
        ShipmentViCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get ShipmentVis by criteria: {}", criteria);

        Page<ShipmentViDTO> page = shipmentViQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /shipment-vis/count} : count all the shipmentVis.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countShipmentVis(ShipmentViCriteria criteria) {
        LOG.debug("REST request to count ShipmentVis by criteria: {}", criteria);
        return ResponseEntity.ok().body(shipmentViQueryService.countByCriteria(criteria));
    }
