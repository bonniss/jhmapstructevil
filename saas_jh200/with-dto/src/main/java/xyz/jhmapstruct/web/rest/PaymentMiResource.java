...
import xyz.jhmapstruct.service.PaymentMiQueryService;
...
public class PaymentMiResource {
    ...
    private final PaymentMiQueryService paymentMiQueryService;

    public PaymentMiResource(
        PaymentMiService paymentMiService,
        PaymentMiRepository paymentMiRepository,
        PaymentMiQueryService paymentMiQueryService
    ) {
        this.paymentMiService = paymentMiService;
        this.paymentMiRepository = paymentMiRepository;
        this.paymentMiQueryService = paymentMiQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<PaymentMiDTO>> getAllPaymentMis(
        PaymentMiCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get PaymentMis by criteria: {}", criteria);

        Page<PaymentMiDTO> page = paymentMiQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /payment-mis/count} : count all the paymentMis.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countPaymentMis(PaymentMiCriteria criteria) {
        LOG.debug("REST request to count PaymentMis by criteria: {}", criteria);
        return ResponseEntity.ok().body(paymentMiQueryService.countByCriteria(criteria));
    }
