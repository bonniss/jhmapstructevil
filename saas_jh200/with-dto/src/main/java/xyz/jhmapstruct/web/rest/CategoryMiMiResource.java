...
import xyz.jhmapstruct.service.CategoryMiMiQueryService;
...
public class CategoryMiMiResource {
    ...
    private final CategoryMiMiQueryService categoryMiMiQueryService;

    public CategoryMiMiResource(
        CategoryMiMiService categoryMiMiService,
        CategoryMiMiRepository categoryMiMiRepository,
        CategoryMiMiQueryService categoryMiMiQueryService
    ) {
        this.categoryMiMiService = categoryMiMiService;
        this.categoryMiMiRepository = categoryMiMiRepository;
        this.categoryMiMiQueryService = categoryMiMiQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<CategoryMiMiDTO>> getAllCategoryMiMis(
        CategoryMiMiCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get CategoryMiMis by criteria: {}", criteria);

        Page<CategoryMiMiDTO> page = categoryMiMiQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /category-mi-mis/count} : count all the categoryMiMis.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countCategoryMiMis(CategoryMiMiCriteria criteria) {
        LOG.debug("REST request to count CategoryMiMis by criteria: {}", criteria);
        return ResponseEntity.ok().body(categoryMiMiQueryService.countByCriteria(criteria));
    }
