...
import xyz.jhmapstruct.service.PaymentViViQueryService;
...
public class PaymentViViResource {
    ...
    private final PaymentViViQueryService paymentViViQueryService;

    public PaymentViViResource(
        PaymentViViService paymentViViService,
        PaymentViViRepository paymentViViRepository,
        PaymentViViQueryService paymentViViQueryService
    ) {
        this.paymentViViService = paymentViViService;
        this.paymentViViRepository = paymentViViRepository;
        this.paymentViViQueryService = paymentViViQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<PaymentViViDTO>> getAllPaymentViVis(
        PaymentViViCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get PaymentViVis by criteria: {}", criteria);

        Page<PaymentViViDTO> page = paymentViViQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /payment-vi-vis/count} : count all the paymentViVis.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countPaymentViVis(PaymentViViCriteria criteria) {
        LOG.debug("REST request to count PaymentViVis by criteria: {}", criteria);
        return ResponseEntity.ok().body(paymentViViQueryService.countByCriteria(criteria));
    }
