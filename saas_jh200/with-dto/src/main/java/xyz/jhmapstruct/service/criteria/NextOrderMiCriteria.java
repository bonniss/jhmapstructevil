...
/**
 * Criteria class for the {@link xyz.jhmapstruct.domain.NextOrderMi} entity. This class is used
 * in {@link xyz.jhmapstruct.web.rest.NextOrderMiResource} to receive all the possible filtering options from
 * the Http GET request parameters.
 * For example the following could be a valid request:
 * {@code /next-order-mis?id.greaterThan=5&attr1.contains=something&attr2.specified=false}
 * As Spring is unable to properly convert the types, unless specific {@link Filter} class are used, we need to use
 * fix type specific filters.
 */
@ParameterObject
@SuppressWarnings("common-java:DuplicatedBlocks")
public class NextOrderMiCriteria implements Serializable, Criteria {

    /**
     * Class for filtering OrderMiStatus
     */
    public static class OrderMiStatusFilter extends Filter<OrderMiStatus> {

        public OrderMiStatusFilter() {}

        public OrderMiStatusFilter(OrderMiStatusFilter filter) {
            super(filter);
        }

        @Override
        public OrderMiStatusFilter copy() {
            return new OrderMiStatusFilter(this);
        }
    }
    ...
    private OrderMiStatusFilter status;
    ...
    public NextOrderMiCriteria() {}

    public NextOrderMiCriteria(NextOrderMiCriteria other) {
        ...
        this.status = other.optionalStatus().map(OrderMiStatusFilter::copy).orElse(null);
    ...
    @Override
    public NextOrderMiCriteria copy() {
        return new NextOrderMiCriteria(this);
    }
    ...
    public OrderMiStatusFilter getStatus() {
        return status;
    }

    public Optional<OrderMiStatusFilter> optionalStatus() {
        return Optional.ofNullable(status);
    }

    public OrderMiStatusFilter status() {
        if (status == null) {
            setStatus(new OrderMiStatusFilter());
        }
        return status;
    }

    public void setStatus(OrderMiStatusFilter status) {
        this.status = status;
    }
    ...
    public boolean equals(Object o) {
        ...
        final NextOrderMiCriteria that = (NextOrderMiCriteria) o;
        return (
