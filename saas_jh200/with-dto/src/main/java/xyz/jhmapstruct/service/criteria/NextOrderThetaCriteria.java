...
/**
 * Criteria class for the {@link xyz.jhmapstruct.domain.NextOrderTheta} entity. This class is used
 * in {@link xyz.jhmapstruct.web.rest.NextOrderThetaResource} to receive all the possible filtering options from
 * the Http GET request parameters.
 * For example the following could be a valid request:
 * {@code /next-order-thetas?id.greaterThan=5&attr1.contains=something&attr2.specified=false}
 * As Spring is unable to properly convert the types, unless specific {@link Filter} class are used, we need to use
 * fix type specific filters.
 */
@ParameterObject
@SuppressWarnings("common-java:DuplicatedBlocks")
public class NextOrderThetaCriteria implements Serializable, Criteria {

    /**
     * Class for filtering OrderThetaStatus
     */
    public static class OrderThetaStatusFilter extends Filter<OrderThetaStatus> {

        public OrderThetaStatusFilter() {}

        public OrderThetaStatusFilter(OrderThetaStatusFilter filter) {
            super(filter);
        }

        @Override
        public OrderThetaStatusFilter copy() {
            return new OrderThetaStatusFilter(this);
        }
    }
    ...
    private OrderThetaStatusFilter status;
    ...
    public NextOrderThetaCriteria() {}

    public NextOrderThetaCriteria(NextOrderThetaCriteria other) {
        ...
        this.status = other.optionalStatus().map(OrderThetaStatusFilter::copy).orElse(null);
    ...
    @Override
    public NextOrderThetaCriteria copy() {
        return new NextOrderThetaCriteria(this);
    }
    ...
    public OrderThetaStatusFilter getStatus() {
        return status;
    }

    public Optional<OrderThetaStatusFilter> optionalStatus() {
        return Optional.ofNullable(status);
    }

    public OrderThetaStatusFilter status() {
        if (status == null) {
            setStatus(new OrderThetaStatusFilter());
        }
        return status;
    }

    public void setStatus(OrderThetaStatusFilter status) {
        this.status = status;
    }
    ...
    public boolean equals(Object o) {
        ...
        final NextOrderThetaCriteria that = (NextOrderThetaCriteria) o;
        return (
