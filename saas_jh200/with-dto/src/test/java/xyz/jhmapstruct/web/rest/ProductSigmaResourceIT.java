...
class ProductSigmaResourceIT {
    ...
    void createProductSigma() throws Exception {
        ...
        ProductSigmaDTO productSigmaDTO = productSigmaMapper.toDto(productSigma);
        var returnedProductSigmaDTO = om.readValue(
            restProductSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            ProductSigmaDTO.class
        );

        // Validate the ProductSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedProductSigma = productSigmaMapper.toEntity(returnedProductSigmaDTO);
    ...
    void putExistingProductSigma() throws Exception {
        ...
        ProductSigmaDTO productSigmaDTO = productSigmaMapper.toDto(updatedProductSigma);

        restProductSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, productSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(productSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamProductSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productSigmaDTO)))
    ...
    void patchNonExistingProductSigma() throws Exception {
        ...
        restProductSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, productSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(productSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamProductSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(productSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
