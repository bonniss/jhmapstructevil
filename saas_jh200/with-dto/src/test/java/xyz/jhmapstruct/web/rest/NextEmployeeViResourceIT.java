...
class NextEmployeeViResourceIT {
    ...
    void createNextEmployeeVi() throws Exception {
        ...
        NextEmployeeViDTO nextEmployeeViDTO = nextEmployeeViMapper.toDto(nextEmployeeVi);
        var returnedNextEmployeeViDTO = om.readValue(
            restNextEmployeeViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextEmployeeViDTO.class
        );

        // Validate the NextEmployeeVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextEmployeeVi = nextEmployeeViMapper.toEntity(returnedNextEmployeeViDTO);
    ...
    void putExistingNextEmployeeVi() throws Exception {
        ...
        updatedNextEmployeeVi
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        NextEmployeeViDTO nextEmployeeViDTO = nextEmployeeViMapper.toDto(updatedNextEmployeeVi);

        restNextEmployeeViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextEmployeeViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextEmployeeViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextEmployeeVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeViDTO)))
    ...
    void patchNonExistingNextEmployeeVi() throws Exception {
        ...
        restNextEmployeeViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextEmployeeViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextEmployeeViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextEmployeeVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextEmployeeViDTO)))
            .andExpect(status().isMethodNotAllowed());
