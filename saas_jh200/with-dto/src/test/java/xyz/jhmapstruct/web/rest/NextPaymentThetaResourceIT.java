...
class NextPaymentThetaResourceIT {
    ...
    void createNextPaymentTheta() throws Exception {
        ...
        NextPaymentThetaDTO nextPaymentThetaDTO = nextPaymentThetaMapper.toDto(nextPaymentTheta);
        var returnedNextPaymentThetaDTO = om.readValue(
            restNextPaymentThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentThetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentThetaDTO.class
        );

        // Validate the NextPaymentTheta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPaymentTheta = nextPaymentThetaMapper.toEntity(returnedNextPaymentThetaDTO);
    ...
    void putExistingNextPaymentTheta() throws Exception {
        ...
        NextPaymentThetaDTO nextPaymentThetaDTO = nextPaymentThetaMapper.toDto(updatedNextPaymentTheta);

        restNextPaymentThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentThetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentThetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPaymentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentThetaDTO)))
    ...
    void patchNonExistingNextPaymentTheta() throws Exception {
        ...
        restNextPaymentThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentThetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentThetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentThetaDTO)))
            .andExpect(status().isMethodNotAllowed());
