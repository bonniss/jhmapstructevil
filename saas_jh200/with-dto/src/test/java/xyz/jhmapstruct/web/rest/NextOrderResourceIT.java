...
class NextOrderResourceIT {
    ...
    void createNextOrder() throws Exception {
        ...
        NextOrderDTO nextOrderDTO = nextOrderMapper.toDto(nextOrder);
        var returnedNextOrderDTO = om.readValue(
            restNextOrderMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderDTO)))
        ...
        var returnedNextOrder = nextOrderMapper.toEntity(returnedNextOrderDTO);
    ...
    void putExistingNextOrder() throws Exception {
        ...
        NextOrderDTO nextOrderDTO = nextOrderMapper.toDto(updatedNextOrder);

        restNextOrderMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextOrder() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderDTO)))
    ...
    void patchNonExistingNextOrder() throws Exception {
        ...
        restNextOrderMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrder() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderDTO)))
            .andExpect(status().isMethodNotAllowed());
