...
import static xyz.jhmapstruct.domain.OrderAsserts.*;
...
class OrderResourceIT {
    ...
    public static Order createEntity() {
        return new Order().orderDate(DEFAULT_ORDER_DATE).totalPrice(DEFAULT_TOTAL_PRICE).status(DEFAULT_STATUS);
    }
    ...
    public static Order createUpdatedEntity() {
        return new Order().orderDate(UPDATED_ORDER_DATE).totalPrice(UPDATED_TOTAL_PRICE).status(UPDATED_STATUS);
    }
    ...
    @Test
    @Transactional
    void getAllOrdersByPaymentIsEqualToSomething() throws Exception {
        Payment payment;
        ...
        // Get all the orderList where payment equals to paymentId
        defaultOrderShouldBeFound("paymentId.equals=" + paymentId);

        // Get all the orderList where payment equals to (paymentId + 1)
        defaultOrderShouldNotBeFound("paymentId.equals=" + (paymentId + 1));
    }

    @Test
    @Transactional
    void getAllOrdersByShipmentIsEqualToSomething() throws Exception {
        Shipment shipment;
        if (TestUtil.findAll(em, Shipment.class).isEmpty()) {
            orderRepository.saveAndFlush(order);
            shipment = ShipmentResourceIT.createEntity();
        } else {
        ...
        // Get all the orderList where shipment equals to shipmentId
        defaultOrderShouldBeFound("shipmentId.equals=" + shipmentId);

        // Get all the orderList where shipment equals to (shipmentId + 1)
        defaultOrderShouldNotBeFound("shipmentId.equals=" + (shipmentId + 1));
    }

    @Test
    @Transactional
    void getAllOrdersByTenantIsEqualToSomething() throws Exception {
        MasterTenant tenant;
        ...
        // Get all the orderList where tenant equals to tenantId
        defaultOrderShouldBeFound("tenantId.equals=" + tenantId);

        // Get all the orderList where tenant equals to (tenantId + 1)
        defaultOrderShouldNotBeFound("tenantId.equals=" + (tenantId + 1));
    }
    ...
    void putExistingOrder() throws Exception {
        ...
        updatedOrder.orderDate(UPDATED_ORDER_DATE).totalPrice(UPDATED_TOTAL_PRICE).status(UPDATED_STATUS);
    ...
    void fullUpdateOrderWithPatch() throws Exception {
        ...
        partialUpdatedOrder.orderDate(UPDATED_ORDER_DATE).totalPrice(UPDATED_TOTAL_PRICE).status(UPDATED_STATUS);
