...
class NextOrderAlphaResourceIT {
    ...
    void createNextOrderAlpha() throws Exception {
        ...
        NextOrderAlphaDTO nextOrderAlphaDTO = nextOrderAlphaMapper.toDto(nextOrderAlpha);
        var returnedNextOrderAlphaDTO = om.readValue(
            restNextOrderAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextOrderAlphaDTO.class
        );

        // Validate the NextOrderAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextOrderAlpha = nextOrderAlphaMapper.toEntity(returnedNextOrderAlphaDTO);
    ...
    void putExistingNextOrderAlpha() throws Exception {
        ...
        NextOrderAlphaDTO nextOrderAlphaDTO = nextOrderAlphaMapper.toDto(updatedNextOrderAlpha);

        restNextOrderAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextOrderAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderAlphaDTO)))
    ...
    void patchNonExistingNextOrderAlpha() throws Exception {
        ...
        restNextOrderAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
