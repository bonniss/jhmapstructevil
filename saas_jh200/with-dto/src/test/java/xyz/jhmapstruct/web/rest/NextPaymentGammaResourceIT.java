...
class NextPaymentGammaResourceIT {
    ...
    void createNextPaymentGamma() throws Exception {
        ...
        NextPaymentGammaDTO nextPaymentGammaDTO = nextPaymentGammaMapper.toDto(nextPaymentGamma);
        var returnedNextPaymentGammaDTO = om.readValue(
            restNextPaymentGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentGammaDTO.class
        );

        // Validate the NextPaymentGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPaymentGamma = nextPaymentGammaMapper.toEntity(returnedNextPaymentGammaDTO);
    ...
    void putExistingNextPaymentGamma() throws Exception {
        ...
        NextPaymentGammaDTO nextPaymentGammaDTO = nextPaymentGammaMapper.toDto(updatedNextPaymentGamma);

        restNextPaymentGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPaymentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentGammaDTO)))
    ...
    void patchNonExistingNextPaymentGamma() throws Exception {
        ...
        restNextPaymentGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
