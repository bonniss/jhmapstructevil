...
class NextProductMiMiResourceIT {
    ...
    void createNextProductMiMi() throws Exception {
        ...
        NextProductMiMiDTO nextProductMiMiDTO = nextProductMiMiMapper.toDto(nextProductMiMi);
        var returnedNextProductMiMiDTO = om.readValue(
            restNextProductMiMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductMiMiDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextProductMiMiDTO.class
        );

        // Validate the NextProductMiMi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextProductMiMi = nextProductMiMiMapper.toEntity(returnedNextProductMiMiDTO);
    ...
    void putExistingNextProductMiMi() throws Exception {
        ...
        NextProductMiMiDTO nextProductMiMiDTO = nextProductMiMiMapper.toDto(updatedNextProductMiMi);

        restNextProductMiMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextProductMiMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextProductMiMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextProductMiMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductMiMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductMiMiDTO)))
    ...
    void patchNonExistingNextProductMiMi() throws Exception {
        ...
        restNextProductMiMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextProductMiMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextProductMiMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextProductMiMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductMiMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextProductMiMiDTO)))
            .andExpect(status().isMethodNotAllowed());
