...
class PaymentSigmaResourceIT {
    ...
    void createPaymentSigma() throws Exception {
        ...
        PaymentSigmaDTO paymentSigmaDTO = paymentSigmaMapper.toDto(paymentSigma);
        var returnedPaymentSigmaDTO = om.readValue(
            restPaymentSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            PaymentSigmaDTO.class
        );

        // Validate the PaymentSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedPaymentSigma = paymentSigmaMapper.toEntity(returnedPaymentSigmaDTO);
    ...
    void putExistingPaymentSigma() throws Exception {
        ...
        PaymentSigmaDTO paymentSigmaDTO = paymentSigmaMapper.toDto(updatedPaymentSigma);

        restPaymentSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paymentSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(paymentSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPaymentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentSigmaDTO)))
    ...
    void patchNonExistingPaymentSigma() throws Exception {
        ...
        restPaymentSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paymentSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(paymentSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaymentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(paymentSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
