...
class NextPaymentResourceIT {
    ...
    void createNextPayment() throws Exception {
        ...
        NextPaymentDTO nextPaymentDTO = nextPaymentMapper.toDto(nextPayment);
        var returnedNextPaymentDTO = om.readValue(
            restNextPaymentMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentDTO.class
        );

        // Validate the NextPayment in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPayment = nextPaymentMapper.toEntity(returnedNextPaymentDTO);
    ...
    void putExistingNextPayment() throws Exception {
        ...
        NextPaymentDTO nextPaymentDTO = nextPaymentMapper.toDto(updatedNextPayment);

        restNextPaymentMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPayment() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentDTO)))
    ...
    void patchNonExistingNextPayment() throws Exception {
        ...
        restNextPaymentMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPayment() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentDTO)))
            .andExpect(status().isMethodNotAllowed());
