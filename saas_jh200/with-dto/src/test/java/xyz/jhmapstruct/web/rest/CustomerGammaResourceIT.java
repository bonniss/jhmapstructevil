...
class CustomerGammaResourceIT {
    ...
    void createCustomerGamma() throws Exception {
        ...
        CustomerGammaDTO customerGammaDTO = customerGammaMapper.toDto(customerGamma);
        var returnedCustomerGammaDTO = om.readValue(
            restCustomerGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            CustomerGammaDTO.class
        );

        // Validate the CustomerGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedCustomerGamma = customerGammaMapper.toEntity(returnedCustomerGammaDTO);
    ...
    void putExistingCustomerGamma() throws Exception {
        ...
        updatedCustomerGamma
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        CustomerGammaDTO customerGammaDTO = customerGammaMapper.toDto(updatedCustomerGamma);

        restCustomerGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, customerGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(customerGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCustomerGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCustomerGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerGammaDTO)))
    ...
    void patchNonExistingCustomerGamma() throws Exception {
        ...
        restCustomerGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, customerGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(customerGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCustomerGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCustomerGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(customerGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
