...
class NextEmployeeMiResourceIT {
    ...
    void createNextEmployeeMi() throws Exception {
        ...
        NextEmployeeMiDTO nextEmployeeMiDTO = nextEmployeeMiMapper.toDto(nextEmployeeMi);
        var returnedNextEmployeeMiDTO = om.readValue(
            restNextEmployeeMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeMiDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextEmployeeMiDTO.class
        );

        // Validate the NextEmployeeMi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextEmployeeMi = nextEmployeeMiMapper.toEntity(returnedNextEmployeeMiDTO);
    ...
    void putExistingNextEmployeeMi() throws Exception {
        ...
        updatedNextEmployeeMi
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        NextEmployeeMiDTO nextEmployeeMiDTO = nextEmployeeMiMapper.toDto(updatedNextEmployeeMi);

        restNextEmployeeMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextEmployeeMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextEmployeeMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextEmployeeMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeMiDTO)))
    ...
    void patchNonExistingNextEmployeeMi() throws Exception {
        ...
        restNextEmployeeMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextEmployeeMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextEmployeeMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextEmployeeMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextEmployeeMiDTO)))
            .andExpect(status().isMethodNotAllowed());
