...
class CategoryViResourceIT {
    ...
    void createCategoryVi() throws Exception {
        ...
        CategoryViDTO categoryViDTO = categoryViMapper.toDto(categoryVi);
        var returnedCategoryViDTO = om.readValue(
            restCategoryViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categoryViDTO)))
        ...
        var returnedCategoryVi = categoryViMapper.toEntity(returnedCategoryViDTO);
    ...
    void putExistingCategoryVi() throws Exception {
        ...
        CategoryViDTO categoryViDTO = categoryViMapper.toDto(updatedCategoryVi);

        restCategoryViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, categoryViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(categoryViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCategoryVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategoryViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categoryViDTO)))
    ...
    void patchNonExistingCategoryVi() throws Exception {
        ...
        restCategoryViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, categoryViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(categoryViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCategoryVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategoryViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(categoryViDTO)))
            .andExpect(status().isMethodNotAllowed());
