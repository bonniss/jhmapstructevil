...
class PaymentGammaResourceIT {
    ...
    void createPaymentGamma() throws Exception {
        ...
        PaymentGammaDTO paymentGammaDTO = paymentGammaMapper.toDto(paymentGamma);
        var returnedPaymentGammaDTO = om.readValue(
            restPaymentGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            PaymentGammaDTO.class
        );

        // Validate the PaymentGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedPaymentGamma = paymentGammaMapper.toEntity(returnedPaymentGammaDTO);
    ...
    void putExistingPaymentGamma() throws Exception {
        ...
        PaymentGammaDTO paymentGammaDTO = paymentGammaMapper.toDto(updatedPaymentGamma);

        restPaymentGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paymentGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(paymentGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPaymentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentGammaDTO)))
    ...
    void patchNonExistingPaymentGamma() throws Exception {
        ...
        restPaymentGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paymentGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(paymentGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaymentGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(paymentGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
