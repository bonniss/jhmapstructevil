...
class ShipmentThetaResourceIT {
    ...
    void createShipmentTheta() throws Exception {
        ...
        ShipmentThetaDTO shipmentThetaDTO = shipmentThetaMapper.toDto(shipmentTheta);
        var returnedShipmentThetaDTO = om.readValue(
            restShipmentThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentThetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            ShipmentThetaDTO.class
        );

        // Validate the ShipmentTheta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedShipmentTheta = shipmentThetaMapper.toEntity(returnedShipmentThetaDTO);
    ...
    void putExistingShipmentTheta() throws Exception {
        ...
        ShipmentThetaDTO shipmentThetaDTO = shipmentThetaMapper.toDto(updatedShipmentTheta);

        restShipmentThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, shipmentThetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(shipmentThetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamShipmentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentThetaDTO)))
    ...
    void patchNonExistingShipmentTheta() throws Exception {
        ...
        restShipmentThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, shipmentThetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(shipmentThetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamShipmentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(shipmentThetaDTO)))
            .andExpect(status().isMethodNotAllowed());
