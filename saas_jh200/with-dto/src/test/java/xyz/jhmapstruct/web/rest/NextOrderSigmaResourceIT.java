...
class NextOrderSigmaResourceIT {
    ...
    void createNextOrderSigma() throws Exception {
        ...
        NextOrderSigmaDTO nextOrderSigmaDTO = nextOrderSigmaMapper.toDto(nextOrderSigma);
        var returnedNextOrderSigmaDTO = om.readValue(
            restNextOrderSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextOrderSigmaDTO.class
        );

        // Validate the NextOrderSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextOrderSigma = nextOrderSigmaMapper.toEntity(returnedNextOrderSigmaDTO);
    ...
    void putExistingNextOrderSigma() throws Exception {
        ...
        NextOrderSigmaDTO nextOrderSigmaDTO = nextOrderSigmaMapper.toDto(updatedNextOrderSigma);

        restNextOrderSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextOrderSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderSigmaDTO)))
    ...
    void patchNonExistingNextOrderSigma() throws Exception {
        ...
        restNextOrderSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
