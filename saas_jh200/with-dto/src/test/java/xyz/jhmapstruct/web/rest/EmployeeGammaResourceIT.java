...
class EmployeeGammaResourceIT {
    ...
    void createEmployeeGamma() throws Exception {
        ...
        EmployeeGammaDTO employeeGammaDTO = employeeGammaMapper.toDto(employeeGamma);
        var returnedEmployeeGammaDTO = om.readValue(
            restEmployeeGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            EmployeeGammaDTO.class
        );

        // Validate the EmployeeGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedEmployeeGamma = employeeGammaMapper.toEntity(returnedEmployeeGammaDTO);
    ...
    void putExistingEmployeeGamma() throws Exception {
        ...
        updatedEmployeeGamma
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        EmployeeGammaDTO employeeGammaDTO = employeeGammaMapper.toDto(updatedEmployeeGamma);

        restEmployeeGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, employeeGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(employeeGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamEmployeeGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeGammaDTO)))
    ...
    void patchNonExistingEmployeeGamma() throws Exception {
        ...
        restEmployeeGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, employeeGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(employeeGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEmployeeGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(employeeGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
