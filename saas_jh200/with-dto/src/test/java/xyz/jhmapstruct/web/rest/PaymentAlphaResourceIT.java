...
class PaymentAlphaResourceIT {
    ...
    void createPaymentAlpha() throws Exception {
        ...
        PaymentAlphaDTO paymentAlphaDTO = paymentAlphaMapper.toDto(paymentAlpha);
        var returnedPaymentAlphaDTO = om.readValue(
            restPaymentAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            PaymentAlphaDTO.class
        );

        // Validate the PaymentAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedPaymentAlpha = paymentAlphaMapper.toEntity(returnedPaymentAlphaDTO);
    ...
    void putExistingPaymentAlpha() throws Exception {
        ...
        PaymentAlphaDTO paymentAlphaDTO = paymentAlphaMapper.toDto(updatedPaymentAlpha);

        restPaymentAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paymentAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(paymentAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPaymentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentAlphaDTO)))
    ...
    void patchNonExistingPaymentAlpha() throws Exception {
        ...
        restPaymentAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paymentAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(paymentAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaymentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(paymentAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
