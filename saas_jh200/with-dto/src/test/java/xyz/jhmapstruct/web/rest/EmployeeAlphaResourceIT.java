...
class EmployeeAlphaResourceIT {
    ...
    void createEmployeeAlpha() throws Exception {
        ...
        EmployeeAlphaDTO employeeAlphaDTO = employeeAlphaMapper.toDto(employeeAlpha);
        var returnedEmployeeAlphaDTO = om.readValue(
            restEmployeeAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            EmployeeAlphaDTO.class
        );

        // Validate the EmployeeAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedEmployeeAlpha = employeeAlphaMapper.toEntity(returnedEmployeeAlphaDTO);
    ...
    void putExistingEmployeeAlpha() throws Exception {
        ...
        updatedEmployeeAlpha
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        EmployeeAlphaDTO employeeAlphaDTO = employeeAlphaMapper.toDto(updatedEmployeeAlpha);

        restEmployeeAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, employeeAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(employeeAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamEmployeeAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeAlphaDTO)))
    ...
    void patchNonExistingEmployeeAlpha() throws Exception {
        ...
        restEmployeeAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, employeeAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(employeeAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEmployeeAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(employeeAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
