...
class ProductBetaResourceIT {
    ...
    void createProductBeta() throws Exception {
        ...
        ProductBetaDTO productBetaDTO = productBetaMapper.toDto(productBeta);
        var returnedProductBetaDTO = om.readValue(
            restProductBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productBetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            ProductBetaDTO.class
        );

        // Validate the ProductBeta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedProductBeta = productBetaMapper.toEntity(returnedProductBetaDTO);
    ...
    void putExistingProductBeta() throws Exception {
        ...
        ProductBetaDTO productBetaDTO = productBetaMapper.toDto(updatedProductBeta);

        restProductBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, productBetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(productBetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamProductBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productBetaDTO)))
    ...
    void patchNonExistingProductBeta() throws Exception {
        ...
        restProductBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, productBetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(productBetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamProductBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(productBetaDTO)))
            .andExpect(status().isMethodNotAllowed());
