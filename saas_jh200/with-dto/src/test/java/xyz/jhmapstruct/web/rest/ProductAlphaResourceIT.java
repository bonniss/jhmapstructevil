...
class ProductAlphaResourceIT {
    ...
    void createProductAlpha() throws Exception {
        ...
        ProductAlphaDTO productAlphaDTO = productAlphaMapper.toDto(productAlpha);
        var returnedProductAlphaDTO = om.readValue(
            restProductAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            ProductAlphaDTO.class
        );

        // Validate the ProductAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedProductAlpha = productAlphaMapper.toEntity(returnedProductAlphaDTO);
    ...
    void putExistingProductAlpha() throws Exception {
        ...
        ProductAlphaDTO productAlphaDTO = productAlphaMapper.toDto(updatedProductAlpha);

        restProductAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, productAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(productAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamProductAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productAlphaDTO)))
    ...
    void patchNonExistingProductAlpha() throws Exception {
        ...
        restProductAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, productAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(productAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamProductAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(productAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
