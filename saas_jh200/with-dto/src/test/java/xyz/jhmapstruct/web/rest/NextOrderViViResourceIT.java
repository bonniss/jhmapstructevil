...
class NextOrderViViResourceIT {
    ...
    void createNextOrderViVi() throws Exception {
        ...
        NextOrderViViDTO nextOrderViViDTO = nextOrderViViMapper.toDto(nextOrderViVi);
        var returnedNextOrderViViDTO = om.readValue(
            restNextOrderViViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderViViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextOrderViViDTO.class
        );

        // Validate the NextOrderViVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextOrderViVi = nextOrderViViMapper.toEntity(returnedNextOrderViViDTO);
    ...
    void putExistingNextOrderViVi() throws Exception {
        ...
        NextOrderViViDTO nextOrderViViDTO = nextOrderViViMapper.toDto(updatedNextOrderViVi);

        restNextOrderViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderViViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderViViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextOrderViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderViViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderViViDTO)))
    ...
    void patchNonExistingNextOrderViVi() throws Exception {
        ...
        restNextOrderViViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderViViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderViViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderViViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderViViDTO)))
            .andExpect(status().isMethodNotAllowed());
