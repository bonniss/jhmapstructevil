...
class NextEmployeeAlphaResourceIT {
    ...
    void createNextEmployeeAlpha() throws Exception {
        ...
        NextEmployeeAlphaDTO nextEmployeeAlphaDTO = nextEmployeeAlphaMapper.toDto(nextEmployeeAlpha);
        var returnedNextEmployeeAlphaDTO = om.readValue(
            restNextEmployeeAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextEmployeeAlphaDTO.class
        );

        // Validate the NextEmployeeAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextEmployeeAlpha = nextEmployeeAlphaMapper.toEntity(returnedNextEmployeeAlphaDTO);
    ...
    void putExistingNextEmployeeAlpha() throws Exception {
        ...
        updatedNextEmployeeAlpha
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        NextEmployeeAlphaDTO nextEmployeeAlphaDTO = nextEmployeeAlphaMapper.toDto(updatedNextEmployeeAlpha);

        restNextEmployeeAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextEmployeeAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextEmployeeAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextEmployeeAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeAlphaDTO)))
    ...
    void patchNonExistingNextEmployeeAlpha() throws Exception {
        ...
        restNextEmployeeAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextEmployeeAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextEmployeeAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextEmployeeAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextEmployeeAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
