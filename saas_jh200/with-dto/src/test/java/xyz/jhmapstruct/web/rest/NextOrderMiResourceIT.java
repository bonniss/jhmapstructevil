...
class NextOrderMiResourceIT {
    ...
    void createNextOrderMi() throws Exception {
        ...
        NextOrderMiDTO nextOrderMiDTO = nextOrderMiMapper.toDto(nextOrderMi);
        var returnedNextOrderMiDTO = om.readValue(
            restNextOrderMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderMiDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextOrderMiDTO.class
        );

        // Validate the NextOrderMi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextOrderMi = nextOrderMiMapper.toEntity(returnedNextOrderMiDTO);
    ...
    void putExistingNextOrderMi() throws Exception {
        ...
        NextOrderMiDTO nextOrderMiDTO = nextOrderMiMapper.toDto(updatedNextOrderMi);

        restNextOrderMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextOrderMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderMiDTO)))
    ...
    void patchNonExistingNextOrderMi() throws Exception {
        ...
        restNextOrderMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderMiDTO)))
            .andExpect(status().isMethodNotAllowed());
