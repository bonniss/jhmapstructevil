...
class PaymentThetaResourceIT {
    ...
    void createPaymentTheta() throws Exception {
        ...
        PaymentThetaDTO paymentThetaDTO = paymentThetaMapper.toDto(paymentTheta);
        var returnedPaymentThetaDTO = om.readValue(
            restPaymentThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentThetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            PaymentThetaDTO.class
        );

        // Validate the PaymentTheta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedPaymentTheta = paymentThetaMapper.toEntity(returnedPaymentThetaDTO);
    ...
    void putExistingPaymentTheta() throws Exception {
        ...
        PaymentThetaDTO paymentThetaDTO = paymentThetaMapper.toDto(updatedPaymentTheta);

        restPaymentThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paymentThetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(paymentThetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPaymentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentThetaDTO)))
    ...
    void patchNonExistingPaymentTheta() throws Exception {
        ...
        restPaymentThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paymentThetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(paymentThetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaymentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(paymentThetaDTO)))
            .andExpect(status().isMethodNotAllowed());
