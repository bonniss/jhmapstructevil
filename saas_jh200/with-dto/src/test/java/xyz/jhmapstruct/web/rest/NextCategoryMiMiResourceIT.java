...
class NextCategoryMiMiResourceIT {
    ...
    void createNextCategoryMiMi() throws Exception {
        ...
        NextCategoryMiMiDTO nextCategoryMiMiDTO = nextCategoryMiMiMapper.toDto(nextCategoryMiMi);
        var returnedNextCategoryMiMiDTO = om.readValue(
            restNextCategoryMiMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryMiMiDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCategoryMiMiDTO.class
        );

        // Validate the NextCategoryMiMi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCategoryMiMi = nextCategoryMiMiMapper.toEntity(returnedNextCategoryMiMiDTO);
    ...
    void putExistingNextCategoryMiMi() throws Exception {
        ...
        NextCategoryMiMiDTO nextCategoryMiMiDTO = nextCategoryMiMiMapper.toDto(updatedNextCategoryMiMi);

        restNextCategoryMiMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCategoryMiMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCategoryMiMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCategoryMiMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryMiMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryMiMiDTO)))
    ...
    void patchNonExistingNextCategoryMiMi() throws Exception {
        ...
        restNextCategoryMiMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCategoryMiMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCategoryMiMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCategoryMiMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryMiMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCategoryMiMiDTO)))
            .andExpect(status().isMethodNotAllowed());
