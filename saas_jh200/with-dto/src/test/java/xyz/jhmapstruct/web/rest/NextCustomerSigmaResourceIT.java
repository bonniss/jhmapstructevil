...
class NextCustomerSigmaResourceIT {
    ...
    void createNextCustomerSigma() throws Exception {
        ...
        NextCustomerSigmaDTO nextCustomerSigmaDTO = nextCustomerSigmaMapper.toDto(nextCustomerSigma);
        var returnedNextCustomerSigmaDTO = om.readValue(
            restNextCustomerSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerSigmaDTO.class
        );

        // Validate the NextCustomerSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCustomerSigma = nextCustomerSigmaMapper.toEntity(returnedNextCustomerSigmaDTO);
    ...
    void putExistingNextCustomerSigma() throws Exception {
        ...
        updatedNextCustomerSigma
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextCustomerSigmaDTO nextCustomerSigmaDTO = nextCustomerSigmaMapper.toDto(updatedNextCustomerSigma);

        restNextCustomerSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCustomerSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerSigmaDTO)))
    ...
    void patchNonExistingNextCustomerSigma() throws Exception {
        ...
        restNextCustomerSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
