...
class NextOrderBetaResourceIT {
    ...
    void createNextOrderBeta() throws Exception {
        ...
        NextOrderBetaDTO nextOrderBetaDTO = nextOrderBetaMapper.toDto(nextOrderBeta);
        var returnedNextOrderBetaDTO = om.readValue(
            restNextOrderBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderBetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextOrderBetaDTO.class
        );

        // Validate the NextOrderBeta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextOrderBeta = nextOrderBetaMapper.toEntity(returnedNextOrderBetaDTO);
    ...
    void putExistingNextOrderBeta() throws Exception {
        ...
        NextOrderBetaDTO nextOrderBetaDTO = nextOrderBetaMapper.toDto(updatedNextOrderBeta);

        restNextOrderBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextOrderBetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextOrderBetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextOrderBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextOrderBetaDTO)))
    ...
    void patchNonExistingNextOrderBeta() throws Exception {
        ...
        restNextOrderBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextOrderBetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextOrderBetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextOrderBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextOrderBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextOrderBetaDTO)))
            .andExpect(status().isMethodNotAllowed());
