...
class ReviewMiResourceIT {
    ...
    void createReviewMi() throws Exception {
        ...
        // Create the ReviewMi
        ReviewMiDTO reviewMiDTO = reviewMiMapper.toDto(reviewMi);
        var returnedReviewMiDTO = om.readValue(
            restReviewMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewMiDTO)))
        ...
        var returnedReviewMi = reviewMiMapper.toEntity(returnedReviewMiDTO);
    ...
    void putExistingReviewMi() throws Exception {
        ...
        ReviewMiDTO reviewMiDTO = reviewMiMapper.toDto(updatedReviewMi);

        restReviewMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, reviewMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(reviewMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamReviewMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(reviewMiDTO)))
    ...
    void patchNonExistingReviewMi() throws Exception {
        ...
        restReviewMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, reviewMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(reviewMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamReviewMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(reviewMiDTO)))
            .andExpect(status().isMethodNotAllowed());
