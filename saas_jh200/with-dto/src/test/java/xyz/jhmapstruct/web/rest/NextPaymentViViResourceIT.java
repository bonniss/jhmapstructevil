...
class NextPaymentViViResourceIT {
    ...
    void createNextPaymentViVi() throws Exception {
        ...
        NextPaymentViViDTO nextPaymentViViDTO = nextPaymentViViMapper.toDto(nextPaymentViVi);
        var returnedNextPaymentViViDTO = om.readValue(
            restNextPaymentViViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentViViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentViViDTO.class
        );

        // Validate the NextPaymentViVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPaymentViVi = nextPaymentViViMapper.toEntity(returnedNextPaymentViViDTO);
    ...
    void putExistingNextPaymentViVi() throws Exception {
        ...
        NextPaymentViViDTO nextPaymentViViDTO = nextPaymentViViMapper.toDto(updatedNextPaymentViVi);

        restNextPaymentViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentViViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentViViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPaymentViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentViViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentViViDTO)))
    ...
    void patchNonExistingNextPaymentViVi() throws Exception {
        ...
        restNextPaymentViViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentViViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentViViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentViViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentViViDTO)))
            .andExpect(status().isMethodNotAllowed());
