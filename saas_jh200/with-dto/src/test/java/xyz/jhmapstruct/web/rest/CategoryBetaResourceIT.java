...
class CategoryBetaResourceIT {
    ...
    void createCategoryBeta() throws Exception {
        ...
        CategoryBetaDTO categoryBetaDTO = categoryBetaMapper.toDto(categoryBeta);
        var returnedCategoryBetaDTO = om.readValue(
            restCategoryBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categoryBetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            CategoryBetaDTO.class
        );

        // Validate the CategoryBeta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedCategoryBeta = categoryBetaMapper.toEntity(returnedCategoryBetaDTO);
    ...
    void putExistingCategoryBeta() throws Exception {
        ...
        CategoryBetaDTO categoryBetaDTO = categoryBetaMapper.toDto(updatedCategoryBeta);

        restCategoryBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, categoryBetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(categoryBetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCategoryBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategoryBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categoryBetaDTO)))
    ...
    void patchNonExistingCategoryBeta() throws Exception {
        ...
        restCategoryBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, categoryBetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(categoryBetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCategoryBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategoryBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(categoryBetaDTO)))
            .andExpect(status().isMethodNotAllowed());
