...
class NextPaymentMiMiResourceIT {
    ...
    void createNextPaymentMiMi() throws Exception {
        ...
        NextPaymentMiMiDTO nextPaymentMiMiDTO = nextPaymentMiMiMapper.toDto(nextPaymentMiMi);
        var returnedNextPaymentMiMiDTO = om.readValue(
            restNextPaymentMiMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentMiMiDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentMiMiDTO.class
        );

        // Validate the NextPaymentMiMi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPaymentMiMi = nextPaymentMiMiMapper.toEntity(returnedNextPaymentMiMiDTO);
    ...
    void putExistingNextPaymentMiMi() throws Exception {
        ...
        NextPaymentMiMiDTO nextPaymentMiMiDTO = nextPaymentMiMiMapper.toDto(updatedNextPaymentMiMi);

        restNextPaymentMiMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentMiMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentMiMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPaymentMiMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentMiMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentMiMiDTO)))
    ...
    void patchNonExistingNextPaymentMiMi() throws Exception {
        ...
        restNextPaymentMiMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentMiMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentMiMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentMiMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentMiMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentMiMiDTO)))
            .andExpect(status().isMethodNotAllowed());
