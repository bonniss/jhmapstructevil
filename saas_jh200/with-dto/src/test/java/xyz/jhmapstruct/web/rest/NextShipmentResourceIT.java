...
class NextShipmentResourceIT {
    ...
    void createNextShipment() throws Exception {
        ...
        NextShipmentDTO nextShipmentDTO = nextShipmentMapper.toDto(nextShipment);
        var returnedNextShipmentDTO = om.readValue(
            restNextShipmentMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentDTO.class
        );

        // Validate the NextShipment in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextShipment = nextShipmentMapper.toEntity(returnedNextShipmentDTO);
    ...
    void putExistingNextShipment() throws Exception {
        ...
        NextShipmentDTO nextShipmentDTO = nextShipmentMapper.toDto(updatedNextShipment);

        restNextShipmentMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextShipment() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentDTO)))
    ...
    void patchNonExistingNextShipment() throws Exception {
        ...
        restNextShipmentMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipment() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentDTO)))
            .andExpect(status().isMethodNotAllowed());
