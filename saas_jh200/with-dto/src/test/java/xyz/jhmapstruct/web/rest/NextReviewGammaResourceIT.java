...
class NextReviewGammaResourceIT {
    ...
    void createNextReviewGamma() throws Exception {
        ...
        NextReviewGammaDTO nextReviewGammaDTO = nextReviewGammaMapper.toDto(nextReviewGamma);
        var returnedNextReviewGammaDTO = om.readValue(
            restNextReviewGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextReviewGammaDTO.class
        );

        // Validate the NextReviewGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextReviewGamma = nextReviewGammaMapper.toEntity(returnedNextReviewGammaDTO);
    ...
    void putExistingNextReviewGamma() throws Exception {
        ...
        NextReviewGammaDTO nextReviewGammaDTO = nextReviewGammaMapper.toDto(updatedNextReviewGamma);

        restNextReviewGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextReviewGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextReviewGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextReviewGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewGammaDTO)))
    ...
    void patchNonExistingNextReviewGamma() throws Exception {
        ...
        restNextReviewGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextReviewGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextReviewGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextReviewGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextReviewGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
