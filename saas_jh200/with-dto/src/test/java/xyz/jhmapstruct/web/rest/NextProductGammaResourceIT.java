...
class NextProductGammaResourceIT {
    ...
    void createNextProductGamma() throws Exception {
        ...
        NextProductGammaDTO nextProductGammaDTO = nextProductGammaMapper.toDto(nextProductGamma);
        var returnedNextProductGammaDTO = om.readValue(
            restNextProductGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextProductGammaDTO.class
        );

        // Validate the NextProductGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextProductGamma = nextProductGammaMapper.toEntity(returnedNextProductGammaDTO);
    ...
    void putExistingNextProductGamma() throws Exception {
        ...
        NextProductGammaDTO nextProductGammaDTO = nextProductGammaMapper.toDto(updatedNextProductGamma);

        restNextProductGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextProductGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextProductGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextProductGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductGammaDTO)))
    ...
    void patchNonExistingNextProductGamma() throws Exception {
        ...
        restNextProductGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextProductGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextProductGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextProductGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextProductGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
