...
class NextCustomerViViResourceIT {
    ...
    void createNextCustomerViVi() throws Exception {
        ...
        NextCustomerViViDTO nextCustomerViViDTO = nextCustomerViViMapper.toDto(nextCustomerViVi);
        var returnedNextCustomerViViDTO = om.readValue(
            restNextCustomerViViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerViViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerViViDTO.class
        );

        // Validate the NextCustomerViVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCustomerViVi = nextCustomerViViMapper.toEntity(returnedNextCustomerViViDTO);
    ...
    void putExistingNextCustomerViVi() throws Exception {
        ...
        updatedNextCustomerViVi
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextCustomerViViDTO nextCustomerViViDTO = nextCustomerViViMapper.toDto(updatedNextCustomerViVi);

        restNextCustomerViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerViViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerViViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCustomerViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerViViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerViViDTO)))
    ...
    void patchNonExistingNextCustomerViVi() throws Exception {
        ...
        restNextCustomerViViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerViViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerViViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerViViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerViViDTO)))
            .andExpect(status().isMethodNotAllowed());
