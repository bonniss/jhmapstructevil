...
class CategorySigmaResourceIT {
    ...
    void createCategorySigma() throws Exception {
        ...
        CategorySigmaDTO categorySigmaDTO = categorySigmaMapper.toDto(categorySigma);
        var returnedCategorySigmaDTO = om.readValue(
            restCategorySigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categorySigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            CategorySigmaDTO.class
        );

        // Validate the CategorySigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedCategorySigma = categorySigmaMapper.toEntity(returnedCategorySigmaDTO);
    ...
    void putExistingCategorySigma() throws Exception {
        ...
        CategorySigmaDTO categorySigmaDTO = categorySigmaMapper.toDto(updatedCategorySigma);

        restCategorySigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, categorySigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(categorySigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCategorySigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategorySigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categorySigmaDTO)))
    ...
    void patchNonExistingCategorySigma() throws Exception {
        ...
        restCategorySigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, categorySigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(categorySigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCategorySigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategorySigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(categorySigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
