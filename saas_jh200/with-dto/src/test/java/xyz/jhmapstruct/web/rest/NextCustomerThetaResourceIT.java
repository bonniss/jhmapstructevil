...
class NextCustomerThetaResourceIT {
    ...
    void createNextCustomerTheta() throws Exception {
        ...
        NextCustomerThetaDTO nextCustomerThetaDTO = nextCustomerThetaMapper.toDto(nextCustomerTheta);
        var returnedNextCustomerThetaDTO = om.readValue(
            restNextCustomerThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerThetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerThetaDTO.class
        );

        // Validate the NextCustomerTheta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCustomerTheta = nextCustomerThetaMapper.toEntity(returnedNextCustomerThetaDTO);
    ...
    void putExistingNextCustomerTheta() throws Exception {
        ...
        updatedNextCustomerTheta
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextCustomerThetaDTO nextCustomerThetaDTO = nextCustomerThetaMapper.toDto(updatedNextCustomerTheta);

        restNextCustomerThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerThetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerThetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCustomerTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerThetaDTO)))
    ...
    void patchNonExistingNextCustomerTheta() throws Exception {
        ...
        restNextCustomerThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerThetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerThetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerThetaDTO)))
            .andExpect(status().isMethodNotAllowed());
