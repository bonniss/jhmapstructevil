...
class InvoiceGammaResourceIT {
    ...
    void createInvoiceGamma() throws Exception {
        ...
        InvoiceGammaDTO invoiceGammaDTO = invoiceGammaMapper.toDto(invoiceGamma);
        var returnedInvoiceGammaDTO = om.readValue(
            restInvoiceGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            InvoiceGammaDTO.class
        );

        // Validate the InvoiceGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedInvoiceGamma = invoiceGammaMapper.toEntity(returnedInvoiceGammaDTO);
    ...
    void putExistingInvoiceGamma() throws Exception {
        ...
        updatedInvoiceGamma
            ...
            .dueDate(UPDATED_DUE_DATE)
            .amount(UPDATED_AMOUNT);
        InvoiceGammaDTO invoiceGammaDTO = invoiceGammaMapper.toDto(updatedInvoiceGamma);

        restInvoiceGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, invoiceGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(invoiceGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamInvoiceGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceGammaDTO)))
    ...
    void patchNonExistingInvoiceGamma() throws Exception {
        ...
        restInvoiceGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, invoiceGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(invoiceGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamInvoiceGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(invoiceGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
