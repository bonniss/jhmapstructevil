...
class NextSupplierSigmaResourceIT {
    ...
    void createNextSupplierSigma() throws Exception {
        ...
        NextSupplierSigmaDTO nextSupplierSigmaDTO = nextSupplierSigmaMapper.toDto(nextSupplierSigma);
        var returnedNextSupplierSigmaDTO = om.readValue(
            restNextSupplierSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextSupplierSigmaDTO.class
        );

        // Validate the NextSupplierSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextSupplierSigma = nextSupplierSigmaMapper.toEntity(returnedNextSupplierSigmaDTO);
    ...
    void putExistingNextSupplierSigma() throws Exception {
        ...
        updatedNextSupplierSigma
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextSupplierSigmaDTO nextSupplierSigmaDTO = nextSupplierSigmaMapper.toDto(updatedNextSupplierSigma);

        restNextSupplierSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextSupplierSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextSupplierSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextSupplierSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierSigmaDTO)))
    ...
    void patchNonExistingNextSupplierSigma() throws Exception {
        ...
        restNextSupplierSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextSupplierSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextSupplierSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextSupplierSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextSupplierSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
