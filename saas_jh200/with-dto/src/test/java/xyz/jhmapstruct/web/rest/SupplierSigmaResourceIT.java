...
class SupplierSigmaResourceIT {
    ...
    void createSupplierSigma() throws Exception {
        ...
        SupplierSigmaDTO supplierSigmaDTO = supplierSigmaMapper.toDto(supplierSigma);
        var returnedSupplierSigmaDTO = om.readValue(
            restSupplierSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(supplierSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            SupplierSigmaDTO.class
        );

        // Validate the SupplierSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedSupplierSigma = supplierSigmaMapper.toEntity(returnedSupplierSigmaDTO);
    ...
    void putExistingSupplierSigma() throws Exception {
        ...
        updatedSupplierSigma
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        SupplierSigmaDTO supplierSigmaDTO = supplierSigmaMapper.toDto(updatedSupplierSigma);

        restSupplierSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, supplierSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(supplierSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamSupplierSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSupplierSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(supplierSigmaDTO)))
    ...
    void patchNonExistingSupplierSigma() throws Exception {
        ...
        restSupplierSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, supplierSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(supplierSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSupplierSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSupplierSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(supplierSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
