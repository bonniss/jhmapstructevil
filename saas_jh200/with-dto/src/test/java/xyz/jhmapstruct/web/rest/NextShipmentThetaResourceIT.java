...
class NextShipmentThetaResourceIT {
    ...
    void createNextShipmentTheta() throws Exception {
        ...
        NextShipmentThetaDTO nextShipmentThetaDTO = nextShipmentThetaMapper.toDto(nextShipmentTheta);
        var returnedNextShipmentThetaDTO = om.readValue(
            restNextShipmentThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentThetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextShipmentThetaDTO.class
        );

        // Validate the NextShipmentTheta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextShipmentTheta = nextShipmentThetaMapper.toEntity(returnedNextShipmentThetaDTO);
    ...
    void putExistingNextShipmentTheta() throws Exception {
        ...
        updatedNextShipmentTheta
            ...
            .shippedDate(UPDATED_SHIPPED_DATE)
            .deliveryDate(UPDATED_DELIVERY_DATE);
        NextShipmentThetaDTO nextShipmentThetaDTO = nextShipmentThetaMapper.toDto(updatedNextShipmentTheta);

        restNextShipmentThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextShipmentThetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextShipmentThetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextShipmentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextShipmentThetaDTO)))
    ...
    void patchNonExistingNextShipmentTheta() throws Exception {
        ...
        restNextShipmentThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextShipmentThetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextShipmentThetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextShipmentTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextShipmentThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextShipmentThetaDTO)))
            .andExpect(status().isMethodNotAllowed());
