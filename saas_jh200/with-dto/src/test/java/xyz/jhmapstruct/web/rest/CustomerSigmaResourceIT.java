...
class CustomerSigmaResourceIT {
    ...
    void createCustomerSigma() throws Exception {
        ...
        CustomerSigmaDTO customerSigmaDTO = customerSigmaMapper.toDto(customerSigma);
        var returnedCustomerSigmaDTO = om.readValue(
            restCustomerSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            CustomerSigmaDTO.class
        );

        // Validate the CustomerSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedCustomerSigma = customerSigmaMapper.toEntity(returnedCustomerSigmaDTO);
    ...
    void putExistingCustomerSigma() throws Exception {
        ...
        updatedCustomerSigma
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        CustomerSigmaDTO customerSigmaDTO = customerSigmaMapper.toDto(updatedCustomerSigma);

        restCustomerSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, customerSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(customerSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCustomerSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCustomerSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(customerSigmaDTO)))
    ...
    void patchNonExistingCustomerSigma() throws Exception {
        ...
        restCustomerSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, customerSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(customerSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCustomerSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCustomerSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(customerSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
