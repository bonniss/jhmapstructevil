...
class NextCategoryViResourceIT {
    ...
    void createNextCategoryVi() throws Exception {
        ...
        NextCategoryViDTO nextCategoryViDTO = nextCategoryViMapper.toDto(nextCategoryVi);
        var returnedNextCategoryViDTO = om.readValue(
            restNextCategoryViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCategoryViDTO.class
        );

        // Validate the NextCategoryVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCategoryVi = nextCategoryViMapper.toEntity(returnedNextCategoryViDTO);
    ...
    void putExistingNextCategoryVi() throws Exception {
        ...
        NextCategoryViDTO nextCategoryViDTO = nextCategoryViMapper.toDto(updatedNextCategoryVi);

        restNextCategoryViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCategoryViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCategoryViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCategoryVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryViDTO)))
    ...
    void patchNonExistingNextCategoryVi() throws Exception {
        ...
        restNextCategoryViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCategoryViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCategoryViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCategoryVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCategoryViDTO)))
            .andExpect(status().isMethodNotAllowed());
