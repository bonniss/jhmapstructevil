...
class NextCategoryGammaResourceIT {
    ...
    void createNextCategoryGamma() throws Exception {
        ...
        NextCategoryGammaDTO nextCategoryGammaDTO = nextCategoryGammaMapper.toDto(nextCategoryGamma);
        var returnedNextCategoryGammaDTO = om.readValue(
            restNextCategoryGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCategoryGammaDTO.class
        );

        // Validate the NextCategoryGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCategoryGamma = nextCategoryGammaMapper.toEntity(returnedNextCategoryGammaDTO);
    ...
    void putExistingNextCategoryGamma() throws Exception {
        ...
        NextCategoryGammaDTO nextCategoryGammaDTO = nextCategoryGammaMapper.toDto(updatedNextCategoryGamma);

        restNextCategoryGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCategoryGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCategoryGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCategoryGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryGammaDTO)))
    ...
    void patchNonExistingNextCategoryGamma() throws Exception {
        ...
        restNextCategoryGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCategoryGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCategoryGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCategoryGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCategoryGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
