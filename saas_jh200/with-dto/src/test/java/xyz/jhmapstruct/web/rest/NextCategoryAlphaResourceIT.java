...
class NextCategoryAlphaResourceIT {
    ...
    void createNextCategoryAlpha() throws Exception {
        ...
        NextCategoryAlphaDTO nextCategoryAlphaDTO = nextCategoryAlphaMapper.toDto(nextCategoryAlpha);
        var returnedNextCategoryAlphaDTO = om.readValue(
            restNextCategoryAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCategoryAlphaDTO.class
        );

        // Validate the NextCategoryAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCategoryAlpha = nextCategoryAlphaMapper.toEntity(returnedNextCategoryAlphaDTO);
    ...
    void putExistingNextCategoryAlpha() throws Exception {
        ...
        NextCategoryAlphaDTO nextCategoryAlphaDTO = nextCategoryAlphaMapper.toDto(updatedNextCategoryAlpha);

        restNextCategoryAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCategoryAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCategoryAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCategoryAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryAlphaDTO)))
    ...
    void patchNonExistingNextCategoryAlpha() throws Exception {
        ...
        restNextCategoryAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCategoryAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCategoryAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCategoryAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCategoryAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
