...
class ShipmentSigmaResourceIT {
    ...
    void createShipmentSigma() throws Exception {
        ...
        ShipmentSigmaDTO shipmentSigmaDTO = shipmentSigmaMapper.toDto(shipmentSigma);
        var returnedShipmentSigmaDTO = om.readValue(
            restShipmentSigmaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentSigmaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            ShipmentSigmaDTO.class
        );

        // Validate the ShipmentSigma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedShipmentSigma = shipmentSigmaMapper.toEntity(returnedShipmentSigmaDTO);
    ...
    void putExistingShipmentSigma() throws Exception {
        ...
        ShipmentSigmaDTO shipmentSigmaDTO = shipmentSigmaMapper.toDto(updatedShipmentSigma);

        restShipmentSigmaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, shipmentSigmaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(shipmentSigmaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamShipmentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentSigmaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentSigmaDTO)))
    ...
    void patchNonExistingShipmentSigma() throws Exception {
        ...
        restShipmentSigmaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, shipmentSigmaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(shipmentSigmaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamShipmentSigma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentSigmaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(shipmentSigmaDTO)))
            .andExpect(status().isMethodNotAllowed());
