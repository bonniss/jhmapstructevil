...
class ShipmentViResourceIT {
    ...
    void createShipmentVi() throws Exception {
        ...
        ShipmentViDTO shipmentViDTO = shipmentViMapper.toDto(shipmentVi);
        var returnedShipmentViDTO = om.readValue(
            restShipmentViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentViDTO)))
        ...
        var returnedShipmentVi = shipmentViMapper.toEntity(returnedShipmentViDTO);
    ...
    void putExistingShipmentVi() throws Exception {
        ...
        ShipmentViDTO shipmentViDTO = shipmentViMapper.toDto(updatedShipmentVi);

        restShipmentViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, shipmentViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(shipmentViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamShipmentVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(shipmentViDTO)))
    ...
    void patchNonExistingShipmentVi() throws Exception {
        ...
        restShipmentViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, shipmentViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(shipmentViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamShipmentVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restShipmentViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(shipmentViDTO)))
            .andExpect(status().isMethodNotAllowed());
