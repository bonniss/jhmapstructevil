...
class CategoryAlphaResourceIT {
    ...
    void createCategoryAlpha() throws Exception {
        ...
        CategoryAlphaDTO categoryAlphaDTO = categoryAlphaMapper.toDto(categoryAlpha);
        var returnedCategoryAlphaDTO = om.readValue(
            restCategoryAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categoryAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            CategoryAlphaDTO.class
        );

        // Validate the CategoryAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedCategoryAlpha = categoryAlphaMapper.toEntity(returnedCategoryAlphaDTO);
    ...
    void putExistingCategoryAlpha() throws Exception {
        ...
        CategoryAlphaDTO categoryAlphaDTO = categoryAlphaMapper.toDto(updatedCategoryAlpha);

        restCategoryAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, categoryAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(categoryAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamCategoryAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategoryAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(categoryAlphaDTO)))
    ...
    void patchNonExistingCategoryAlpha() throws Exception {
        ...
        restCategoryAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, categoryAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(categoryAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamCategoryAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCategoryAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(categoryAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
