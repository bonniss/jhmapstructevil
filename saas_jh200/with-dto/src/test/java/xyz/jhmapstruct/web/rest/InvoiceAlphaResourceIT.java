...
class InvoiceAlphaResourceIT {
    ...
    void createInvoiceAlpha() throws Exception {
        ...
        InvoiceAlphaDTO invoiceAlphaDTO = invoiceAlphaMapper.toDto(invoiceAlpha);
        var returnedInvoiceAlphaDTO = om.readValue(
            restInvoiceAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            InvoiceAlphaDTO.class
        );

        // Validate the InvoiceAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedInvoiceAlpha = invoiceAlphaMapper.toEntity(returnedInvoiceAlphaDTO);
    ...
    void putExistingInvoiceAlpha() throws Exception {
        ...
        updatedInvoiceAlpha
            ...
            .dueDate(UPDATED_DUE_DATE)
            .amount(UPDATED_AMOUNT);
        InvoiceAlphaDTO invoiceAlphaDTO = invoiceAlphaMapper.toDto(updatedInvoiceAlpha);

        restInvoiceAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, invoiceAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(invoiceAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamInvoiceAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceAlphaDTO)))
    ...
    void patchNonExistingInvoiceAlpha() throws Exception {
        ...
        restInvoiceAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, invoiceAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(invoiceAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamInvoiceAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(invoiceAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
