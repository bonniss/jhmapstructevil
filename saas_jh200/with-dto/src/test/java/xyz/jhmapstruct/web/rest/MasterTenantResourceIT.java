...
class MasterTenantResourceIT {
    ...
    void createMasterTenant() throws Exception {
        ...
        MasterTenantDTO masterTenantDTO = masterTenantMapper.toDto(masterTenant);
        var returnedMasterTenantDTO = om.readValue(
            restMasterTenantMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(masterTenantDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            MasterTenantDTO.class
        );

        // Validate the MasterTenant in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedMasterTenant = masterTenantMapper.toEntity(returnedMasterTenantDTO);
    ...
    void putExistingMasterTenant() throws Exception {
        ...
        MasterTenantDTO masterTenantDTO = masterTenantMapper.toDto(updatedMasterTenant);

        restMasterTenantMockMvc
            .perform(
                put(ENTITY_API_URL_ID, masterTenantDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(masterTenantDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamMasterTenant() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMasterTenantMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(masterTenantDTO)))
    ...
    void patchNonExistingMasterTenant() throws Exception {
        ...
        restMasterTenantMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, masterTenantDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(masterTenantDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamMasterTenant() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restMasterTenantMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(masterTenantDTO)))
            .andExpect(status().isMethodNotAllowed());
