...
class EmployeeBetaResourceIT {
    ...
    void createEmployeeBeta() throws Exception {
        ...
        EmployeeBetaDTO employeeBetaDTO = employeeBetaMapper.toDto(employeeBeta);
        var returnedEmployeeBetaDTO = om.readValue(
            restEmployeeBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeBetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            EmployeeBetaDTO.class
        );

        // Validate the EmployeeBeta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedEmployeeBeta = employeeBetaMapper.toEntity(returnedEmployeeBetaDTO);
    ...
    void putExistingEmployeeBeta() throws Exception {
        ...
        updatedEmployeeBeta
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        EmployeeBetaDTO employeeBetaDTO = employeeBetaMapper.toDto(updatedEmployeeBeta);

        restEmployeeBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, employeeBetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(employeeBetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamEmployeeBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeBetaDTO)))
    ...
    void patchNonExistingEmployeeBeta() throws Exception {
        ...
        restEmployeeBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, employeeBetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(employeeBetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEmployeeBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(employeeBetaDTO)))
            .andExpect(status().isMethodNotAllowed());
