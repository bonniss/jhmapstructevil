...
class NextProductResourceIT {
    ...
    void createNextProduct() throws Exception {
        ...
        NextProductDTO nextProductDTO = nextProductMapper.toDto(nextProduct);
        var returnedNextProductDTO = om.readValue(
            restNextProductMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextProductDTO.class
        );

        // Validate the NextProduct in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextProduct = nextProductMapper.toEntity(returnedNextProductDTO);
    ...
    void putExistingNextProduct() throws Exception {
        ...
        NextProductDTO nextProductDTO = nextProductMapper.toDto(updatedNextProduct);

        restNextProductMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextProductDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextProductDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextProduct() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductDTO)))
    ...
    void patchNonExistingNextProduct() throws Exception {
        ...
        restNextProductMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextProductDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextProductDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextProduct() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextProductDTO)))
            .andExpect(status().isMethodNotAllowed());
