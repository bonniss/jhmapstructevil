...
class NextEmployeeThetaResourceIT {
    ...
    void createNextEmployeeTheta() throws Exception {
        ...
        NextEmployeeThetaDTO nextEmployeeThetaDTO = nextEmployeeThetaMapper.toDto(nextEmployeeTheta);
        var returnedNextEmployeeThetaDTO = om.readValue(
            restNextEmployeeThetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeThetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextEmployeeThetaDTO.class
        );

        // Validate the NextEmployeeTheta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextEmployeeTheta = nextEmployeeThetaMapper.toEntity(returnedNextEmployeeThetaDTO);
    ...
    void putExistingNextEmployeeTheta() throws Exception {
        ...
        updatedNextEmployeeTheta
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        NextEmployeeThetaDTO nextEmployeeThetaDTO = nextEmployeeThetaMapper.toDto(updatedNextEmployeeTheta);

        restNextEmployeeThetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextEmployeeThetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextEmployeeThetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextEmployeeTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeThetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextEmployeeThetaDTO)))
    ...
    void patchNonExistingNextEmployeeTheta() throws Exception {
        ...
        restNextEmployeeThetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextEmployeeThetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextEmployeeThetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextEmployeeTheta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextEmployeeThetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextEmployeeThetaDTO)))
            .andExpect(status().isMethodNotAllowed());
