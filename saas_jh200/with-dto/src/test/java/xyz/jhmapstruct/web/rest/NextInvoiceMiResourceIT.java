...
class NextInvoiceMiResourceIT {
    ...
    void createNextInvoiceMi() throws Exception {
        ...
        NextInvoiceMiDTO nextInvoiceMiDTO = nextInvoiceMiMapper.toDto(nextInvoiceMi);
        var returnedNextInvoiceMiDTO = om.readValue(
            restNextInvoiceMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceMiDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextInvoiceMiDTO.class
        );

        // Validate the NextInvoiceMi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextInvoiceMi = nextInvoiceMiMapper.toEntity(returnedNextInvoiceMiDTO);
    ...
    void putExistingNextInvoiceMi() throws Exception {
        ...
        updatedNextInvoiceMi
            ...
            .dueDate(UPDATED_DUE_DATE)
            .amount(UPDATED_AMOUNT);
        NextInvoiceMiDTO nextInvoiceMiDTO = nextInvoiceMiMapper.toDto(updatedNextInvoiceMi);

        restNextInvoiceMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextInvoiceMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextInvoiceMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextInvoiceMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextInvoiceMiDTO)))
    ...
    void patchNonExistingNextInvoiceMi() throws Exception {
        ...
        restNextInvoiceMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextInvoiceMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextInvoiceMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextInvoiceMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextInvoiceMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextInvoiceMiDTO)))
            .andExpect(status().isMethodNotAllowed());
