...
class NextProductBetaResourceIT {
    ...
    void createNextProductBeta() throws Exception {
        ...
        NextProductBetaDTO nextProductBetaDTO = nextProductBetaMapper.toDto(nextProductBeta);
        var returnedNextProductBetaDTO = om.readValue(
            restNextProductBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductBetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextProductBetaDTO.class
        );

        // Validate the NextProductBeta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextProductBeta = nextProductBetaMapper.toEntity(returnedNextProductBetaDTO);
    ...
    void putExistingNextProductBeta() throws Exception {
        ...
        NextProductBetaDTO nextProductBetaDTO = nextProductBetaMapper.toDto(updatedNextProductBeta);

        restNextProductBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextProductBetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextProductBetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextProductBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextProductBetaDTO)))
    ...
    void patchNonExistingNextProductBeta() throws Exception {
        ...
        restNextProductBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextProductBetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextProductBetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextProductBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextProductBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextProductBetaDTO)))
            .andExpect(status().isMethodNotAllowed());
