...
class NextSupplierBetaResourceIT {
    ...
    void createNextSupplierBeta() throws Exception {
        ...
        NextSupplierBetaDTO nextSupplierBetaDTO = nextSupplierBetaMapper.toDto(nextSupplierBeta);
        var returnedNextSupplierBetaDTO = om.readValue(
            restNextSupplierBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierBetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextSupplierBetaDTO.class
        );

        // Validate the NextSupplierBeta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextSupplierBeta = nextSupplierBetaMapper.toEntity(returnedNextSupplierBetaDTO);
    ...
    void putExistingNextSupplierBeta() throws Exception {
        ...
        updatedNextSupplierBeta
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextSupplierBetaDTO nextSupplierBetaDTO = nextSupplierBetaMapper.toDto(updatedNextSupplierBeta);

        restNextSupplierBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextSupplierBetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextSupplierBetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextSupplierBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierBetaDTO)))
    ...
    void patchNonExistingNextSupplierBeta() throws Exception {
        ...
        restNextSupplierBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextSupplierBetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextSupplierBetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextSupplierBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextSupplierBetaDTO)))
            .andExpect(status().isMethodNotAllowed());
