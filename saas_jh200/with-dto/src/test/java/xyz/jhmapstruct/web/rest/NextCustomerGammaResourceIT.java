...
class NextCustomerGammaResourceIT {
    ...
    void createNextCustomerGamma() throws Exception {
        ...
        NextCustomerGammaDTO nextCustomerGammaDTO = nextCustomerGammaMapper.toDto(nextCustomerGamma);
        var returnedNextCustomerGammaDTO = om.readValue(
            restNextCustomerGammaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerGammaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerGammaDTO.class
        );

        // Validate the NextCustomerGamma in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCustomerGamma = nextCustomerGammaMapper.toEntity(returnedNextCustomerGammaDTO);
    ...
    void putExistingNextCustomerGamma() throws Exception {
        ...
        updatedNextCustomerGamma
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextCustomerGammaDTO nextCustomerGammaDTO = nextCustomerGammaMapper.toDto(updatedNextCustomerGamma);

        restNextCustomerGammaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerGammaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerGammaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCustomerGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerGammaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerGammaDTO)))
    ...
    void patchNonExistingNextCustomerGamma() throws Exception {
        ...
        restNextCustomerGammaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerGammaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerGammaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerGamma() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerGammaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerGammaDTO)))
            .andExpect(status().isMethodNotAllowed());
