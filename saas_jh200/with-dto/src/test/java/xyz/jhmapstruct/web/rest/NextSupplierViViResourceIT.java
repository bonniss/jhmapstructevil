...
class NextSupplierViViResourceIT {
    ...
    void createNextSupplierViVi() throws Exception {
        ...
        NextSupplierViViDTO nextSupplierViViDTO = nextSupplierViViMapper.toDto(nextSupplierViVi);
        var returnedNextSupplierViViDTO = om.readValue(
            restNextSupplierViViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierViViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextSupplierViViDTO.class
        );

        // Validate the NextSupplierViVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextSupplierViVi = nextSupplierViViMapper.toEntity(returnedNextSupplierViViDTO);
    ...
    void putExistingNextSupplierViVi() throws Exception {
        ...
        updatedNextSupplierViVi
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextSupplierViViDTO nextSupplierViViDTO = nextSupplierViViMapper.toDto(updatedNextSupplierViVi);

        restNextSupplierViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextSupplierViViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextSupplierViViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextSupplierViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierViViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextSupplierViViDTO)))
    ...
    void patchNonExistingNextSupplierViVi() throws Exception {
        ...
        restNextSupplierViViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextSupplierViViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextSupplierViViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextSupplierViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextSupplierViViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextSupplierViViDTO)))
            .andExpect(status().isMethodNotAllowed());
