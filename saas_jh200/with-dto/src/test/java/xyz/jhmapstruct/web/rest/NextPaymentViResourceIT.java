...
class NextPaymentViResourceIT {
    ...
    void createNextPaymentVi() throws Exception {
        ...
        NextPaymentViDTO nextPaymentViDTO = nextPaymentViMapper.toDto(nextPaymentVi);
        var returnedNextPaymentViDTO = om.readValue(
            restNextPaymentViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentViDTO.class
        );

        // Validate the NextPaymentVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPaymentVi = nextPaymentViMapper.toEntity(returnedNextPaymentViDTO);
    ...
    void putExistingNextPaymentVi() throws Exception {
        ...
        NextPaymentViDTO nextPaymentViDTO = nextPaymentViMapper.toDto(updatedNextPaymentVi);

        restNextPaymentViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPaymentVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentViDTO)))
    ...
    void patchNonExistingNextPaymentVi() throws Exception {
        ...
        restNextPaymentViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentViDTO)))
            .andExpect(status().isMethodNotAllowed());
