...
class NextReviewAlphaResourceIT {
    ...
    void createNextReviewAlpha() throws Exception {
        ...
        NextReviewAlphaDTO nextReviewAlphaDTO = nextReviewAlphaMapper.toDto(nextReviewAlpha);
        var returnedNextReviewAlphaDTO = om.readValue(
            restNextReviewAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextReviewAlphaDTO.class
        );

        // Validate the NextReviewAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextReviewAlpha = nextReviewAlphaMapper.toEntity(returnedNextReviewAlphaDTO);
    ...
    void putExistingNextReviewAlpha() throws Exception {
        ...
        NextReviewAlphaDTO nextReviewAlphaDTO = nextReviewAlphaMapper.toDto(updatedNextReviewAlpha);

        restNextReviewAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextReviewAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextReviewAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextReviewAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextReviewAlphaDTO)))
    ...
    void patchNonExistingNextReviewAlpha() throws Exception {
        ...
        restNextReviewAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextReviewAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextReviewAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextReviewAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextReviewAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextReviewAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
