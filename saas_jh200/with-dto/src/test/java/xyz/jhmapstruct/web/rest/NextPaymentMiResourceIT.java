...
class NextPaymentMiResourceIT {
    ...
    void createNextPaymentMi() throws Exception {
        ...
        NextPaymentMiDTO nextPaymentMiDTO = nextPaymentMiMapper.toDto(nextPaymentMi);
        var returnedNextPaymentMiDTO = om.readValue(
            restNextPaymentMiMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentMiDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentMiDTO.class
        );

        // Validate the NextPaymentMi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPaymentMi = nextPaymentMiMapper.toEntity(returnedNextPaymentMiDTO);
    ...
    void putExistingNextPaymentMi() throws Exception {
        ...
        NextPaymentMiDTO nextPaymentMiDTO = nextPaymentMiMapper.toDto(updatedNextPaymentMi);

        restNextPaymentMiMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentMiDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentMiDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPaymentMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentMiMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentMiDTO)))
    ...
    void patchNonExistingNextPaymentMi() throws Exception {
        ...
        restNextPaymentMiMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentMiDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentMiDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentMi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentMiMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentMiDTO)))
            .andExpect(status().isMethodNotAllowed());
