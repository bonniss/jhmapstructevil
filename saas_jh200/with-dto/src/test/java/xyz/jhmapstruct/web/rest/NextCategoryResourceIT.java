...
class NextCategoryResourceIT {
    ...
    void createNextCategory() throws Exception {
        ...
        NextCategoryDTO nextCategoryDTO = nextCategoryMapper.toDto(nextCategory);
        var returnedNextCategoryDTO = om.readValue(
            restNextCategoryMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCategoryDTO.class
        );

        // Validate the NextCategory in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCategory = nextCategoryMapper.toEntity(returnedNextCategoryDTO);
    ...
    void putExistingNextCategory() throws Exception {
        ...
        NextCategoryDTO nextCategoryDTO = nextCategoryMapper.toDto(updatedNextCategory);

        restNextCategoryMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCategoryDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCategoryDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCategory() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCategoryDTO)))
    ...
    void patchNonExistingNextCategory() throws Exception {
        ...
        restNextCategoryMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCategoryDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCategoryDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCategory() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCategoryMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCategoryDTO)))
            .andExpect(status().isMethodNotAllowed());
