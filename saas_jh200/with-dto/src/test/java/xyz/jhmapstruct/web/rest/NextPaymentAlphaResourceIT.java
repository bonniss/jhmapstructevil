...
class NextPaymentAlphaResourceIT {
    ...
    void createNextPaymentAlpha() throws Exception {
        ...
        NextPaymentAlphaDTO nextPaymentAlphaDTO = nextPaymentAlphaMapper.toDto(nextPaymentAlpha);
        var returnedNextPaymentAlphaDTO = om.readValue(
            restNextPaymentAlphaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentAlphaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextPaymentAlphaDTO.class
        );

        // Validate the NextPaymentAlpha in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextPaymentAlpha = nextPaymentAlphaMapper.toEntity(returnedNextPaymentAlphaDTO);
    ...
    void putExistingNextPaymentAlpha() throws Exception {
        ...
        NextPaymentAlphaDTO nextPaymentAlphaDTO = nextPaymentAlphaMapper.toDto(updatedNextPaymentAlpha);

        restNextPaymentAlphaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextPaymentAlphaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextPaymentAlphaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextPaymentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentAlphaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextPaymentAlphaDTO)))
    ...
    void patchNonExistingNextPaymentAlpha() throws Exception {
        ...
        restNextPaymentAlphaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextPaymentAlphaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextPaymentAlphaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextPaymentAlpha() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextPaymentAlphaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextPaymentAlphaDTO)))
            .andExpect(status().isMethodNotAllowed());
