...
class NextCustomerViResourceIT {
    ...
    void createNextCustomerVi() throws Exception {
        ...
        NextCustomerViDTO nextCustomerViDTO = nextCustomerViMapper.toDto(nextCustomerVi);
        var returnedNextCustomerViDTO = om.readValue(
            restNextCustomerViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            NextCustomerViDTO.class
        );

        // Validate the NextCustomerVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedNextCustomerVi = nextCustomerViMapper.toEntity(returnedNextCustomerViDTO);
    ...
    void putExistingNextCustomerVi() throws Exception {
        ...
        updatedNextCustomerVi
            ...
            .email(UPDATED_EMAIL)
            .phoneNumber(UPDATED_PHONE_NUMBER);
        NextCustomerViDTO nextCustomerViDTO = nextCustomerViMapper.toDto(updatedNextCustomerVi);

        restNextCustomerViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, nextCustomerViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(nextCustomerViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamNextCustomerVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(nextCustomerViDTO)))
    ...
    void patchNonExistingNextCustomerVi() throws Exception {
        ...
        restNextCustomerViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, nextCustomerViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(nextCustomerViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamNextCustomerVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restNextCustomerViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(nextCustomerViDTO)))
            .andExpect(status().isMethodNotAllowed());
