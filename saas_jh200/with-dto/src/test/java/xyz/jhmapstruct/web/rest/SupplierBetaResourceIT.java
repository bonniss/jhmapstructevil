...
class SupplierBetaResourceIT {
    ...
    void createSupplierBeta() throws Exception {
        ...
        SupplierBetaDTO supplierBetaDTO = supplierBetaMapper.toDto(supplierBeta);
        var returnedSupplierBetaDTO = om.readValue(
            restSupplierBetaMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(supplierBetaDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            SupplierBetaDTO.class
        );

        // Validate the SupplierBeta in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedSupplierBeta = supplierBetaMapper.toEntity(returnedSupplierBetaDTO);
    ...
    void putExistingSupplierBeta() throws Exception {
        ...
        SupplierBetaDTO supplierBetaDTO = supplierBetaMapper.toDto(updatedSupplierBeta);

        restSupplierBetaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, supplierBetaDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(supplierBetaDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamSupplierBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSupplierBetaMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(supplierBetaDTO)))
    ...
    void patchNonExistingSupplierBeta() throws Exception {
        ...
        restSupplierBetaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, supplierBetaDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(supplierBetaDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamSupplierBeta() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSupplierBetaMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(supplierBetaDTO)))
            .andExpect(status().isMethodNotAllowed());
