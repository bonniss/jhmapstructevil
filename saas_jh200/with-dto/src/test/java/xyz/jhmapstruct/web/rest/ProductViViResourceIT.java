...
class ProductViViResourceIT {
    ...
    void createProductViVi() throws Exception {
        ...
        ProductViViDTO productViViDTO = productViViMapper.toDto(productViVi);
        var returnedProductViViDTO = om.readValue(
            restProductViViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productViViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            ProductViViDTO.class
        );

        // Validate the ProductViVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedProductViVi = productViViMapper.toEntity(returnedProductViViDTO);
    ...
    void putExistingProductViVi() throws Exception {
        ...
        ProductViViDTO productViViDTO = productViViMapper.toDto(updatedProductViVi);

        restProductViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, productViViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(productViViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamProductViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductViViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(productViViDTO)))
    ...
    void patchNonExistingProductViVi() throws Exception {
        ...
        restProductViViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, productViViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(productViViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamProductViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProductViViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(productViViDTO)))
            .andExpect(status().isMethodNotAllowed());
