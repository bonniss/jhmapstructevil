...
class EmployeeViViResourceIT {
    ...
    void createEmployeeViVi() throws Exception {
        ...
        EmployeeViViDTO employeeViViDTO = employeeViViMapper.toDto(employeeViVi);
        var returnedEmployeeViViDTO = om.readValue(
            restEmployeeViViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeViViDTO)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            EmployeeViViDTO.class
        );

        // Validate the EmployeeViVi in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        var returnedEmployeeViVi = employeeViViMapper.toEntity(returnedEmployeeViViDTO);
    ...
    void putExistingEmployeeViVi() throws Exception {
        ...
        updatedEmployeeViVi
            ...
            .hireDate(UPDATED_HIRE_DATE)
            .position(UPDATED_POSITION);
        EmployeeViViDTO employeeViViDTO = employeeViViMapper.toDto(updatedEmployeeViVi);

        restEmployeeViViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, employeeViViDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(employeeViViDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamEmployeeViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeViViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(employeeViViDTO)))
    ...
    void patchNonExistingEmployeeViVi() throws Exception {
        ...
        restEmployeeViViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, employeeViViDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(employeeViViDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamEmployeeViVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restEmployeeViViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(employeeViViDTO)))
            .andExpect(status().isMethodNotAllowed());
