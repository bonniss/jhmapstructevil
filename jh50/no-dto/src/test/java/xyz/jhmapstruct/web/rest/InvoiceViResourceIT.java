...
class InvoiceViResourceIT {
    ...
    void createInvoiceVi() throws Exception {
        ...
        var returnedInvoiceVi = om.readValue(
            restInvoiceViMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceVi)))
    ...
    void putExistingInvoiceVi() throws Exception {
        ...
        restInvoiceViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedInvoiceVi.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedInvoiceVi))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingInvoiceVi() throws Exception {
        ...
        restInvoiceViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, invoiceVi.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceVi))
            )
            .andExpect(status().isBadRequest());
        ...
        restInvoiceViMockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(invoiceVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamInvoiceVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceViMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(invoiceVi)))
    ...
    void patchNonExistingInvoiceVi() throws Exception {
        ...
        restInvoiceViMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, invoiceVi.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(invoiceVi))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamInvoiceVi() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restInvoiceViMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(invoiceVi)))
            .andExpect(status().isMethodNotAllowed());
